 ...
 * routes HTML. Rôles : 301 canonique workers.dev → taqinor.ma ; (W33) forcer
 * la revalidation des documents HTML pour qu'un déploiement ne serve jamais
 * une page périmée ; et (W315) injecter les en-têtes de sécurité (CSP, HSTS,
 * Referrer-Policy…) sur les documents HTML. Le reste passe à l'app Astro
 * inchangé.
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 4) App Astro. Les documents HTML sont forcés à se revalider (W33) et
    //    reçoivent les en-têtes de sécurité (W315) ; /api/* et toute réponse
