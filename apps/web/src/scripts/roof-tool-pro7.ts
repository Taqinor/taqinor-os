 ...
 * /preview/toiture-3d-pro-7 (V3 du cerveau).
 *
 * COPIE de roof-tool-pro5.ts : pro-5 reste la baseline intacte. Le CHEMIN TOIT
 * PLAT est inchangé (même appel `recommend()` du CERVEAU V2). V3 (estimatorBrainV3)
 * ajoute, par-dessus, sans toucher V2 :
 *  - un bouton « Optimum » qui cale tous les contrôles sur le VRAI meilleur
 *    compromis (recherche pleine : inclinaison fine × azimut × calepinage × marge),
 *    plafonné au besoin ; une option épinglée est tenue, le reste est re-résolu ;
 *  - un toggle « Toit plat / Toit en pente (tuiles) » : en mode pente les panneaux
 *    sont posés AFFLEURANTS, l'inclinaison = la pente et l'azimut = la face du toit
 *    (imposés, lecture seule) ; la production vient de PVGIS à ces valeurs réelles.
 *
 * Reprend le rendu haute fidélité de pro-2 (vrais panneaux Canadian Solar 720 W,
 * Three.js dans une couche MapLibre, vrai sud géo-ancré, vrai soleil/ombres) ; le
 * rendu affleurant réutilise ce moteur avec `flush=true` (sans châssis ni lest).
 * Chaque réglage RE-CALCULE depuis la table de productible committée (instantané) ;
 * PVGIS live n'affine qu'UNE fois la recommandation toit-plat. Le diagnostic enrichi
 * est seulement PRÉ-REMPLI (jamais de lead posté). Aucune nouvelle dépendance.
 *
 * Voir apps/web/BRAIN_V3_NOTES.md, BRAIN_V2_NOTES.md et SOLAR_3D_PRO2_NOTES.md.
 */
...
import {
  fineGridOptimum,
  pvgisCandidatePairs,
  type OptimumV4Result,
} from '../lib/estimatorBrainV4';
...
export function initRoofToolPro7(opts: InitOptions): void {
  if (booted) return;
  booted = true;
  ...
  const mapEl = $('rp7-map');
  const statusEl = $('rp7-status');
  const billEl = $<HTMLInputElement>('rp7-bill');
  const billKwhEl = $('rp7-bill-kwh');
  const finishBtn = $<HTMLButtonElement>('rp7-finish');
  const clearBtn = $<HTMLButtonElement>('rp7-clear');
  const searchForm = $<HTMLFormElement>('rp7-search');
  const addressEl = $<HTMLInputElement>('rp7-address');
  const configPanel = $('rp7-config');
  // W1 : groupe AZIMUT, masqué quand le toit n'est pas tourné (cf. syncAzimuthGroupVisibility).
  const azimuthGroup = $('rp7-azimuth-group');
  const compassArrow = $('rp7-compass-arrow');
  const areaValueEl = $('rp7-area-value');
  const needInputEl = $<HTMLInputElement>('rp7-need-input');
  const needMinusEl = $<HTMLButtonElement>('rp7-need-minus');
  const needPlusEl = $<HTMLButtonElement>('rp7-need-plus');
  const needNoteEl = $('rp7-need-note');
  // V2 : contrôle d'inclinaison (curseur 5–35° + bouton « reco »).
  const tiltRangeEl = $<HTMLInputElement>('rp7-tilt-range');
  const tiltValueEl = $('rp7-tilt-value');
  const tiltRecoBtn = $<HTMLButtonElement>('rp7-tilt-reco');
  const obstacleBtn = $<HTMLButtonElement>('rp7-obstacle');
  const obstacleClearBtn = $<HTMLButtonElement>('rp7-obstacle-clear');
  const obsEditPanel = $('rp7-obs-edit');
  const obsLengthEl = $<HTMLInputElement>('rp7-obs-length');
  const obsWidthEl = $<HTMLInputElement>('rp7-obs-width');
  const obsDimsEl = $('rp7-obs-dims');
  const obsDeleteBtn = $<HTMLButtonElement>('rp7-obs-delete');
  const obsPlusBtn = $<HTMLButtonElement>('rp7-obs-plus');
  const obsMinusBtn = $<HTMLButtonElement>('rp7-obs-minus');
  // V3 : bouton Optimum, toggle type de toit, et contrôles toit en pente.
  const optimumBtn = $<HTMLButtonElement>('rp7-optimum');
  const optimumNoteEl = $('rp7-optimum-note');
  // V4 : carte « Optimum calculé » (PVGIS source de vérité) — sa propre ligne.
  const optimumCard = $('rp7-optimum-card');
  const optimumLabelEl = $('rp7-optimum-label');
  const optimumSourceEl = $('rp7-optimum-source');
  const optimumKwcEl = $('rp7-optimum-kwc');
  const optimumPanelsEl = $('rp7-optimum-panels');
  const optimumProdEl = $('rp7-optimum-prod');
  const optimumCoverEl = $('rp7-optimum-cover');
  const optimumWhyEl = $('rp7-optimum-why');
  const optimumApplyBtn = $<HTMLButtonElement>('rp7-optimum-apply');
  const flatControlsEl = $('rp7-flat-controls');
  const pitchedControlsEl = $('rp7-pitched-controls');
  const pitchRangeEl = $<HTMLInputElement>('rp7-pitch-range');
  const pitchValueEl = $('rp7-pitch-value');
  const pitchedNoteEl = $('rp7-pitched-note');
  if (!mapEl) return;
  ...
  let v4Token = 0;
  let v4Optimum: OptimumV4Result | null = null;
  ...
  function renderScene(pack: PackResult, grid: PanelGrid, tiltDeg: number, family: ConfigFamily, maxCount: number, flush = false) {
    ...
    for (const p of panels) {
      ...
      // Toit en pente (flush) : panneau couché sur la pente, presque sur le toit.
      const pZ = flush ? baseZ + rise / 2 + 0.04 : baseZ + frontStrut + rise / 2 + 0.07;
  ...
  // — Carte / tracé —
  map.on('load', () => {
    map.addSource('rp7-line', { type: 'geojson', data: empty as never });
    map.addSource('rp7-pts', { type: 'geojson', data: empty as never });
    map.addSource('rp7-obs', { type: 'geojson', data: empty as never });
    map.addSource('rp7-obs-preview', { type: 'geojson', data: empty as never });
    map.addLayer({ id: 'rp7-line', type: 'line', source: 'rp7-line', paint: { 'line-color': GOLD, 'line-width': 2.5, 'line-dasharray': [2, 1.5] } });
    map.addLayer({ id: 'rp7-pts', type: 'circle', source: 'rp7-pts', paint: { 'circle-radius': 5, 'circle-color': GOLD, 'circle-stroke-color': '#070b1d', 'circle-stroke-width': 1.5 } });
  ...
  function redrawTrace() {
    srcOf('rp7-line')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: vertices }, properties: {} } as never);
    srcOf('rp7-pts')?.setData({ type: 'FeatureCollection', features: vertices.map((v) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: v }, properties: {} })) } as never);
    if (finishBtn) finishBtn.disabled = vertices.length < 3 || closed;
    updateAreaReadout();
  }

  function redrawObstacles() {
    srcOf('rp7-obs')?.setData({
  ...
  function setPreviewRect(a: LngLat, b: LngLat) {
    ...
    srcOf('rp7-obs-preview')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: ring }, properties: {} } as never);
  }
  function clearPreview() {
    srcOf('rp7-obs-preview')?.setData(empty as never);
  }
  ...
  function obstacleAtPoint(pt: maplibregl.Point): string | null {
    const hits = map.queryRenderedFeatures(pt, { layers: ['rp7-obs'] });
  ...
  function renderSelection() {
    ...
    const setbackM = setbackOf();

    if (useRecommended) {
      const r = rec.recommended;
      // W1 : l'azimut recommandé vit dans rec.recommended.azimuthDeg (180 = plein
      // sud, sinon aligné toit) ; on le passe au pavage pour suivre les arêtes, et
      // la marge de rive courante (toggle) au setback.
    ...
    // W1 : azimut de face selon le groupe AZIMUT. « aligné » suit les arêtes ; pour
    // l'Est-Ouest le faîte aligné = azimut de famille (90) pivoté de l'écart au sud.
    const azimuthDeg =
      sel.azimuth === 'aligned' && rec
        ? family === 'eastwest'
          ? ((rec.roofAlignedAzimuthDeg - 180 + 90) + 360) % 360
          : rec.roofAlignedAzimuthDeg
        : undefined;
    const pack = packConfig(ring, centroidLat, { family, tiltDeg, azimuthDeg, obstructions: obstructionRings(), setbackM });
  ...
  function paintCard(d: CardData, sourceLabel?: string) {
    ...
    set('rp7-reco-title', d.isReco ? `${d.title}  ·  ✓ recommandé` : d.title);
    set('rp7-reco-kwc', `${d.kwc.toLocaleString('fr-FR', { maximumFractionDigits: 1 })} kWc`);
    set('rp7-reco-panels', `${fmt(d.count)} × 720 W`);
    set('rp7-reco-prod', d.annualKwh > 0 ? `${fmt(Math.round(d.annualKwh))} kWh/an` : '—');
    set('rp7-reco-cover', d.pct > 0 ? `${Math.round(d.pct)} %` : '—');
    set('rp7-reco-savings', `${fmtMad(d.savingsLow)} – ${fmtMad(d.savingsHigh)}/an`);
    const why = $('rp7-reco-why');
    if (why) why.textContent = d.why + (sourceLabel ? ` ${sourceLabel}` : '');
    const bif = $('rp7-reco-bifacial');
    if (bif) {
    ...
    $('rp7-results')?.classList.add('rp7-results--ready');
    const cta = $<HTMLButtonElement>('rp7-cta');
    if (cta && d.count > 0) {
      cta.hidden = false;
  ...
  function paintComparison() {
    if (!rec) return;
    const tbody = $('rp7-compare');
    const wrap = $('rp7-compare-wrap');
    if (!tbody) return;
    tbody.innerHTML = '';
    for (const c of rec.comparison) {
      ...
      tr.addEventListener('click', () => {
        ...
        // W1 : une ligne « aligné toit » sélectionne aussi le groupe AZIMUT ; sinon
        // plein sud. La marge courante est conservée.
        const aligned = c.azimuthDeg !== 180;
        // V3 : choisir une ligne épingle sa famille/inclinaison/azimut — « Optimum »
        // les tiendra et re-résoudra le reste.
        pinned.clear();
        pinned.add('family');
        pinned.add('tilt');
        pinned.add('azimuth');
        sel = { family: c.family, tilt: c.tiltDeg, orient: 'auto', azimuth: aligned ? 'aligned' : 'south', margin: sel.margin };
        syncChips();
        renderSelection();
        highlightRow(c.id);
      });
  ...
  function pitchedNote(): string {
    if (!pitchedRec) return '';
    const p = pitchedRec.planes[0];
    return `Inclinaison ${Math.round(p.pitchDeg)}° = pente · azimut ${Math.round(p.facingAzimuthDeg)}° = face (imposés, non balayés). Rendement PVGIS : ~${Math.round(p.perPanelYield)} kWh/kWc/an. Panneaux qui tiennent sur ce pan : ${p.fitCount}.`;
  }
    ...
    paintCard(
      {
        ...
        annualKwh: pitchedRec.totalAnnualKwh,
        pct: pitchedRec.pctOfTarget,
        savingsLow: pitchedRec.savingsLow,
        savingsHigh: pitchedRec.savingsHigh,
        why: pitchedWhy(),
      },
      '(production PVGIS à la pente + face réelles du toit)',
    );
  ...
  function setRoofType(t: RoofType) {
    ...
    // V4 : la carte « Optimum calculé » est propre au toit plat ; recompute la
    // rouvre en mode plat (via computeV4Optimum).
  ...
  function recompute() {
    ...
    // V4 — optimum PVGIS au GPS exact (sa propre ligne « Optimum calculé »).
    if (rec.recommended.count > 0) void computeV4Optimum();
  }
  ...
  // Le VRAI optimum (grille fine) noté sur le rendement PVGIS du GPS EXACT : on
  // préfetch les couples (tilt, aspect) que l'optimiseur interrogera (cache réutilisé
  // entre réglages), puis fineGridOptimum avec la source PVGIS injectée (repli table).
  async function computeV4Optimum() {
    if (!closed || vertices.length < 3 || roofType !== 'flat') return;
    const token = ++v4Token;
    const ring: LngLat[] = [...vertices];
    const bill = monthlyBill();
    const roofAz = roofDominantAzimuthDeg(ring);
    const pairs = pvgisCandidatePairs(centroidLat, roofAz);
    await Promise.all(pairs.map((p) => v4SpecificYield(p.tiltDeg, p.aspect)));
    if (token !== v4Token) return; // un tracé/réglage plus récent a pris la main
    const yieldFn = (tiltDeg: number, aspect: number): number | null => {
      const v = v4YieldCache.get(v4Key(tiltDeg, aspect));
      return v == null ? null : v;
    };
    v4Optimum = fineGridOptimum(ring, centroidLat, bill, obstructionRings(), { yieldFn });
    renderV4Optimum(v4Optimum);
  }

  function renderV4Optimum(opt: OptimumV4Result) {
    if (!optimumCard) return;
    const w = opt.winner;
    optimumCard.hidden = false;
    if (optimumLabelEl) optimumLabelEl.textContent = opt.optimumRow.label;
    if (optimumSourceEl) optimumSourceEl.textContent = opt.yieldSource === 'pvgis' ? 'PVGIS · GPS exact' : 'estimé · table committée';
    ...
    if (optimumWhyEl) optimumWhyEl.textContent = opt.optimumRow.reason;
  }

  // Applique l'optimum V4 (PVGIS) à tous les contrôles, puis recalcule via le moteur.
  function applyV4Optimum() {
    if (!v4Optimum) { applyOptimum(); return; }
    const r = v4Optimum.recommendedOptions;
    useRecommended = false;
    sel = { family: r.family, tilt: r.tiltDeg, orient: r.orientation, azimuth: r.azimuth, margin: r.margin };
    syncChips();
    recompute();
    if (optimumNoteEl) optimumNoteEl.textContent = 'Optimum (PVGIS) : meilleur compromis de la grille fine au GPS exact, plafonné au besoin.';
  }

  // Le bouton « ⚡ Optimum » : en toit plat sans épingle, on cale sur l'optimum
  // PVGIS (V4) ; avec une épingle (ou en mode pente), on garde la ré-optimisation
  // contrainte V3 qui tient l'épingle.
  function applyOptimumSmart() {
    if (roofType === 'flat' && v4Optimum && Object.keys(currentPins()).length === 0) {
      applyV4Optimum();
      return;
    }
    applyOptimum();
  ...
  function close() {
    ...
    srcOf('rp7-line')?.setData(empty as never);
    srcOf('rp7-pts')?.setData(empty as never);
  ...
  function reset() {
    ...
    $('rp7-results')?.classList.remove('rp7-results--ready');
    const cta = $<HTMLButtonElement>('rp7-cta');
  ...
  function updateRecoBadges() {
    const show = (b: Element | null, on: boolean) => {
      const badge = b?.querySelector<HTMLElement>('.rp7-reco-badge');
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-tilt]').forEach((b) => {
    b.addEventListener('click', () => {
      if (b.dataset.tilt === 'reco') {
        useRecommended = true;
        pinned.clear(); // « Recommandé » = on relâche toutes les épingles
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-orient]').forEach((b) => {
    b.addEventListener('click', () => {
      useRecommended = false;
      if (b.dataset.orient === 'auto') pinned.delete('orient');
      else pinned.add('orient');
      sel = { ...sel, orient: b.dataset.orient as OrientMode };
      syncChips();
      renderSelection();
    });
  });
  // W1 — Groupe AZIMUT (plein sud / aligné toit) : ne change que l'aspect, pas la
  // surface utile → re-rendu suffisant (pas de recompute complet).
  document.querySelectorAll<HTMLButtonElement>('[data-azimuth]').forEach((b) => {
    b.addEventListener('click', () => {
      useRecommended = false;
      pinned.add('azimuth');
      sel = { ...sel, azimuth: b.dataset.azimuth as AzimuthMode };
      syncChips();
      renderSelection();
    });
  });
  // W1 — Groupe MARGE (garder / retirer la rive) : change la SURFACE utile → on
  // recalcule tout (panneaux, kWc, kWh/an, % facture, économies, reco et badges).
  document.querySelectorAll<HTMLButtonElement>('[data-margin]').forEach((b) => {
    b.addEventListener('click', () => {
      pinned.add('margin');
      sel = { ...sel, margin: b.dataset.margin as MarginMode };
      syncChips();
      recompute();
    });
  });

  // V3/V4 — Bouton Optimum (cale sur l'optimum PVGIS sans épingle) + toggle type
  // de toit + contrôles toit en pente.
  optimumBtn?.addEventListener('click', applyOptimumSmart);
  optimumApplyBtn?.addEventListener('click', applyV4Optimum);
  document.querySelectorAll<HTMLButtonElement>('[data-rooftype]').forEach((b) => {
