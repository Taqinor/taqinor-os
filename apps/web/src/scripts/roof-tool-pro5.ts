 ...
 * /preview/toiture-3d-pro-5 (V2 du cerveau).
 *
...
export function initRoofToolPro5(opts: InitOptions): void {
  if (booted) return;
  booted = true;
  ...
  const mapEl = $('rp5-map');
  const statusEl = $('rp5-status');
  const billEl = $<HTMLInputElement>('rp5-bill');
  const billKwhEl = $('rp5-bill-kwh');
  const finishBtn = $<HTMLButtonElement>('rp5-finish');
  const clearBtn = $<HTMLButtonElement>('rp5-clear');
  const searchForm = $<HTMLFormElement>('rp5-search');
  const addressEl = $<HTMLInputElement>('rp5-address');
  const configPanel = $('rp5-config');
  // W1 : groupe AZIMUT, masqué quand le toit n'est pas tourné (cf. syncAzimuthGroupVisibility).
  const azimuthGroup = $('rp5-azimuth-group');
  const compassArrow = $('rp5-compass-arrow');
  const areaValueEl = $('rp5-area-value');
  const needInputEl = $<HTMLInputElement>('rp5-need-input');
  const needMinusEl = $<HTMLButtonElement>('rp5-need-minus');
  const needPlusEl = $<HTMLButtonElement>('rp5-need-plus');
  const needNoteEl = $('rp5-need-note');
  // V2 : contrôle d'inclinaison (curseur 5–35° + bouton « reco »).
  const tiltRangeEl = $<HTMLInputElement>('rp5-tilt-range');
  const tiltValueEl = $('rp5-tilt-value');
  const tiltRecoBtn = $<HTMLButtonElement>('rp5-tilt-reco');
  const obstacleBtn = $<HTMLButtonElement>('rp5-obstacle');
  const obstacleClearBtn = $<HTMLButtonElement>('rp5-obstacle-clear');
  const obsEditPanel = $('rp5-obs-edit');
  const obsLengthEl = $<HTMLInputElement>('rp5-obs-length');
  const obsWidthEl = $<HTMLInputElement>('rp5-obs-width');
  const obsDimsEl = $('rp5-obs-dims');
  const obsDeleteBtn = $<HTMLButtonElement>('rp5-obs-delete');
  const obsPlusBtn = $<HTMLButtonElement>('rp5-obs-plus');
  const obsMinusBtn = $<HTMLButtonElement>('rp5-obs-minus');
  if (!mapEl) return;
  ...
  map.on('error', (e: unknown) => {
    ...
    console.warn('[roof-tool-pro5] erreur carte (non bloquante) :', msg);
  });
  ...
  // — Carte / tracé —
  map.on('load', () => {
    map.addSource('rp5-line', { type: 'geojson', data: empty as never });
    map.addSource('rp5-pts', { type: 'geojson', data: empty as never });
    map.addSource('rp5-obs', { type: 'geojson', data: empty as never });
    map.addSource('rp5-obs-preview', { type: 'geojson', data: empty as never });
    map.addLayer({ id: 'rp5-line', type: 'line', source: 'rp5-line', paint: { 'line-color': GOLD, 'line-width': 2.5, 'line-dasharray': [2, 1.5] } });
    map.addLayer({ id: 'rp5-pts', type: 'circle', source: 'rp5-pts', paint: { 'circle-radius': 5, 'circle-color': GOLD, 'circle-stroke-color': '#070b1d', 'circle-stroke-width': 1.5 } });
  ...
  function redrawTrace() {
    srcOf('rp5-line')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: vertices }, properties: {} } as never);
    srcOf('rp5-pts')?.setData({ type: 'FeatureCollection', features: vertices.map((v) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: v }, properties: {} })) } as never);
    if (finishBtn) finishBtn.disabled = vertices.length < 3 || closed;
    updateAreaReadout();
  }

  function redrawObstacles() {
    srcOf('rp5-obs')?.setData({
  ...
  function setPreviewRect(a: LngLat, b: LngLat) {
    ...
    srcOf('rp5-obs-preview')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: ring }, properties: {} } as never);
  }
  function clearPreview() {
    srcOf('rp5-obs-preview')?.setData(empty as never);
  }
  ...
  function obstacleAtPoint(pt: maplibregl.Point): string | null {
    const hits = map.queryRenderedFeatures(pt, { layers: ['rp5-obs'] });
  ...
  function paintCard(d: CardData, sourceLabel?: string) {
    ...
    set('rp5-reco-title', d.isReco ? `${d.title}  ·  ✓ recommandé` : d.title);
    set('rp5-reco-kwc', `${d.kwc.toLocaleString('fr-FR', { maximumFractionDigits: 1 })} kWc`);
    set('rp5-reco-panels', `${fmt(d.count)} × 720 W`);
    set('rp5-reco-prod', d.annualKwh > 0 ? `${fmt(Math.round(d.annualKwh))} kWh/an` : '—');
    set('rp5-reco-cover', d.pct > 0 ? `${Math.round(d.pct)} %` : '—');
    set('rp5-reco-savings', `${fmtMad(d.savingsLow)} – ${fmtMad(d.savingsHigh)}/an`);
    const why = $('rp5-reco-why');
    if (why) why.textContent = d.why + (sourceLabel ? ` ${sourceLabel}` : '');
    const bif = $('rp5-reco-bifacial');
    if (bif) {
    ...
    $('rp5-results')?.classList.add('rp5-results--ready');
    const cta = $<HTMLButtonElement>('rp5-cta');
    if (cta && d.count > 0) {
      cta.hidden = false;
  ...
  function paintComparison() {
    if (!rec) return;
    const tbody = $('rp5-compare');
    const wrap = $('rp5-compare-wrap');
    if (!tbody) return;
    tbody.innerHTML = '';
  ...
  function close() {
    ...
    srcOf('rp5-line')?.setData(empty as never);
    srcOf('rp5-pts')?.setData(empty as never);
  ...
  function reset() {
    ...
    $('rp5-results')?.classList.remove('rp5-results--ready');
    const cta = $<HTMLButtonElement>('rp5-cta');
  ...
  function updateRecoBadges() {
    const show = (b: Element | null, on: boolean) => {
      const badge = b?.querySelector<HTMLElement>('.rp5-reco-badge');
