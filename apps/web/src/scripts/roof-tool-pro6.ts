 ...
 * /preview/toiture-3d-pro-6 (V3 du cerveau).
 *
...
export function initRoofToolPro6(opts: InitOptions): void {
  if (booted) return;
  booted = true;
  ...
  const mapEl = $('rp6-map');
  const statusEl = $('rp6-status');
  const billEl = $<HTMLInputElement>('rp6-bill');
  const billKwhEl = $('rp6-bill-kwh');
  const finishBtn = $<HTMLButtonElement>('rp6-finish');
  const clearBtn = $<HTMLButtonElement>('rp6-clear');
  const searchForm = $<HTMLFormElement>('rp6-search');
  const addressEl = $<HTMLInputElement>('rp6-address');
  const configPanel = $('rp6-config');
  // W1 : groupe AZIMUT, masqué quand le toit n'est pas tourné (cf. syncAzimuthGroupVisibility).
  const azimuthGroup = $('rp6-azimuth-group');
  const compassArrow = $('rp6-compass-arrow');
  const areaValueEl = $('rp6-area-value');
  const needInputEl = $<HTMLInputElement>('rp6-need-input');
  const needMinusEl = $<HTMLButtonElement>('rp6-need-minus');
  const needPlusEl = $<HTMLButtonElement>('rp6-need-plus');
  const needNoteEl = $('rp6-need-note');
  // V2 : contrôle d'inclinaison (curseur 5–35° + bouton « reco »).
  const tiltRangeEl = $<HTMLInputElement>('rp6-tilt-range');
  const tiltValueEl = $('rp6-tilt-value');
  const tiltRecoBtn = $<HTMLButtonElement>('rp6-tilt-reco');
  const obstacleBtn = $<HTMLButtonElement>('rp6-obstacle');
  const obstacleClearBtn = $<HTMLButtonElement>('rp6-obstacle-clear');
  const obsEditPanel = $('rp6-obs-edit');
  const obsLengthEl = $<HTMLInputElement>('rp6-obs-length');
  const obsWidthEl = $<HTMLInputElement>('rp6-obs-width');
  const obsDimsEl = $('rp6-obs-dims');
  const obsDeleteBtn = $<HTMLButtonElement>('rp6-obs-delete');
  const obsPlusBtn = $<HTMLButtonElement>('rp6-obs-plus');
  const obsMinusBtn = $<HTMLButtonElement>('rp6-obs-minus');
  // V3 : bouton Optimum, toggle type de toit, et contrôles toit en pente.
  const optimumBtn = $<HTMLButtonElement>('rp6-optimum');
  const optimumNoteEl = $('rp6-optimum-note');
  const flatControlsEl = $('rp6-flat-controls');
  const pitchedControlsEl = $('rp6-pitched-controls');
  const pitchRangeEl = $<HTMLInputElement>('rp6-pitch-range');
  const pitchValueEl = $('rp6-pitch-value');
  const pitchedNoteEl = $('rp6-pitched-note');
  if (!mapEl) return;
  ...
  // — Carte / tracé —
  map.on('load', () => {
    map.addSource('rp6-line', { type: 'geojson', data: empty as never });
    map.addSource('rp6-pts', { type: 'geojson', data: empty as never });
    map.addSource('rp6-obs', { type: 'geojson', data: empty as never });
    map.addSource('rp6-obs-preview', { type: 'geojson', data: empty as never });
    map.addLayer({ id: 'rp6-line', type: 'line', source: 'rp6-line', paint: { 'line-color': GOLD, 'line-width': 2.5, 'line-dasharray': [2, 1.5] } });
    map.addLayer({ id: 'rp6-pts', type: 'circle', source: 'rp6-pts', paint: { 'circle-radius': 5, 'circle-color': GOLD, 'circle-stroke-color': '#070b1d', 'circle-stroke-width': 1.5 } });
  ...
  function redrawTrace() {
    srcOf('rp6-line')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: vertices }, properties: {} } as never);
    srcOf('rp6-pts')?.setData({ type: 'FeatureCollection', features: vertices.map((v) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: v }, properties: {} })) } as never);
    if (finishBtn) finishBtn.disabled = vertices.length < 3 || closed;
    updateAreaReadout();
  }

  function redrawObstacles() {
    srcOf('rp6-obs')?.setData({
  ...
  function setPreviewRect(a: LngLat, b: LngLat) {
    ...
    srcOf('rp6-obs-preview')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: ring }, properties: {} } as never);
  }
  function clearPreview() {
    srcOf('rp6-obs-preview')?.setData(empty as never);
  }
  ...
  function obstacleAtPoint(pt: maplibregl.Point): string | null {
    const hits = map.queryRenderedFeatures(pt, { layers: ['rp6-obs'] });
  ...
  function paintCard(d: CardData, sourceLabel?: string) {
    ...
    set('rp6-reco-title', d.isReco ? `${d.title}  ·  ✓ recommandé` : d.title);
    set('rp6-reco-kwc', `${d.kwc.toLocaleString('fr-FR', { maximumFractionDigits: 1 })} kWc`);
    set('rp6-reco-panels', `${fmt(d.count)} × 720 W`);
    set('rp6-reco-prod', d.annualKwh > 0 ? `${fmt(Math.round(d.annualKwh))} kWh/an` : '—');
    set('rp6-reco-cover', d.pct > 0 ? `${Math.round(d.pct)} %` : '—');
    set('rp6-reco-savings', `${fmtMad(d.savingsLow)} – ${fmtMad(d.savingsHigh)}/an`);
    const why = $('rp6-reco-why');
    if (why) why.textContent = d.why + (sourceLabel ? ` ${sourceLabel}` : '');
    const bif = $('rp6-reco-bifacial');
    if (bif) {
    ...
    $('rp6-results')?.classList.add('rp6-results--ready');
    const cta = $<HTMLButtonElement>('rp6-cta');
    if (cta && d.count > 0) {
      cta.hidden = false;
  ...
  function paintComparison() {
    if (!rec) return;
    const tbody = $('rp6-compare');
    const wrap = $('rp6-compare-wrap');
    if (!tbody) return;
    tbody.innerHTML = '';
  ...
  function close() {
    ...
    srcOf('rp6-line')?.setData(empty as never);
    srcOf('rp6-pts')?.setData(empty as never);
  ...
  function reset() {
    ...
    $('rp6-results')?.classList.remove('rp6-results--ready');
    const cta = $<HTMLButtonElement>('rp6-cta');
  ...
  function updateRecoBadges() {
    const show = (b: Element | null, on: boolean) => {
      const badge = b?.querySelector<HTMLElement>('.rp6-reco-badge');
  ...
  // V3 — Bouton Optimum + toggle type de toit + contrôles toit en pente.
  optimumBtn?.addEventListener('click', applyOptimum);
  document.querySelectorAll<HTMLButtonElement>('[data-rooftype]').forEach((b) => {
