 ...
 * /preview/toiture-3d-pro-8 (V3 du cerveau).
 *
...
export function initRoofToolPro8(opts: InitOptions): void {
  ...
  const mapEl = $('rp8-map');
  const statusEl = $('rp8-status');
  const billEl = $<HTMLInputElement>('rp8-bill');
  const billKwhEl = $('rp8-bill-kwh');
  const finishBtn = $<HTMLButtonElement>('rp8-finish');
  const clearBtn = $<HTMLButtonElement>('rp8-clear');
  const searchForm = $<HTMLFormElement>('rp8-search');
  const addressEl = $<HTMLInputElement>('rp8-address');
  const configPanel = $('rp8-config');
  // W1 : groupe AZIMUT, masqué quand le toit n'est pas tourné (cf. syncAzimuthGroupVisibility).
  const azimuthGroup = $('rp8-azimuth-group');
  const compassArrow = $('rp8-compass-arrow');
  const areaValueEl = $('rp8-area-value');
  const needInputEl = $<HTMLInputElement>('rp8-need-input');
  const needMinusEl = $<HTMLButtonElement>('rp8-need-minus');
  const needPlusEl = $<HTMLButtonElement>('rp8-need-plus');
  const needNoteEl = $('rp8-need-note');
  // V2 : contrôle d'inclinaison (curseur 5–35° + bouton « reco »).
  const tiltRangeEl = $<HTMLInputElement>('rp8-tilt-range');
  const tiltValueEl = $('rp8-tilt-value');
  const tiltRecoBtn = $<HTMLButtonElement>('rp8-tilt-reco');
  const obstacleBtn = $<HTMLButtonElement>('rp8-obstacle');
  const obstacleClearBtn = $<HTMLButtonElement>('rp8-obstacle-clear');
  const obsEditPanel = $('rp8-obs-edit');
  const obsLengthEl = $<HTMLInputElement>('rp8-obs-length');
  const obsWidthEl = $<HTMLInputElement>('rp8-obs-width');
  const obsDimsEl = $('rp8-obs-dims');
  const obsDeleteBtn = $<HTMLButtonElement>('rp8-obs-delete');
  const obsPlusBtn = $<HTMLButtonElement>('rp8-obs-plus');
  const obsMinusBtn = $<HTMLButtonElement>('rp8-obs-minus');
  // V3 : bouton Optimum, toggle type de toit, et contrôles toit en pente.
  const optimumBtn = $<HTMLButtonElement>('rp8-optimum');
  const optimumNoteEl = $('rp8-optimum-note');
  // V4 : carte « Optimum calculé » (PVGIS source de vérité) — sa propre ligne.
  const optimumCard = $('rp8-optimum-card');
  const optimumLabelEl = $('rp8-optimum-label');
  const optimumSourceEl = $('rp8-optimum-source');
  const optimumKwcEl = $('rp8-optimum-kwc');
  const optimumPanelsEl = $('rp8-optimum-panels');
  const optimumProdEl = $('rp8-optimum-prod');
  const optimumCoverEl = $('rp8-optimum-cover');
  const optimumWhyEl = $('rp8-optimum-why');
  const optimumApplyBtn = $<HTMLButtonElement>('rp8-optimum-apply');
  const flatControlsEl = $('rp8-flat-controls');
  const pitchedControlsEl = $('rp8-pitched-controls');
  const pitchRangeEl = $<HTMLInputElement>('rp8-pitch-range');
  const pitchValueEl = $('rp8-pitch-value');
  const pitchedNoteEl = $('rp8-pitched-note');
  if (!mapEl) return;
  ...
  // — Carte / tracé —
  map.on('load', () => {
    map.addSource('rp8-line', { type: 'geojson', data: empty as never });
    map.addSource('rp8-pts', { type: 'geojson', data: empty as never });
    map.addSource('rp8-obs', { type: 'geojson', data: empty as never });
    map.addSource('rp8-obs-preview', { type: 'geojson', data: empty as never });
    map.addLayer({ id: 'rp8-line', type: 'line', source: 'rp8-line', paint: { 'line-color': GOLD, 'line-width': 2.5, 'line-dasharray': [2, 1.5] } });
    map.addLayer({ id: 'rp8-pts', type: 'circle', source: 'rp8-pts', paint: { 'circle-radius': 5, 'circle-color': GOLD, 'circle-stroke-color': '#070b1d', 'circle-stroke-width': 1.5 } });
  ...
  function redrawTrace() {
    srcOf('rp8-line')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: vertices }, properties: {} } as never);
    srcOf('rp8-pts')?.setData({ type: 'FeatureCollection', features: vertices.map((v) => ({ type: 'Feature', geometry: { type: 'Point', coordinates: v }, properties: {} })) } as never);
    if (finishBtn) finishBtn.disabled = vertices.length < 3 || closed;
    updateAreaReadout();
  }

  function redrawObstacles() {
    srcOf('rp8-obs')?.setData({
  ...
  function setPreviewRect(a: LngLat, b: LngLat) {
    ...
    srcOf('rp8-obs-preview')?.setData({ type: 'Feature', geometry: { type: 'LineString', coordinates: ring }, properties: {} } as never);
  }
  function clearPreview() {
    srcOf('rp8-obs-preview')?.setData(empty as never);
  }
  ...
  function obstacleAtPoint(pt: maplibregl.Point): string | null {
    const hits = map.queryRenderedFeatures(pt, { layers: ['rp8-obs'] });
  ...
  function paintCard(d: CardData, sourceLabel?: string) {
    ...
    set('rp8-reco-title', d.isReco ? `${d.title}  ·  ✓ recommandé` : d.title);
    set('rp8-reco-kwc', `${d.kwc.toLocaleString('fr-FR', { maximumFractionDigits: 1 })} kWc`);
    set('rp8-reco-panels', `${fmt(d.count)} × 720 W`);
    set('rp8-reco-prod', d.annualKwh > 0 ? `${fmt(Math.round(d.annualKwh))} kWh/an` : '—');
    set('rp8-reco-cover', d.pct > 0 ? `${Math.round(d.pct)} %` : '—');
    set('rp8-reco-savings', `${fmtMad(d.savingsLow)} – ${fmtMad(d.savingsHigh)}/an`);
    const why = $('rp8-reco-why');
    if (why) why.textContent = d.why + (sourceLabel ? ` ${sourceLabel}` : '');
    const bif = $('rp8-reco-bifacial');
    if (bif) {
    ...
    $('rp8-results')?.classList.add('rp8-results--ready');
    const cta = $<HTMLButtonElement>('rp8-cta');
    if (cta && d.count > 0) {
      cta.hidden = false;
  ...
  function paintComparison() {
    if (!rec) return;
    const tbody = $('rp8-compare');
    const wrap = $('rp8-compare-wrap');
    if (!tbody) return;
    tbody.innerHTML = '';
  ...
  function close() {
    ...
    srcOf('rp8-line')?.setData(empty as never);
    srcOf('rp8-pts')?.setData(empty as never);
  ...
  function reset() {
    ...
    $('rp8-results')?.classList.remove('rp8-results--ready');
    const cta = $<HTMLButtonElement>('rp8-cta');
  ...
  function updateRecoBadges() {
    const show = (b: Element | null, on: boolean) => {
      const badge = b?.querySelector<HTMLElement>('.rp8-reco-badge');
