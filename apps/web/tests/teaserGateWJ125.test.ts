...
describe.each(Object.entries(PAGES))('WJ125 — %s mon-toit.astro : rendu public gaté', (_label, src) => {
  ...
  it('l\'aperçu 3D est re-masquable d\'un drapeau et le point de contrôle unique tient (PUBLIC_3D_PREVIEW)', () => {
    // Décision fondateur 13-14/09/2026 (fil Meta ads, modèle « Palmetto ») : le
    // bloc 3D — VISUEL seul, aucun chiffre écrit dans le DOM (le compte de
    // panneaux ne se lit qu'en comptant des mailles WebGL, pas en lisant le
    // DOM) — redevient visible AVANT la porte de contact. La visibilité reste
    // contrôlée au point unique updatePanels3dVisibility : PUBLIC_3D_PREVIEW à
    // false re-force le masquage sous gate (ancien comportement, un seul mot).
    const start = src.indexOf('function updatePanels3dVisibility(');
    expect(start).toBeGreaterThan(-1);
    const fn = src.slice(start, start + 700);
    expect(fn).toContain('if (PUBLIC_ESTIMATE_GATED && !PUBLIC_3D_PREVIEW) { panels3dBlock.hidden = true; return; }');
    // Le drapeau est déclaré à côté du gate (réversibilité en un mot).
    expect(src).toContain('const PUBLIC_3D_PREVIEW: boolean');
  ...
  it('les 3 chemins « succès » (résidentiel/pro/agricole) s\'arrêtent AVANT d\'écrire un chiffre', () => {
    // 3 retours gatés des chemins succès (le garde 3D est passé sur le motif
    // combiné PUBLIC_ESTIMATE_GATED && !PUBLIC_3D_PREVIEW — voir le test
    // ci-dessus). Le VRAI compte des retours succès reste 3 — vérifié par les
    // 3 GATED_ANNOUNCE ci-dessous.
    const gatedBlocks = (src.match(/if \(PUBLIC_ESTIMATE_GATED\) \{/g) ?? []).length;
    expect(gatedBlocks).toBe(3);
