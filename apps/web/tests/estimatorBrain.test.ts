...
describe('billToAnnualKwh — facture mensuelle → conso annuelle (base du site, 1,4 MAD/kWh)', () => {
  it('1 400 MAD/mois ≈ 12 000 kWh/an', () => {
    expect(billToAnnualKwh(1400)).toBeCloseTo((1400 * 12) / 1.4, 0);
    expect(billToAnnualKwh(1400)).toBeCloseTo(12000, 0);
  });
...
describe('recommend — l’algorithme (les 3 branches)', () => {
  ...
  it('BRANCHE « densifie » : sud optimal ne couvre pas mais une config plus dense oui', () => {
    // side 14 : sud-opt ≈ 24 960 kWh < cible 3 000 MAD (≈ 25 700) < sud-15 ≈ 29 880.
    const ring = squareRing(14);
    const rec = recommend(ring, 33.59, 3000);
