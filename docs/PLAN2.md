...
- [ ] N12 — Bon de commande PDF for the supplier (French): SKU, description, quantity, unit buy price, company identity from Paramètres; internal-only, never sent to clients.
- [ ] N13 — Besoin matériel view per chantier listing components needed from its source devis with current stock availability per item, flagging shortfalls, plus one-click action drafting a bon de commande for the shortfall lines to the relevant supplier.
...
- [ ] N55 — Admin audit of settings changes (who/what/when, existing audit pattern).
