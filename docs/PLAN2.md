...
**LANE LB10R — retours fondateur du 2026-07-20 05h (cockpit UNE ligne + vues de compte) :**

- [x] LB46 — **Desktop : TOUT le cockpit en UNE ligne large.** « je ne vois même pas un lead complet — trop de choses en haut » : fusionner la rangée KPI ET la rangée vues enregistrées DANS la ligne de contrôle : titre+compteur → recherche+facettes+« Filtres ▾ » → KPI en CHIPS compactes inline (nombre+libellé sur une ligne, plus des tuiles hautes ; « Prév. » MASQUÉ — le total Pipeline MAD seul, prévisionnel en tooltip) → chips de vues enregistrées inline → ⋯ / + Nouveau / sélecteur. Repli gracieux en 2 lignes MINCES sous ~1450px (« one wide line looks better »). Files : LeadsPage.jsx, LeadsKpiStrip.jsx, SavedViewsBar.jsx (variante inline), index.css. (ROUTINE — M) (@model: opus) (@lane: LB10R)
- [x] LB47 — **Phone : UNE seule ligne de chrome, le reste dans ⋯ (+ LB45 absorbé).** Ligne unique : titre+compteur → recherche (icône qui déplie, patron Odoo SearchBarToggler) → ⋯ → + Nouveau. Le sélecteur de vues devient des ITEMS du menu ⋯ sur mobile (LB45 absorbé) ; chips Mes leads/Rappels + KPI-filtres déménagent EN TÊTE du panneau « Filtres » (VX223/224 « toujours visibles » SUPPLANTÉS par cette instruction fondateur explicite — noté ici). Files : LeadsPage.jsx, FilterBar.jsx, ViewSwitcher.jsx, LeadsKpiStrip.jsx, index.css. (ROUTINE — M) (@model: opus) (@lane: LB10R)
- [x] LB48 — **Backend : vues enregistrées DE COMPTE (crm.SavedView).** Modèle company+user+page ('crm.leads')+name+rank+payload JSON (filters+view), contrainte unicité (user,page,name) ; ViewSet company-scopé (queryset filtré request.user.company + company/user forcés en perform_create — JAMAIS du body), actions reorder (liste d'ids → ranks) ; migration additive ; tests API (tenant, rank, CRUD). Files : apps/crm/models.py, serializers.py, views.py, urls.py, migration, tests. (SCHEMA — M) (@model: sonnet) (@lane: LB10R-BE)
- [x] LB49 — **Frontend : vues liées au compte — défaut rang 1 à chaque connexion, continuité de session.** SavedViews du leads passe de localStorage à l'API LB48 : toutes les vues listées, réordonnables (la n°1 = défaut appliqué à toute NOUVELLE connexion sans URL) ; la vue/les filtres COURANTS persistent en sessionStorage (aller-retour vers un autre module = on retrouve l'état exact) — priorité : URL > session > vue rang 1 > défauts. urlFilters/pins/e2e adaptés. Files : hooks/useSavedViews.js (variante serveur), pages/crm/leads/urlFilters.js, LeadsPage.jsx, SavedViewsBar.jsx, api. (ROUTINE — M) (@model: opus) (@lane: LB10R)

**LANE LB11R — « copie les meilleurs du monde » (fondateur 2026-07-20 06h ; blueprint Fable d'après 5 études : Attio/Folk/Twenty, Linear/Notion/Airtable, HubSpot/Pipedrive/Salesforce — docs/design/leads-cockpit-blueprint.md) :**

- [x] LB50 — **LeadViewPicker : le titre DEVIENT le sélecteur de vues (patron Twenty/Attio).** `[Pipeline · 143 ▾]` : trigger = nom de la vue active + compteur filtré + chevron ; menu = vues du compte (★ rang 1, ▲▼ réordonner, 🔗 copier le lien, ✕ supprimer) + « ⭐ Enregistrer la vue actuelle… ». Remplace le h2, les chips SavedViewsBar inline ET l'item ⋯ — desktop ET mobile. Files : LeadViewPicker.jsx (nouveau), LeadsPage.jsx, index.css. (ROUTINE — M) (@model: opus) (@lane: LB11R)
- [x] LB51 — **QuickFilterChips : tuiles KPI + chips fréquentes = UN jeu de chips comptées.** Dû aujourd'hui/En retard/Chauds (toggles facettés sur kpiPool) fusionnés avec Mes leads/☎ Rappels ; badge numérique rendu SEULEMENT si > 0 ; Pipeline MAD réduit à un texte discret (PipelineTotal, prévisionnel en infobulle, masqué < 1600px). Files : QuickFilterChips.jsx + PipelineTotal.jsx (nouveaux), LeadsKpiStrip.jsx (supprimé), FilterBar.jsx, LeadsPage.jsx. (ROUTINE — M) (@model: opus) (@lane: LB11R)
- [x] LB52 — **Rangée facettes CONDITIONNELLE (Twenty ViewBarDetails / Linear).** Les facettes desktop quittent la barre pour une rangée sœur 32px qui rend `null` à 0 facette (physiquement hors DOM) + « Effacer les filtres » ; calcul extrait en helper pur `leadFacets.js` partagé avec le panneau mobile. Files : leadFacets.js (nouveau), FilterBar.jsx, LeadsPage.jsx, index.css. (ROUTINE — S) (@model: opus) (@lane: LB11R)
- [x] LB53 — **Contrat CSS chrome : ~52px au repos.** Rangée contrôle 44px, facettes 32px, repli 2 lignes minces < 1450px, PipelineTotal masqué < 1600px ; contrat data-view intouché. Files : index.css. DoD : chrome mesuré ≤ 100px au repos à 1920. (ROUTINE — S) (@model: opus) (@lane: LB11R)
- [x] LB54 — **Mobile : `[picker|🔍|Filtres|⋯]`.** Le picker remplace le titre nu (vues du compte au pouce) ; les items SavedView SORTENT de ⋯ (les 6 types de vue y restent) ; FAB inchangé. Files : LeadsPage.jsx. (ROUTINE — S) (@model: opus) (@lane: LB11R)
- [x] LB55 — **E2E/pins : « Pipeline » heading→button, tuiles→chips.** Retarget des specs/pins au nouveau DOM (radios « Vue kanban »…, '+ Nouveau lead', bulk inchangés). Files : e2e specs + .mjs pins. (ROUTINE — S) (@model: opus) (@lane: LB11R)
- [x] LB56 — **Régression continuité : URL > session > rang 1 > kanban.** Test verrouillant les 4 priorités après le refactor picker. Files : LeadsPageUrlFilters.test.mjs. (ROUTINE — S) (@model: opus) (@lane: LB11R)

#### DONE LOG — Groupe LB (page leads)

- 2026-07-20 (06h-08h) — LB50-LB56 (« copie les meilleurs du monde ») : 5 études parallèles (Attio/Folk/Twenty source-vérifié, Linear/Notion/Airtable, HubSpot/Pipedrive/Salesforce) + synthèse Fable (2e appel du run) → blueprint docs/design/leads-cockpit-blueprint.md. Livré : LeadViewPicker (le titre EST le sélecteur de vues — nom+compteur+chevron, menu ★/▲▼/🔗/✕ + Enregistrer), QuickFilterChips (tuiles KPI + chips fréquentes fusionnées, badges seulement si >0), PipelineTotal discret (Prév en infobulle, masqué <1600px), rangée facettes CONDITIONNELLE hors-DOM à vide (helper pur leadFacets.js), contrat CSS. **Chrome mesuré au rig : 61px au repos desktop (Twenty : 87), 44px mobile, board 70% de l'écran téléphone.** LeadsKpiStrip supprimé ; pins/e2e retargetés (« Pipeline » heading→button picker). 15/15 sondes navigateur réel. (ROUTINE)

- 2026-07-20 (05h-06h) — LB46-LB49 (cockpit une ligne + vues de compte, retours fondateur à chaud) : KPI en chips compactes SANS Prév (tooltip) fondues dans la ligne de contrôle avec les chips de vues (▲▼ + ★ rang 1) ; téléphone réduit à [titre|🔍 dépliable|Filtres|⋯] (⋯ = Express/Doublons/Import/Export/Enregistrer + changement de vue + vues du compte ; KPI et chips dans le panneau Filtres ; création via FAB au nom accessible « + Nouveau lead ») ; backend crm.SavedView (TenantModel SCA4 après rouge check_platform, migration 0067 RÉGÉNÉRÉE via makemigrations — jamais à la main, unique (user,page,name), action reorder atomique, tests API) ; persistance session (sessionStorage) + défaut de connexion = vue rang 1 (URL > session > rang 1 > défauts). (SCHEMA)
...
- [x] ODY1 — **La source de vérité « mes apps » : un hook unique `useInstalledApps()`.** Créer `frontend/src/lib/apps/useInstalledApps.js` croisant, en UN endroit : le registre (`moduleConfigs` de `frontend/src/router/moduleRoutes.jsx`), les modules actifs société (`selectModulesDesactives` de `frontend/src/router/moduleGating.js`, alimenté par `/auth/me/` — ODX6) et les gardes rôle/permission (`useHasRole`/`useHasPermission` de `frontend/src/hooks/useHasPermission.js` — convention ARC47) → liste ordonnée d'apps visibles `{key, label FR, icon, accent (VX8), to (cockpit), description}`. Pour CRM, `to` résout `/crm/leads` (porte fondateur 2026-08-01, verrouillée par le test APX1 — jamais un écran cockpit). Brancher IMMÉDIATEMENT `AppLauncher.jsx` (constat vérifié `AppLauncher.jsx:14-16` : il affiche AUJOURD'HUI tous les `moduleConfigs`, apps désactivées comprises — le câblage ODX6 promis n'a jamais été fait) et `PinnedApps.jsx` dessus. **Done =** désactiver une app en Paramètres la fait disparaître du launcher et des épinglés (tests vitest rôle × module OFF), zéro nouvelle source de liste d'apps (grep). Files: frontend/src/lib/apps/useInstalledApps.js (nouveau), frontend/src/components/layout/AppLauncher.jsx, frontend/src/components/layout/PinnedApps.jsx. (ROUTINE — M, sonnet) (@lane: frontend/apps-core)
- [x] ODY2 — **Le Menu d'accueil plein écran (la porte d'entrée).** Créer `frontend/src/pages/home/HomeMenu.jsx` (route `/apps` — vérifiée libre) : grille des apps de `useInstalledApps()` UNIQUEMENT — `AppIcon` (ODY9, placeholder simple en attendant), label FR, badge (ODY10 plus tard) ; type-ahead à la Odoo (taper filtre la grille, Entrée ouvre la première, flèches naviguent, Échap efface), Favoris en tête (MÊME clé localStorage que VX9/VX10) puis 3 Récents ; fond signature « Lumière sur Nuit » (nuit + halo brass ≤8 % — seul écran autorisé au gradient avec ModuleHero) ; AUCUN fetch bloquant (tout vient du bootstrap + registre). **Done =** grille = installées ∩ rôle exactement, navigation 100 % clavier, AA clair/sombre, reduced-motion, tests vitest (filtrage, type-ahead). Files: frontend/src/pages/home/HomeMenu.jsx (nouveau), frontend/src/router/index.jsx, frontend/src/index.css. (ROUTINE — L, sonnet) (@lane: frontend/home) (@after: ODY1)
- [x] ODY3 — **L'atterrissage : ouvrir l'ERP = voir ses apps.** L'atterrissage post-login N'EST PAS dans le router : `Login.jsx:142` navigue via `resolveLandingPath` de `pages/preferences/prefs.js` (repli `/dashboard` ligne ~71 → `/apps` ; assertions `prefs.test.jsx:43-49` mises à jour). Ajouter la garde manquante : `/` authentifié → `/apps` (`router/index.jsx:188` rend Login SANS garde aujourd'hui ; `/` anonyme → Login inchangé). EXCEPTIONS : préférence « module d'atterrissage » VX46 renseignée → droit dans l'app ; mono-app (1 seule app visible) → entrer directement dedans (antidote au piège Odoo « un clic en plus ») ; deep-link/F5 → URL demandée, jamais interceptée ; `?next=` (VX65) prioritaire. `/dashboard` RESTE une route valide (tuile « Tableau de bord », ODY23) — le repointage du logo appartient à ODY5, celui de l'onglet Accueil mobile à ODY6. ATTENTION e2e : le setup PARTAGÉ `frontend/e2e/auth.setup.js:13-17` ancre `/dashboard` + heading « Tableau de bord » — l'adapter DANS cette tâche sinon les 27 specs échouent au setup ; idem `login.spec.js:14-20`, `mobile.spec.js:257`, `health.spec.js:6`, `visual.spec.js:17`, `comptes-justes.spec.js:64,93`. **Done =** matrice défaut/préférence/mono-app/deep-link/F5/`?next=` testée, setup e2e + specs adaptés verts. Files: frontend/src/pages/Login.jsx, frontend/src/pages/preferences/prefs.js, frontend/src/pages/preferences/prefs.test.jsx, frontend/src/router/index.jsx, frontend/e2e/auth.setup.js, frontend/e2e/login.spec.js, frontend/e2e/mobile.spec.js, frontend/e2e/health.spec.js, frontend/e2e/visual.spec.js. (ROUTINE — L, sonnet) (@lane: frontend/home) (@after: ODY2) (@coord NTMOB6 — la variante d'accueil par rôle mobile s'ajoutera par-dessus)
- [x] ODY4 — **Le mode immersion : dans une app, la coquille est CELLE de l'app.** Le cœur du paradigme. Créer `frontend/src/lib/apps/ActiveAppContext.jsx` dérivant l'« app active » de la route (namespace des moduleRoutes/routes.meta) ; quand une app est active, `Sidebar.jsx` ne rend PLUS la pile globale (~106 destinations) mais : un en-tête d'app (icône + nom + accent VX8, cliquable → cockpit), les items de CETTE app uniquement (son module.config, sous-sections dépliées, ≤2 niveaux), et en pied le bouton « ⊞ Toutes les apps » (→ Menu d'accueil). Écrans hors app (Menu d'accueil, préférences) = coquille neutre minimale. Extraire les sections encore codées en dur de `Sidebar.jsx:150-208` (tête Dashboard/Ma file/Messages, DOCUMENTS/ged, INTELLIGENCE, ADMINISTRATION — vérifié : ARC48/ARC54 n'ont migré que les ROUTES, la Sidebar est restée intouchée) pour que chaque destination appartienne à une app. ATTENTION : `BottomTabBar.jsx:21` importe `NAV_SECTIONS`/`LEGACY_NAV_KEYS` depuis Sidebar — le casser ici, le réparer ici. LE critère : **aucune entrée d'une autre app visible en immersion**. Tests qui cassent RÉELLEMENT (vérifiés) et à adapter dans cette tâche : `Sidebar.test.jsx:44-82`, `Sidebar.odx6.test.jsx:30-54`, `Sidebar.WIR23.test.jsx:40-61`, `BottomTabBar.test.jsx:56-71`, `Header.test.jsx:62-68` (`menu.layout.test.mjs` et `Layout*.test` ne sont PAS affectés — CSS pur / Sidebar mockée). **Done =** garde automatisée « en immersion CRM, aucun lien Ventes/RH/… dans le DOM de la sidebar », navigation intra-app intacte, les 5 fichiers de tests adaptés verts. Files: frontend/src/lib/apps/ActiveAppContext.jsx (nouveau), frontend/src/components/layout/Sidebar.jsx, frontend/src/components/layout/Layout.jsx, frontend/src/components/layout/BottomTabBar.jsx, frontend/src/components/layout/Header.jsx, frontend/src/features/*/module.config.jsx (extractions restantes). (ARCH — XL, opus) (@lane: frontend/shell-ody) (@after: ODY1, ODY30)
- [x] ODY5 — **La topbar d'app : identité + sortie canonique (et un breadcrumb honnête).** `Header.jsx` en immersion : à gauche, pastille icône+nom de l'app active (liseré accent), cliquable = menu rapide des sections de l'app ; le logo repointé `/apps` (`Header.jsx:109` cible `/dashboard` en dur aujourd'hui) ; **le bouton ⊞ du header est LA sortie canonique** — celle que teste ODY31 et celle qui restaure le focus (ODY32) ; le lanceur VX9 reste un raccourci overlay (binding défini par ODY28 après résolution des collisions `g` — ne PAS présumer `g a` ici), jamais testé comme sortie. `Breadcrumbs.jsx` : en immersion le 1er segment est l'app (→ cockpit, VX11 ; pour CRM la cible reste `/crm/leads` — porte fondateur, test APX1) et le fil reste une HIÉRARCHIE lisible app › section › fiche — jamais la pile confuse d'Odoo (issues #46616/#41891 documentées). Le systray (recherche, cloches, chat, société, langue, thème, user) reste TRANSVERSE et identique dans toutes les apps. **Done =** l'utilisateur sait toujours dans quelle app il est (nom + accent), sortie en 1 clic au ⊞, logo → Menu d'accueil, Header.test/Breadcrumbs.test adaptés verts. Files: frontend/src/components/layout/Header.jsx, frontend/src/components/layout/Breadcrumbs.jsx, frontend/src/index.css. (ROUTINE — L, sonnet) (@lane: frontend/shell-ody) (@after: ODY4)
- [x] ODY6 — **Mobile : le même paradigme au pouce.** Le Menu d'accueil EST l'accueil mobile (grille 3-4 colonnes, safe-area, type-ahead remplacé par une barre de recherche tactile) ; en immersion `BottomTabBar.jsx` n'affiche que les onglets de l'app active + un onglet « Apps » (= sortie — remplace l'onglet Accueil `BottomTabBar.jsx:31` qui cible `/dashboard` en dur) ; le tiroir « Plus » (VX12) devient le tiroir DE l'app (ses items seulement). **Done =** parcours mobile complet home → app → sortie → autre app dans la gate Playwright mobile, hooks DOM préservés. Files: frontend/src/components/layout/BottomTabBar.jsx, frontend/src/pages/home/HomeMenu.jsx, frontend/src/index.css. (ROUTINE — L, sonnet) (@lane: frontend/shell-ody) (@after: ODY4, ODY2)
- [x] ODY7 — **Les liens croisés changent d'app PROPREMENT.** Audit + correction des navigations inter-apps (lead→devis, devis→facture/BC/chantier, ticket→équipement/client, produit→devis…) : suivre un lien croisé bascule l'app active (coquille + accent + breadcrumb reconstruit sur l'app cible — le comportement Odoo), JAMAIS un écran de Ventes rendu « dans » la coquille CRM. Si besoin un util `crossAppNavigate(to)` dans ActiveAppContext ; les routes elles-mêmes ne changent pas. **Done =** ≥6 parcours croisés testés (vitest router + 1 e2e), breadcrumb cohérent après bascule, zéro écran orphelin de coquille. Files: frontend/src/lib/apps/ActiveAppContext.jsx, frontend/src/components/layout/Breadcrumbs.jsx (+ points d'appel recensés par l'audit). (ROUTINE — M, sonnet) (@lane: frontend/shell-ody) (@after: ODY5)
- [x] ODY8 — **Route d'app non installée : une vraie porte, pas un renvoi muet.** Le redirect silencieux vers `/dashboard` existe à DEUX endroits (vérifié) : `router/index.jsx:140` ET `router/moduleRoutes.jsx:69` (ODX6). Les remplacer par un écran « App non activée » : icône+nom de l'app, « Cette app n'est pas activée pour votre société », CTA « Activer » (admin → Paramètres/Applications ODX5 ; non-admin → « demandez à votre administrateur »), lien Menu d'accueil. Cas distinct : app installée mais rôle insuffisant (message différent, AUCUNE donnée révélée). NE PAS régresser `index.vx78NotFound.test.mjs:24` et `index.vx131Forbidden.test.mjs:22-24` (404/403 ne redirigent plus vers /dashboard — acquis VX78/VX131). **Done =** URL d'un module OFF → écran dédié (tests router aux DEUX points), CTA visible admin-only, matrice module OFF × rôle, tests VX78/VX131 toujours verts. Files: frontend/src/router/moduleRoutes.jsx, frontend/src/router/index.jsx, frontend/src/pages/home/AppNotInstalled.jsx (nouveau). (ROUTINE — M, sonnet) (@lane: frontend/apps-core) (@after: ODY1)

**B — LE PLUS BEL ÉCRAN D'ACCUEIL (la signature) :**
- [x] ODY9 — **Iconographie d'apps signature : UN composant, QUATRE surfaces.** Créer `frontend/src/ui/AppIcon.jsx` : conteneur superellipse (échelle `--radius`), fond teinte accent module ~12 %, icône lucide du module (celle du `module.config`) à contraste AA, tailles 64/56/48px, hover (élévation + liseré accent plein), focus visible ; consommé par HomeMenu, AppLauncher, PinnedApps ET l'écran Applications (ODX5) — l'antidote « incohérence inter-apps ». **Done =** les 4 surfaces rendent la MÊME icône pour la même app (test), AA clair/sombre, captures. Files: frontend/src/ui/AppIcon.jsx (nouveau), frontend/src/pages/home/HomeMenu.jsx, frontend/src/components/layout/AppLauncher.jsx, frontend/src/components/layout/PinnedApps.jsx. (ROUTINE — M, sonnet) (@lane: frontend/home) (@after: ODY2)
- [x] ODY10 — **Badges vivants sur la grille (l'accueil respire).** Compteurs par app sur le Menu d'accueil : relances en retard (CRM), devis expirant ≤7 j (Ventes), factures impayées, tickets SLA (SAV), interventions du jour (Chantiers)… via l'endpoint fédéré ARC40 EXISTANT `GET /reporting/reports/kpi-federes/` (`apps/reporting/reports.py::kpi_federes` — providers déclarés en `apps/*/platform.py::kpi_providers`, collectés par `core/platform.py`, gating ModuleToggle inclus) ; l'étendre d'un format « badge » léger si nécessaire, JAMAIS re-agréger à la main. UN appel agrégé, cache court, jamais bloquant (grille d'abord, badges ensuite) ; module OFF = pas de badge. **Done =** badges exacts par société/rôle (test backend multi-tenant + vitest), 1 seul appel réseau, grille rendue avant les badges. Files: frontend/src/pages/home/HomeMenu.jsx, frontend/src/lib/api/, backend/django_core/apps/reporting/reports.py (+ tests). (ROUTINE — L, sonnet) (@lane: frontend/home) (@after: ODY9)
- [x] ODY11 — **La transition signature app ↔ accueil.** Entrer : l'icône cliquée s'étend/fond vers la coquille de l'app (~180 ms, easing du kit) ; sortir : fondu inverse vers la grille. View Transitions API quand dispo, repli fondu simple, INSTANTANÉ sous `prefers-reduced-motion`, zéro dépendance nouvelle. Odoo est perçu « lourd » à chaque re-navigation — ici la transition rend le paradigme délicieux au lieu de laborieux. **Done =** fluide (pas de jank visible en throttling CPU ×4), reduced-motion instantané, aucun impact sur le temps de navigation mesuré. Files: frontend/src/pages/home/HomeMenu.jsx, frontend/src/lib/apps/ActiveAppContext.jsx, frontend/src/index.css. (ROUTINE — M, sonnet) (@lane: frontend/home) (@after: ODY9)
- [x] ODY12 — **Vitesse perçue : préchargement au survol + zéro flash blanc.** Câbler la grille (et le launcher VX9) sur `frontend/src/router/prefetchMap.js` (existant) : hover/focus d'une icône précharge le chunk du module ; à l'entrée, squelette de coquille (jamais d'écran blanc). **Done =** Home→app sans flash (vérifié), prefetch observé au network, note de budget (entrée d'app perçue < 400 ms sur cache chaud) au DONE LOG. Files: frontend/src/router/prefetchMap.js, frontend/src/pages/home/HomeMenu.jsx, frontend/src/components/layout/AppLauncher.jsx. (ROUTINE — S, sonnet) (@lane: frontend/home) (@after: ODY11)
- [x] ODY13 — **La grille est à MOI : réordonner, épingler, reprendre.** Réordonner la grille par glisser avec `@dnd-kit/core` SEUL (DndContext + useDraggable/useDroppable + KeyboardSensor — `@dnd-kit/sortable` N'EST PAS installé et reste interdit, NE PAS FAIRE VX), favoris en tête (LA clé partagée VX9/VX10 — une seule), « Récents » (3) dessous ; persistance localStorage par utilisateur ; l'ordre se reflète dans le launcher overlay. **Done =** ordre persistant au reload, drag accessible clavier, launcher et grille toujours dans le même ordre (test). Files: frontend/src/pages/home/HomeMenu.jsx, frontend/src/components/layout/AppLauncher.jsx. (ROUTINE — M, sonnet) (@lane: frontend/home) (@after: ODY9)
- [x] ODY14 — **Le premier matin : accueil vide + onboarding.** Utilisateur sans aucune app visible → état vide illustré (variante EmptyState VX40) « Aucune app activée » avec CTA admin vers Applications ; la bannière onboarding (VX36) remonte sur le Menu d'accueil tant que la checklist n'est pas finie. **Done =** état vide et bannière rendus selon l'état réel, aucun flash pour les sociétés normales, tests de rendu. Files: frontend/src/pages/home/HomeMenu.jsx, frontend/src/components/OnboardingBanner.jsx. (ROUTINE — S, haiku) (@lane: frontend/home) (@after: ODY2) (@model: haiku)
...
- [x] ODY15 — **Passe CRM** (leads, clients, activités, territoires, carte — la PORTE du CRM est `/crm/leads`, décision fondateur 2026-08-01 posée par APX1 : le cockpit CRM n'est JAMAIS `items[0]` ni la cible du fil d'Ariane). **Done =** module.config complet (zéro route orpheline vs router/index.jsx), cockpit digne, icône/accent/description FR, nav ≤2 niveaux, tests nav de l'app verts. Files: frontend/src/features/crm/module.config.jsx, frontend/src/pages/crm/, frontend/src/pages/CartePage.jsx. (ROUTINE — M, sonnet) (@lane: frontend/app-crm) (@after: ODY4)
- [x] ODY16 — **Passe Ventes** (générateur, devis, BC, relances, présets) — la section Facturation reste DANS Ventes tant qu'ODX18 n'a pas livré `features/facturation/` ; préparer le sous-groupe nav pour qu'ODX18 n'ait qu'à le déplacer. **Done =** module.config complet (zéro route orpheline), cockpit digne, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/ventes/module.config.jsx, frontend/src/pages/ventes/. (ROUTINE — M, sonnet) (@lane: frontend/app-ventes) (@after: ODY4)
- [x] ODY17 — **Passe Stock + Magasin/Logistique** — même logique pour Achats (ODX20 pas livré : les sections achats restent dans Stock, sous-groupe prêt à déplacer). **Done =** module.config complet (zéro route orpheline), cockpit digne, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/stock/module.config.jsx, frontend/src/features/magasin/, frontend/src/pages/stock/. (ROUTINE — M, sonnet) (@lane: frontend/app-stock) (@after: ODY4)
- [x] ODY18 — **Passe Chantiers/Installations + Interventions** (gates, Ma journée, planning). **Done =** module.config complet (zéro route orpheline), cockpit digne, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/installations/module.config.jsx, frontend/src/pages/installations/, frontend/src/pages/interventions/. (ROUTINE — M, sonnet) (@lane: frontend/app-chantiers) (@after: ODY4)
- [x] ODY19 — **Passe SAV + Monitoring** (tickets, équipements, contrats maintenance ; flotte solaire, CO2, analytics — `features/monitoring/` N'EXISTE PAS : créer `features/monitoring/module.config.jsx` si Monitoring devient une tuile, sinon rattacher explicitement ses écrans `pages/monitoring/` à l'app hôte choisie et le NOTER). **Done =** module.config(s) complets (zéro route orpheline), cockpits dignes, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/sav/module.config.jsx, frontend/src/pages/sav/, frontend/src/pages/monitoring/. (ROUTINE — M, sonnet) (@lane: frontend/app-sav) (@after: ODY4)
- [x] ODY20 — **Passe Compta + AO** — les notes de frais sont une SECTION de Compta (déjà dans `features/compta/module.config.jsx:60-94`, pas de tuile « Frais » séparée sans décision fondateur) ; le portail client est une route PUBLIQUE à jeton (`/portail-contrats/:token`), PAS une tuile d'app — exclu. **Done =** module.configs complets (zéro route orpheline), cockpits dignes, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/compta/module.config.jsx, frontend/src/features/ao/module.config.jsx. (ROUTINE — L, sonnet) (@lane: frontend/app-compta) (@after: ODY4)
- [x] ODY21 — **Passe RH + Paie + Flotte + QHSE** (4 tuiles, 4 configs — vérifier que chaque écran récent NT/X* est bien rattaché à sa nav). **Done =** module.configs complets (zéro route orpheline), cockpits dignes, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/rh/module.config.jsx, frontend/src/features/paie/module.config.jsx, frontend/src/features/flotte/module.config.jsx, frontend/src/features/qhse/module.config.jsx. (ROUTINE — M, sonnet) (@lane: frontend/app-rh) (@after: ODY4)
- [x] ODY22 — **Passe GED + Contrats + Connaissance + Messages + Marketing + Projets + Litiges** — le chat vit dans `features/messaging/` SANS module.config : le CRÉER (`features/messaging/module.config.jsx`, app « Messages ») ; la section GED en dur de Sidebar est extraite par ODY4, cette passe la complète. **Done =** module.configs complets (zéro route orpheline), cockpits dignes, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/ged/module.config.jsx, frontend/src/features/contrats/module.config.jsx, frontend/src/features/kb/module.config.jsx, frontend/src/features/messaging/module.config.jsx (nouveau), frontend/src/features/marketing/module.config.jsx, frontend/src/features/gestion_projet/module.config.jsx, frontend/src/features/litiges/module.config.jsx. (ROUTINE — L, sonnet) (@lane: frontend/app-ged) (@after: ODY4)
- [x] ODY23 — **Passe Intelligence + Rapports + Tableau de bord + Paramètres/Administration** (IA/OCR/agent → app « Intelligence » ; Reporting/Rapports/Journal → app « Rapports » ; Dashboard → app « Tableau de bord » ; admin/roles/users/paramètres → app « Paramètres » — les sections INTELLIGENCE/ADMINISTRATION en dur sont extraites par ODY4, cette passe crée/complète leurs configs). **Done =** module.configs complets (zéro route orpheline), cockpits dignes, icône/accent/description FR, nav ≤2 niveaux, tests nav verts. Files: frontend/src/features/ia/module.config.jsx (nouveau si absent), frontend/src/features/reporting/module.config.jsx, frontend/src/features/parametres/module.config.jsx (nouveau si absent), frontend/src/pages/ia/, frontend/src/pages/admin/. (ROUTINE — L, sonnet) (@lane: frontend/app-admin) (@after: ODY4)

**D — L'APP STORE INTERNE (installer une app doit donner envie) :**
- [x] ODY24 — **« Applications » devient une belle boutique.** Élever l'écran ODX5 (`frontend/src/pages/parametres/ApplicationsSection.jsx`, livré « fonctionnel uniquement, aucun travail de design » — c'était le mandat) au langage du Menu d'accueil : cartes `AppIcon` + description FR du manifest + catégorie + dépendances lisibles (« Nécessite : CRM, Ventes ») + état (« Installée · le 12/07 par Meryem » / « Disponible ») ; Installer/Désinstaller avec aperçu de cascade AVANT confirmation (la fermeture de dépendances ODX3 la renvoie déjà) ; recherche + filtres par catégorie. Fonctionnalité byte-identique (mêmes endpoints, mêmes gardes admin). NE PAS confondre avec la marketplace d'extensions no-code NTEXT14 (`extensions.ExtensionInstall`) — écran et modèle distincts. **Done =** matrice de tests ODX5 toujours verte, cascade lisible avant tout toggle, captures clair/sombre. Files: frontend/src/pages/parametres/ApplicationsSection.jsx, frontend/src/ui/AppIcon.jsx. (ROUTINE — L, sonnet) (@lane: frontend/store) (@after: ODY9)
- [x] ODY25 — **Journal d'installation : qui a activé quoi, quand.** Émettre un événement domaine `module_toggled` (bus `core/events.py`) à chaque bascule ModuleToggle, l'historiser (motif chatter générique `apps/records` si adapté, sinon simple journal), et l'afficher dans la boutique (« Désactivée le 03/08 par Reda — raison : … » — le champ `raison` existe, `core/models.py:1122-1124`). Multi-tenant strict (société A invisible de B). **Done =** bascule loggée + affichée + événement testé, aucun log inter-sociétés. Files: backend/django_core/core/ (service de bascule + événement), frontend/src/pages/parametres/ApplicationsSection.jsx, tests. (ROUTINE — M, sonnet) (@lane: backend/store) (@after: ODY24)
- [x] ODY26 — **Rôles × apps : qui voit quoi, administrable à UN endroit.** Aujourd'hui « installée pour la société » (ModuleToggle) et « visible pour ce rôle » (roles/perm des items de nav) sont deux systèmes sans surface commune. Dans la matrice de rôles (VX38, RolesManagement), ajouter l'axe « App visible » par module — en réutilisant `Role.permissions` existant si suffisant (VÉRIFIER d'abord ; nouveau champ backend seulement si indispensable, additif). Un rôle privé d'une app ne la voit ni en grille, ni en nav, ni au launcher (déjà garanti si ODY1 est la seule source). **Done =** décision documentée (permissions existantes vs champ), matrice UI claire, test rôle-sans-app sur les 3 surfaces. Files: frontend/src/pages/admin/RolesManagement.jsx, frontend/src/lib/apps/useInstalledApps.js, backend/django_core/apps/roles/ (seulement si champ requis). (DECISION — M, opus) (@lane: frontend/store) (@after: ODY24, ODY1)

**E — LE TRANSVERSE : ce qui échappe à l'immersion (le systray, mieux qu'Odoo) :**
- [x] ODY27 — **Enforcement « installée » sur TOUTES les surfaces transverses.** La palette ⌘K (`providers/CommandPalette.jsx` : commandes de navigation, quick-create et résultats d'apps OFF masqués — le backend ARC29 gate déjà la recherche, le frontend liste encore des routes statiques), `GlobalSearch.jsx` (ne proposer que les types d'apps installées — pastilles VX13), `NotificationBell.jsx` (sections d'apps OFF masquées), Dashboard (cartes d'apps OFF). Tout passe par `useInstalledApps()`/un util `isAppVisible(key)` — zéro liste locale. Tout résultat/commande ouvrant un écran d'une AUTRE app passe par `crossAppNavigate()` (ODY7) — la coquille bascule proprement. **Done =** matrice de tests app OFF → absente des 4 surfaces, résultat cross-app = bascule de coquille (test), grep « liste d'apps locale » vide. Files: frontend/src/providers/CommandPalette.jsx, frontend/src/components/layout/GlobalSearch.jsx, frontend/src/components/layout/NotificationBell.jsx, frontend/src/pages/Dashboard.jsx. (ROUTINE — L, sonnet) (@lane: frontend/transverse) (@after: ODY1, ODY7) (@coord NTUX9 — ODY27 ne fait que le filtrage « app installée », NTUX9 possède les actions contextuelles de palette)
- [x] ODY28 — **Raccourcis `g`+lettre : keyboard-first entre les apps (et fin du double gestionnaire).** État vérifié : `providers/shortcuts.js:10-21` définit DÉJÀ 10 séquences (`g d`→dashboard, `g c`→/crm, `g v`→/ventes/devis, `g a`→/approbations…) ET `AppLauncher.jsx:139-152` installe un 2ᵉ listener `g a` privé — aujourd'hui les DEUX tirent (lanceur + navigation /approbations) : bug de collision réel. Supprimer le listener privé d'AppLauncher, tout unifier dans les `SEQUENCE_TABLES` de `ShortcutsProvider.jsx` ; bindings d'apps déclarés en module.config (champ optionnel), collisions résolues par ordre de registre ET affichées dans l'aide `?` ; trancher le sort des 10 bindings existants (repointage vers les cockpits d'app OU préfixe distinct) et le NOTER au DONE LOG ; `g g` = Menu d'accueil ; inactif dans les champs de saisie. **Done =** UN seul gestionnaire de séquences, navigation inter-apps sans souris, aide à jour, tests provider + shortcuts.test.mjs adaptés verts. Files: frontend/src/providers/ShortcutsProvider.jsx, frontend/src/providers/shortcuts.js, frontend/src/providers/shortcuts.test.mjs, frontend/src/components/layout/AppLauncher.jsx, frontend/src/features/*/module.config.jsx (champ shortcut optionnel). (ROUTINE — M, sonnet) (@lane: frontend/transverse) (@after: ODY4)
- [x] ODY29 — **L'état de chaque app survit à la sortie (Odoo le perd).** Mémoriser par app la dernière route visitée (sessionStorage, clé par app+user) ; revenir sur l'app depuis la grille propose « Reprendre » (route mémorisée) — préférence « toujours reprendre / toujours cockpit » dans Mes préférences (VX46). Les filtres/tri DataTable déjà persistés ne bougent pas. **Done =** sortir de CRM depuis une fiche puis re-entrer restaure le contexte selon la préférence, tests du store de reprise. Files: frontend/src/lib/apps/ActiveAppContext.jsx, frontend/src/pages/home/HomeMenu.jsx, frontend/src/pages/preferences/. (ROUTINE — M, sonnet) (@lane: frontend/transverse) (@after: ODY11)
- [x] ODY34 — **Changement de société → retour à MA grille.** `CompanySwitcher.jsx:30-34` fait `POST /auth/switch-company/` puis `window.location.reload()` — on reste sur l'URL courante alors que la société B a un AUTRE jeu de ModuleToggle : l'utilisateur multi-société atterrirait sur « App non activée » (ODY8) au lieu de SES apps. Après bascule, rediriger vers `/apps` (préférence d'atterrissage VX46 de la nouvelle société respectée si définie). **Done =** switch société → Menu d'accueil de la nouvelle société (test), plus jamais de reload vers une app interdite. Files: frontend/src/components/layout/CompanySwitcher.jsx. (ROUTINE — S, sonnet) (@lane: frontend/transverse) (@after: ODY2)

**F — QUALITÉ, BASCULE, RETRAIT DU LEGACY (« parfait » doit le rester) :**
- [x] ODY30 — **Le kill-switch AVANT la bascule (une bascule, jamais deux UX).** Poser le flag build `VITE_APPS_SHELL` (défaut ON) AVANT qu'ODY4 ne bascule la coquille — ODY4 le consomme. Le flag ne gate QUE la coquille (aucune divergence de features) ; le chemin OFF est un SMOKE d'urgence uniquement (les tests unitaires réécrits par ODY4 couvrent le mode Apps seul — assumé et documenté) ; sa mort est queued (ODY33). Un flag VITE se déclare en TROIS endroits (vérifié) : `.env.example` racine (~ligne 109), `docker-compose.yml` build args (~188-191), `frontend/Dockerfile.prod` ARG+ENV (~15-20) — flag build-time → rebuild d'image requis, le documenter. **Done =** flag lu par Layout.jsx, ON par défaut partout, OFF documenté comme secours (smoke des deux modes), triplet env complet. Files: frontend/src/components/layout/Layout.jsx, .env.example, docker-compose.yml, frontend/Dockerfile.prod. (ROUTINE — S, sonnet) (@lane: frontend/shell-ody) (@after: ODY1)
- [x] ODY31 — **La gate e2e du paradigme.** Nouvelle spec `frontend/e2e/apps-shell.spec.js` : login → Menu d'accueil (grille = apps attendues du seed) → type-ahead → entrer CRM → assert immersion (AUCUN lien d'une autre app dans la nav, identité app dans la topbar) → lien croisé lead→devis bascule proprement sur Ventes → sortie ⊞ (LA sortie canonique, ODY5) → grille → route d'app OFF → écran ODY8 → viewport mobile (BottomTabBar scopée + onglet Apps). `.header-title` et hooks existants intacts. **Done =** spec verte sur les gates CI existantes (Chromium + mobile ; WebKit si la matrice VX68 est active), stable sur 2 runs consécutifs. Files: frontend/e2e/apps-shell.spec.js (nouveau). (ROUTINE — L, sonnet) (@lane: e2e) (@after: ODY6, ODY7, ODY8)
- [x] ODY32 — **A11y + budget perf du paradigme.** Axe sans violation sérieuse sur HomeMenu + coquille immersion (clair/sombre) ; gestion du focus (entrer une app → focus contenu ; sortir par le ⊞ → focus revient sur l'icône d'origine) ; `aria-live` discret annonçant la bascule d'app ; budgets mesurés : Home LCP < 1,5 s (cache froid local), entrée d'app sans long task > 200 ms. **Done =** axe vert, focus round-trip testé, mesures notées au DONE LOG. Files: frontend/e2e/ (axe), frontend/src/pages/home/HomeMenu.jsx, frontend/src/lib/apps/ActiveAppContext.jsx. (ROUTINE — M, sonnet) (@lane: e2e) (@after: ODY31)
...
### Groupe APX — L'INTÉRIEUR des apps : chaque app principale devient LA plus belle de sa catégorie ; le CRM s'ouvre sur les Leads ; densité Odoo MESURÉE et gardée en CI (fondateur 2026-08-01)

*Provenance : directive fondateur 2026-08-01 (« make the principal apps — stock, leads, devis… —
look really nice and like the best ; every one should have its own stuff ; the Lead part is the
opening of the CRM ; lead view really nice like in Odoo — we see a lot of leads at once, phone
and computer »). Complément INTÉRIEUR du Groupe ODY : ODY fait la coquille et l'entrée, APX fait
le dedans. Conception : workflow 7 éclaireurs (audits fichier:ligne, anti-doublon 8 plans,
best-in-class web) + DEUX critiques adversariales Opus croisées (ancres/machinerie : ~120 refs
échantillonnées ; intention fondateur : chiffres de densité re-dérivés, budget de signaux,
protections des acquis) + synthèse Fable. Mesures d'état vérifiées : carte kanban lead 120-150 px
au repos vs ~60-70 px Odoo (3-4 cartes visibles/colonne en 1366×768 — delta ×2,3) ; chrome
vertical ≈286 px ; `.kb-col` fixe 272 px → 6ᵉ colonne (COLD) hors champ presque partout ;
atterrissage CRM contradictoire (launcher/épinglés/atterrissage → Calendrier via `items[0]`,
breadcrumb → Clients — jamais Leads). Le commit d'insertion ajoute `APX` à `unmapped_ok` de
`docs/BUILD_ORDER.yml`, régénère CODEMAP §10 + fingerprints, et pose les clauses « porte CRM =
/crm/leads » dans ODY1/ODY5/ODY15.*

**LA SIGNATURE PAR APP.** CRM = le board dense où l'on voit SON pipeline d'un regard. Ventes =
l'app de l'argent (montants héros, chaîne devis→BC→facture visible, aperçu PDF sans quitter
l'écran). Stock = l'app du réel (jauges de niveau, courbes de pompe tracées, scan qui répond,
photos — autorisées « photos ok » 2026-08-01). Chantiers = l'app du temps et de la preuve (grille horaire par technicien,
avant/après appariés). SAV = l'inbox (SLA en échéance qui reste rouge). Compta = l'app de
confiance (chiffre héros, aging coloré, plus un seul titre cassé en sombre). RH/Paie/GED/Projets/
AO/Flotte/QHSE restent volontairement au niveau ODY15-23 (identité+cockpit) — ce n'est pas un
oubli, c'est le périmètre.

**Contraintes (chaque tâche APX).** Celles de VX et ODY s'appliquent intégralement (zéro dep npm ;
règles #2/#4 ; hooks e2e déplacés AVEC leurs specs dans la même tâche ; `noValidate`/`step="any"` ;
`prix_achat` jamais client-facing ; UN Toaster ; AA clair+sombre ; reduced-motion). Propres à APX :
**tactile ≥44 px sur tout élément interactif, même dense** ; **la densité par vue EXISTE DÉJÀ
(NTUX17 livrée, `new_tasks_plan.md:853` — `densityOverride` `ui/datatable/DataTable.jsx:378-386`)
et la préférence globale pilote DÉJÀ les écrans DataTable via `useDensity()` (32/40 px,
`DataTable.jsx:382-394`) : APX ne RE-construit rien de tout ça, il branche les SURFACES MAISON qui
l'ignorent** ; **ARC49/53 sont LIVRÉES** (DevisList/FactureList déjà sur DataTable — toute retouche
de ligne passe par l'API colonnes/cellules, jamais du CSS `<td>` artisanal) ; **virtualisation
react-window toujours rejetée** (pas de dep) ; les acquis LB/LW sont des INVARIANTS nommés
(mono-scroll LB41, snap mobile LB42, chrome-52 LB53, clavier LB12, 44 px/swipe-inerte/PII LB17,
vues de compte LB48-56) — on densifie, on ne casse pas ; la chaîne de hauteur
`.layout-content > .route-fade:has(> .lp-page)` (`index.css:7117`) est intouchable. **Propriété :**
les vagues A/B écrivent dans crm/ventes — la session domaine PLAN_CRM_VENTES reste idle pendant
leur drain (même règle que QX/VX). **Coordination ODY par vague :** A↔ODY15, B↔ODY16, C↔ODY17,
D↔ODY18, E↔ODY19, F↔ODY20/ODY22 — collisions de fichiers, pas de décision : le second arrivé
rebase.

**A — CRM : LEADS D'ABORD + DENSITÉ ODOO (la demande fondateur au mot près) :**
- [x] APX1 — **Entrer dans CRM = arriver sur les Leads — et VERROUILLÉ par test.** État vérifié : `features/crm/module.config.jsx` liste Calendrier(1)→Clients(2)→Leads(3) et route `/crm`→ClientList ; launcher (`AppLauncher.jsx:86-90`), épinglés (`PinnedApps.jsx:37-44`) et atterrissage (`prefs.js:68-79`) dérivent TOUS le cockpit de `items[0].to` → Calendrier ; le fil d'Ariane (`routes.meta.js:168`) pointe en dur `/crm` → Clients. Fix : Leads (`/crm/leads`) remonté en `items[0]` (corrige les 3 surfaces d'un coup), `routes.meta.js:168` → `to: '/crm/leads'`, les 2 liens morts `/crm/clients/${id}` (`IdentityRail.jsx:194`, `PlanComptePage.jsx:89` — vrais 404 vérifiés) → `/crm?id=${id}` (deep-link réel lu par `ClientList.jsx:71-75`), et l'icône d'app CRM FORCÉE sur l'icône du module (sinon `items[0].icon` devient l'icône Leads dans launcher/épinglés tant qu'ODY9 n'est pas là). LE VERROU : un test `crm-porte.test` asserte que les 4 surfaces résolvent `/crm/leads` — ODY1/ODY5/ODY15 portent la clause depuis le commit d'insertion. Aucun test existant n'ancre le CRM sur Calendrier (vérifié). **Done =** 4 surfaces → `/crm/leads` (test de verrou), liens morts réparés, icône stable. Files: frontend/src/features/crm/module.config.jsx, frontend/src/components/layout/routes.meta.js, frontend/src/features/crm/workspace/IdentityRail.jsx, frontend/src/pages/crm/comptes/PlanComptePage.jsx. (ROUTINE — S, sonnet) (@lane: frontend/crm-porte) (@coord ODY15)
- [x] APX2 — **La carte kanban au REPOS : 3 lignes ≤76 px, BUDGET DE SIGNAUX fermé (rien ne disparaît, tout se condense).** État vérifié : `.kb-card` ≈120-150 px (`index.css:4937-5107,7701-7833`) avec 5 zones (tête score+menu / valeur / ligne d'action / tags / pied readiness+âge+avatar+quick). Nouveau contrat de REPOS (remplace EXPLICITEMENT le contrat LB13 « 4 zones ») : L1 nom+société tronqués ; L2 montant `.num` + BUDGET DE SIGNAUX : pastille rotting/activité (rampe LB14) + **signal d'action réduit à une icône colorée JAMAIS supprimée** (relance en retard/devis expiré/SLA restent distinguables SANS survol — test clair+sombre) + **score (VX221) en micro-badge** ; L3 tags en points (3+n) + âge + avatar 16. Révélation du reste (menu •••, quick, readiness, texte d'action) pilotée par **`@media (hover:hover)` UNIQUEMENT** (jamais par largeur — l'iPad `hover:none` hérite de l'anatomie tactile APX7) ; `:focus-within` DÉPLIE réellement le conteneur (test clavier : chaque contrôle révélé a une bounding box non nulle `toBeInViewport` — l'anti-pattern « tabbable invisible » que LB17 a corrigé) ; les éléments révélés n'apparaissent JAMAIS sous la position de repos du curseur (test « clic n'importe où hors zone d'action → la fiche s'ouvre », `LeadCard.jsx:334-337`). Drag/clavier LB12, PII LB17, undo : byte-identiques. Sondes épinglées adaptées DANS la tâche : LeadScore.vx221, précédence VX24, LeadCardFirstTouchTimer, ReadinessChips. **Done =** carte au repos ≤76 px mesurée, budget de signaux rendu (capture annotée), les 4 sondes + specs LB vertes. Files: frontend/src/pages/crm/leads/views/LeadCard.jsx, frontend/src/index.css. (ARCH — L, opus) (@lane: frontend/crm-densite)
- [x] APX3 — **Reprendre le chrome vertical du board : 286 → ≤240 px (le 52 px de LB53 mesure 61 au rig — on prend ce qui est prenable).** Postes vérifiés : `.layout-content` padding 2rem 4 CÔTÉS (`index.css:365` — ne toucher que le TOP), `.lp-view-area` marges 24 (:4190), `.kb-board` pt 12 (:4745), `.kb-col-header` ≈48 (:4802), bas ≈66. MÉTHODE imposée : scoper via l'idiome EXISTANT `.layout-content > .route-fade:has(> .lp-page[data-view])` (le discriminant `[data-view]` n'existe QUE sur LeadsPage — vérifié : `.lp-page` est AUSSI sur Installations/Planification/Interventions qui appartiennent à d'autres lanes, ne pas les toucher ici) ; la chaîne `:has()` de hauteur (`index.css:7117`, invariant LB33) est nommée intouchable — re-passer LB33 dans le même commit. Cibles : pt layout 16, marges view-area 8, en-tête colonne 38, bas 46. **Done =** chrome total ≤240 px mesuré par la gate APX8, LB33 verte, zéro pixel bougé sur les 3 autres pages `.lp-page` (goldens rapides ou assertions de style). Files: frontend/src/index.css. (ROUTINE — M, sonnet) (@lane: frontend/crm-densite) (@after: APX2)
- [x] APX4 — **Les 6 étapes du funnel visibles : colonnes fluides 204 px + « plein écran board ».** État vérifié : `.kb-col{flex:0 0 272px}` + gap 12 (:4763-4764,4742) ; sidebar repliée = 64 px (`index.css:393`). Fix : `minmax(204px, 1fr)` quand la zone utile ≥ 6×204+5×12 (les 6 colonnes se PARTAGENT la largeur), bouton « ⛶ plein écran » dans la zone vue (replie la sidebar via l'état COLLAPSE_KEY existant de Layout) ; en dessous du seuil, comportement actuel (scroll + drag-to-pan LB11) inchangé. Arithmétique honnête : 1440 sidebar repliée → ~1288 utile → 204-210 px/colonne → 6/6 ✓ ; 1920 sidebar dépliée → 6/6 ✓ ; 1366 dépliée reste 4-5 (assumé, pan). **Done =** 6/6 colonnes sans scroll horizontal en 1440×900 sidebar repliée ET 1920×1080 dépliée (gate APX8), cartes lisibles à 204 px (capture), aucun scroll fantôme. Files: frontend/src/index.css, frontend/src/pages/crm/leads/LeadsPage.jsx. (ROUTINE — M, sonnet) (@lane: frontend/crm-densite) (@after: APX3)
- [x] APX5 — **La vue Liste assume « je vois TOUT » : UNE ligne par lead, 18+ visibles.** État vérifié : `.lv-table td` padding 8/12 codé en dur (:5279-5284), cellule lead jusqu'à 3 lignes non plafonnées (`ListView.jsx:252-301`) → 33-65 px/ligne. Fix : mode UNE LIGNE (nom · société inline, tel/WhatsApp en icônes à droite de la cellule, « Archivé » en pastille) piloté par le MÊME système de densité que le reste (`useDensity()`/`data-density` — brancher `var(--row-py)` sur `.lv-table td/th` UNIQUEMENT — `.data-table` appartient à APX34, contradiction de lanes interdite) ; infobulle ViewSwitcher « Liste — la vue la plus dense ». Ne PAS toucher `densityOverride` (NTUX17 livrée). **Done =** ≥18 lignes visibles en 1440×900 préférence compacte (gate APX8), une seule hauteur de ligne par mode (mesure), tri/sélection/hooks intacts. Files: frontend/src/pages/crm/leads/views/ListView.jsx, frontend/src/index.css, frontend/src/design/tokens.css. (ROUTINE — M, sonnet) (@lane: frontend/crm-densite) (@after: APX2)
- [x] APX6 — **En-têtes de colonnes : la progressbar segmentée d'activité (LA signature Odoo) + somme des montants.** VÉRIFIER D'ABORD le contenu réel des en-têtes LB9 (compteur ? somme ?) puis compléter : fine barre segmentée proportionnelle sous le nom d'étape (en retard=destructive / aujourd'hui=warning / planifié=success / sans activité=muted — mêmes données que la rampe LB14, zéro requête nouvelle), cliquable = filtre d'activité de la colonne ; somme `.num` des montants à droite. Clés d'étape importées de STAGES.py (règle #2). **Done =** barre exacte par colonne (test seed), clic-filtre OK, somme juste, AA. Files: frontend/src/pages/crm/leads/views/KanbanView.jsx, frontend/src/index.css. (ROUTINE — M, sonnet) (@lane: frontend/crm-densite) (@after: APX4)
- [x] APX7 — **Téléphone ET tablette : 5+ leads par étape, `hover:none` servi à TOUTE largeur.** État vérifié : `.kb-quick` permanent en `@media(hover:none)` (+36 px/carte, :7825-7833) → ≈3 cartes sur 390×844 ; l'iPad (projet Playwright `tablet`, WebKit `hover:none`) exige des actions atteignables SANS survol (VX68). Fix : anatomie tactile au repos = APX2 avec UNE action 44×44 (WhatsApp) posée sur la ligne montant + tel et le reste derrière ⋯ (bottom-sheet existant) → carte ~89 px ; s'applique à TOUT `hover:none` (téléphone ET tablette — jamais un breakpoint de largeur) ; snap LB42 et swipe-inerte LB17 inchangés. **Done =** ≥5 cartes pleines sur 390×844 ET comptage ajouté au projet tablet (gate APX8), toutes cibles ≥44 px, tel/WhatsApp ≤2 taps, gates mobile+tablet vertes. Files: frontend/src/pages/crm/leads/views/LeadCard.jsx, frontend/src/index.css. (ROUTINE — M, sonnet) (@lane: frontend/crm-densite) (@after: APX2)
- [x] APX8 — **La gate de densité : les chiffres du fondateur deviennent des specs, VIEWPORTS DÉCLARÉS.** Nouvelle spec `frontend/e2e/leads-density.spec.js` (+ extension `frontend/e2e/leads-board.spec.js` LB33 si plus simple) avec viewports EXPLICITES (le projet desktop CI est en 1280×720 — piège vérifié) : 1440×900 = référence (≥7 cartes/colonne au repos, ≥18 lignes liste compacte, 6/6 colonnes sidebar repliée, chrome ≤240 px) ; 1280×720 = plancher CI (≥5 cartes/colonne) ; 390×844 (≥5 cartes) ; projet tablet (≥4 cartes + toutes actions atteignables sans hover). Ajouter un comptage sur la LISTE DEVIS et le CATALOGUE STOCK (mêmes viewports, seuils doux — la densité promise vaut pour les apps nommées par le fondateur, pas que les leads). Test du test : +40 px de padding carte → rouge. **Done =** gate verte et stable (2 runs), rouge sur ré-épaississement. Files: frontend/e2e/leads-density.spec.js (nouveau), frontend/e2e/leads-board.spec.js. (ROUTINE — M, sonnet) (@lane: e2e-apx) (@after: APX3, APX4, APX5, APX7)
- [x] APX9 — **500+ leads : plafond de rendu par colonne + « Charger plus » (Odoo ne monte pas tout non plus).** État vérifié : `fetchLeads` → `fetchAllPages` (tout en mémoire) et KanbanView MONTE toutes les cartes (`col.leads.map`, aucun slice) — densifier ⇒ toucher le mur plus tôt. Fix : plafond de rendu 40 cartes/colonne + bouton « Charger plus (N restants) » consommant le tableau DÉJÀ en mémoire (zéro appel réseau, zéro dep — hors frontière virtualisation) ; compteurs d'en-tête = totaux RÉELS ; drag/LB41/42 inchangés ; idem ListView si elle monte tout (vérifier). **Done =** 500 leads seedés → ≤6×40 nœuds carte montés, board fluide, compteurs vrais, tests. Files: frontend/src/pages/crm/leads/views/KanbanView.jsx, frontend/src/pages/crm/leads/views/ListView.jsx. (ROUTINE — S, sonnet) (@lane: frontend/crm-densite) (@after: APX2)
- [x] APX10 — **Le reste du CRM rejoint le niveau des Leads.** État vérifié : ClientList (`ClientList.jsx:351-358`) = en-tête générique sans identité ; Forecast (`forecast/ForecastPage.jsx`) = Cards nues ; MesActivités (`MesActivitesPage.jsx:320,516`) = idem ; ModuleHero consommé par AUCUN écran crm. Fix : PageHeader/ModuleHero + accent CRM sur les 3, grammaire Forecast sur `StatusAccentCard`, ligne de contrôle ClientList alignée sur celle des leads (LB43) — DataTable intouché (déjà sur `useDensity`). **Done =** identité CRM sur les 3 écrans (captures clair/sombre), zéro `page-header` legacy dans pages/crm/, tests verts. Files: frontend/src/pages/crm/ClientList.jsx, frontend/src/pages/crm/forecast/ForecastPage.jsx, frontend/src/pages/activities/MesActivitesPage.jsx. (ROUTINE — M, sonnet) (@lane: frontend/crm-reste) (@after: APX1) (@coord ODY15)

**B — VENTES : l'app de l'ARGENT (brass enfin à l'écran, la chaîne documentaire visible) :**
- [x] APX11 — **L'identité Ventes atteint enfin ses écrans de flux : PageHeader + brass sur les 5.** État vérifié : hors ToitureDesign, `brass` n'apparaît que 2× dans les écrans de flux (DevisGenerator:1876,2393 — zéro dans DevisList/FactureList/RelancesPage/VentesKanban) ; les 5 utilisent le vieux `<div className="page-header"><h2>` (DevisList:2078, DevisGenerator:1682, FactureList:1169, RelancesPage:276, VentesKanban:155). Fix : `ui/PageHeader` avec icône+accent module et sous-titre utile (« 12 devis à relancer »), liseré accent sur les cartes d'action. **Done =** zéro `page-header` legacy dans pages/ventes/, l'œil dit « je suis dans Ventes » (captures), tests verts. Files: frontend/src/pages/ventes/DevisList.jsx, frontend/src/pages/ventes/DevisGenerator.jsx, frontend/src/pages/ventes/FactureList.jsx, frontend/src/pages/ventes/RelancesPage.jsx, frontend/src/pages/ventes/VentesKanban.jsx. (ROUTINE — M, sonnet) (@lane: frontend/ventes-identite) (@coord ODY16)
- [x] APX12 — **UN seul langage de KPI d'argent : tout passe par `<Stat>` + `.num`.** État vérifié : 3 implémentations de carte KPI (bandeau 5 statuts DevisList:2159-2171 en divs nus ; cockpit trésorerie FactureList:1484-1521 qui régresse VX129 avec `▲`/`▼` texte :1492 — seul site du dossier ; rail DevisGenerator:2921-2939 dont le total héros :2927 n'a NI `.num` NI tabular). Migrer les 3 sur `ui/Stat.jsx` ; le total du rail devient LE chiffre le plus soigné de l'app. **Done =** grep `▲|▼` vide dans pages/ventes/, 3 surfaces sur le même composant, alignement tabulaire (capture), tests verts. Files: frontend/src/pages/ventes/DevisList.jsx, frontend/src/pages/ventes/FactureList.jsx, frontend/src/pages/ventes/DevisGenerator.jsx. (ROUTINE — M, sonnet) (@lane: frontend/ventes-identite) (@after: APX11)
- [x] APX13 — **La chaîne documentaire visible PARTOUT : DocumentStageTrack sur factures et BC.** État vérifié : le stepper devis→BC→facture (`ui/DocumentStageTrack.jsx`, VX141) n'est câblé QUE sur DevisList:597-602 + DevisTab (workspace) — il disparaît aux 2 autres étapes du parcours. L'ajouter aux lignes FactureList et à la liste BC (VentesKanban), amont remonté par les FKs déjà sérialisées (VÉRIFIER les champs ; complément serializer LECTURE seulement si manquant, additif). **Done =** une facture montre son devis d'origine (clic = y aller), un BC pareil, tests. Files: frontend/src/pages/ventes/FactureList.jsx, frontend/src/pages/ventes/VentesKanban.jsx. (ROUTINE — M, sonnet) (@lane: frontend/ventes-chaine) (@after: APX11)
- [x] APX14 — **Aperçu PDF SANS quitter l'écran (la signature « outil premium »).** État vérifié : `handlePreview` (DevisList:1667-1680) ouvre un ONGLET, alors que `features/ventes/PdfCanvas.jsx` (rendu inline) a exactement 4 consommateurs ailleurs (LeadDevisPanel, SigneDialog, InstallationDetail, DocumentsArchive) — jamais l'écran devis. Fix : « Aperçu » → ResponsiveDialog/Sheet latéral avec PdfCanvas (même source `/proposal` — AUCUN chemin PDF nouveau, règle #4 ; factures = leur PDF legacy), Télécharger/Onglet en repli, multi-pages ; iOS Safari via le pattern onglet pré-ouvert VX48 en repli. **Done =** aperçu inline devis ET facture, iOS OK, tests + hooks intacts. Files: frontend/src/pages/ventes/DevisList.jsx, frontend/src/pages/ventes/FactureList.jsx. (ROUTINE — M, sonnet) (@lane: frontend/ventes-chaine) (@after: APX13) (@coord FE-ZSAL8 — son bouton PDF BC rejoint le même menu ; le second arrivé rebase)
- [x] APX15 — **Un VRAI board Ventes (et la fin du fichier qui ment).** État vérifié : `VentesKanban.jsx:44` exporte `BonCommandeList()` — Tabs+table, AUCUN kanban ; `FactureKanbanBoard.jsx:13-73` = grille statique (classes `fkb-*`, zéro parenté `kb-*`). Fix : (a) renommer honnêtement (BonCommandeList) sans casser l'URL ; (b) VRAIE vue board des devis par statut DOCUMENT (brouillon/envoyé/accepté/refusé/expiré — règle #4, pas le funnel STAGES) au langage `kb-*`/cartes montant-héros, SANS drag inter-statuts d'argent (accepter/refuser restent des actions explicites — un drag ne signe pas un devis) ; (c) FactureKanbanBoard aligné sur le même langage ; (d) fuite JSON `VentesKanban.jsx:385` → `frenchError` (pattern DevisList:172-182). **Done =** board devis lisible (capture), zéro action d'état par drag, URL/tests BC inchangés, grep JSON.stringify(err) vide dans pages/ventes/. Files: frontend/src/pages/ventes/VentesKanban.jsx, frontend/src/pages/ventes/FactureKanbanBoard.jsx, frontend/src/index.css. (ROUTINE — L, sonnet) (@lane: frontend/ventes-board) (@after: APX11)
- [x] APX16 — **Générateur : les DEUX options côte à côte + les modèles visibles dès la création.** État vérifié : scénario « Les deux (Sans + Avec) » (DevisGenerator:278) mais le rail n'affiche qu'UN total (:2923-2927) ; DevisPresetPanel seulement en édition (`{editDevis && …}` :2819-2821) ; `text-green-600` en dur (DevisPresetPanel:188). Fix : rail à DEUX totaux quand le scénario est double (écart en MAD et %), présets visibles à la création (replié sous l'en-tête), tokens réparés ; garde `noValidate`/`step="any"` intacte. QX43-52 sont LIVRÉES (vérifié 52/52) — ne pas défaire leur travail sur les 4 modes/renderers. **Done =** comparaison visible pendant la construction, modèle applicable sur devis VIERGE, rail VX16 sans régression. Files: frontend/src/pages/ventes/DevisGenerator.jsx, frontend/src/pages/ventes/DevisPresetPanel.jsx. (ROUTINE — M, sonnet) (@lane: frontend/ventes-gen) (@coord NTMFG18 — bouton faisabilité atelier sur le même fichier, additif, le second arrivé rebase)
- [x] APX17 — **Hygiène premium ventes : plus une seule popup OS, une seule hauteur de ligne (via l'API DataTable — ARC49/53 livrées).** État vérifié : 12 sites `window.confirm/alert` (DevisGenerator:623,827,1671 ; DevisList:958,1730 ; FactureList:921 ; RelancesPage:106,154,189 ; VentesKanban:83,98,108) ; la cellule Statut DevisList empile jusqu'à 6 blocs (:592-669) — et comme DevisList EST sur DataTable (ARC49), la virtualisation >100 lignes estime 40/32 px : le plafonnement RÉPARE aussi le décalage de scroll au-delà de ~100 devis ; RelancesPage encore sur `<table className="data-table">` nue (:375). Fix : AlertDialog/ConfirmDialog partout (pattern VX19), cellule Statut plafonnée à StatusPill+StageTrack (reste en Popover « détails ») VIA l'API cellule DataTable (jamais de CSS td artisanal), RelancesPage → `reporting/Table`. **Done =** grep confirm/alert nu vide dans pages/ventes/, hauteur de ligne DevisList stable mesurée + scroll >100 devis sans saut (test), RelancesPage triable/exportable, tests adaptés. Files: frontend/src/pages/ventes/DevisGenerator.jsx, frontend/src/pages/ventes/DevisList.jsx, frontend/src/pages/ventes/FactureList.jsx, frontend/src/pages/ventes/RelancesPage.jsx, frontend/src/pages/ventes/VentesKanban.jsx. (ROUTINE — L, sonnet) (@lane: frontend/ventes-hygiene) (@after: APX12)

**C — STOCK : l'app du RÉEL (on VOIT le matériel, pas des SKU) :**
- [x] APX18 — **Photo produit : champ + upload + vignettes (DÉGATÉE — mot fondateur « photos ok », 2026-08-01 ; les NE PAS FAIRE VX :1145 et VXD :3450 sont amendés dans le même commit).** `Produit.image` (ImageField → MinIO, nullable, SCHEMA additif — au passage corriger la DOUBLE déclaration de `description` vérifiée `apps/stock/models.py:542` vs `:567-568`, bug latent), serializer, upload ProduitForm (compression VX77), vignette 40 px CatalogueTable (repli = icône de catégorie teintée — le repli est CONSTRUIT d'office pour APX19/APX36), image en tête de ProduitDetail. Jamais dans un PDF, jamais près de `prix_achat` côté client. **Done =** upload+vignette+fiche (captures), migration additive révertable, seed intact, tests. Files: backend/django_core/apps/stock/models.py, backend/django_core/apps/stock/serializers.py, frontend/src/pages/stock/ProduitForm.jsx, frontend/src/pages/stock/CatalogueTable.jsx, frontend/src/pages/stock/ProduitDetail.jsx. (SCHEMA — L, sonnet) (@lane: stock-photo) (@coord ODY17)
- [x] APX19 — **Niveau de stock LISIBLE : jauge colorée + sévérité + hauteur de ligne stable.** État vérifié : chiffre nu gras-rouge si bas (`CatalogueTable.jsx:185-221`) ; rupture (0) et « sous seuil » = MÊME badge (:244-249) ; colonne Seuil empile jusqu'à 4 éléments (:222-257) → hauteur instable. Fix : mini-jauge `Progress` (déjà utilisée par « Santé catalogue » StockList:1170-1192) vert/orange/rouge, badges distincts RUPTURE (destructive) vs SOUS SEUIL (warning), hauteur de ligne FIXE (action « Réapprovisionner » au hover/menu ligne + barre bulk — pattern Shopify) ; vignette/icône de catégorie en 1ʳᵉ colonne (repli APX18, fonctionne gaté ou pas). **Done =** une seule hauteur de ligne (mesure + comptage APX8), sévérité lisible sans lire, réappro ≤2 clics, tests. Files: frontend/src/pages/stock/CatalogueTable.jsx, frontend/src/index.css. (ROUTINE — M, sonnet) (@lane: stock-catalogue)
- [x] APX20 — **Tenir la promesse de la fiche produit : marque/garantie éditables + onglet Fiche technique.** État vérifié : `components/ProduitQuickCreateModal.jsx:62-63` promet « compléter (catégorie, marque, garantie…) plus tard depuis Stock » — or ProduitForm n'expose NI `marque` (models :566) NI `garantie` TEXTE (:570-572 — distinct de `garantie_mois` :579, seuls les numériques sont exposés :668-686), champs pourtant consommés par les fiches produits des PDF devis. Fix : les 2 champs dans ProduitForm + onglet « Fiche technique » sur ProduitDetail (marque, garantie, description, pompe_kw/tension_v si pompe) à côté des 2 onglets achats (:154-213). **Done =** promesse tenue, fiche rendue, tests. Files: frontend/src/pages/stock/ProduitForm.jsx, frontend/src/pages/stock/ProduitDetail.jsx. (ROUTINE — S, sonnet) (@lane: stock-catalogue) (@after: APX19)
- [x] APX21 — **Les courbes de pompe TRACÉES (11 pompes OSP ont leurs données constructeur, personne ne les voit).** État vérifié : `Produit.courbe_pompe` ne sert qu'au dimensionnement (`solar.js:1129-1132`) et s'affiche en badge TEXTE (`features/stock/catalogue.js:38`) ; recharts ^2.15.3 + `ui/charts` déjà là. Fix : graphe débit(x)/HMT(y) du kit dans l'onglet Fiche technique (APX20), grille discrète, clair/sombre ; lecture seule. **Done =** pompe à courbe → graphe (capture), pompe sans courbe → rien (pas de carte vide), test de rendu. Files: frontend/src/pages/stock/ProduitDetail.jsx. (ROUTINE — S, sonnet) (@lane: stock-catalogue) (@after: APX20)
- [x] APX22 — **Cockpits d'inventaire façon Odoo Inventory : des tuiles d'opérations + UNE identité famille.** État vérifié : MagasinCockpit/LogistiqueCockpit = 3-4 chiffres nus très en retrait de PilotageStock ; accents fragmentés (stock=`lune` partagé ×10, magasin/logistique=`success` partagé ×6 — configs :61/:33/:31) ; conteneurs divergents (MagasinCockpit:53 `page` vs StockList:923 `ui-root`). Fix : tuiles « N à traiter » (Réceptions/Livraisons/Transferts/Comptages) cliquables vers les listes FILTRÉES existantes, ModuleHero en tête, UN accent famille inventaire (rampes OKLCH existantes) posé sur les 3 module.config, conteneurs alignés. **Done =** 2 cockpits = files d'action (captures), accent commun sur les 3 configs, tests. Files: frontend/src/features/magasin/MagasinCockpit.jsx, frontend/src/features/logistique/LogistiqueCockpit.jsx, frontend/src/features/stock/module.config.jsx, frontend/src/features/magasin/module.config.jsx, frontend/src/features/logistique/module.config.jsx. (ROUTINE — M, sonnet) (@lane: stock-cockpits) (@coord ODY17 — l'accent vit dans module.config, le second arrivé rebase)
- [x] APX23 — **Le scan qui RÉPOND : feedback fort, SANS bip de succès (« pas de son » reste la règle).** État vérifié : `features/magasin/scan/scanFeedback.js` = `playRejectBeep` seul (appelé :67/:73) ; le scan accepté n'a qu'un bloc texte (`features/magasin/scan/ReceptionScanPanel.jsx:150-161`) — pas de retour visuel FORT ; `PickListScreen.jsx:73-75` (features/magasin/) affiche « X/Y (Z%) » sans `Progress`. Fix : flash plein écran vert/rouge 150 ms (overlay CSS, reduced-motion = bordure statique) + `navigator.vibrate?.(10)` défensif (le canal VX42) + `Progress` sur la pick-list ; le bip de rejet existant reste, AUCUN son nouveau (NE PAS FAIRE VX « pas de son »). AUCUN nouvel écran (modes scanner = NTWMS5/7/8). **Done =** accepté ≠ rejeté à l'œil et au poignet, Progress rendue, tests de la logique. Files: frontend/src/features/magasin/scan/scanFeedback.js, frontend/src/features/magasin/scan/ReceptionScanPanel.jsx, frontend/src/features/magasin/PickListScreen.jsx. (ROUTINE — S, haiku) (@lane: stock-scan) (@coord NTWMS)
- [x] APX24 — **21 pages Stock + cockpit magasin, UN seul idiome d'en-tête (15 fichiers à balayer).** État vérifié : 12 pages en `<h1>` nu (BonsCommandeFournisseur:1006, CategoriesStock:178, ConditionnementsProduit:182, FacturesFournisseur:457, FournisseurFiche360:880, FournisseursStock:503, InventairesAnnuels:122, LotsEntrepot:233, ModelesBcf:284, ReceptionsFournisseur:444, RetoursFournisseur:132, RevalorisationsStock:156), 3 en `<h2>` nu (StockList:935, MouvementsPage:295, OcrStockImport:547) + MagasinCockpit:54 (features/magasin) ; 7 déjà sur PageHeader. Balayage : tout sur `ui/PageHeader` avec icône/accent famille (APX22). **Done =** grep en-têtes nus vide sur pages/stock/ + MagasinCockpit, cohérence 21 pages, tests verts. Files: frontend/src/pages/stock/BonsCommandeFournisseur.jsx, frontend/src/pages/stock/CategoriesStock.jsx, frontend/src/pages/stock/ConditionnementsProduit.jsx, frontend/src/pages/stock/FacturesFournisseur.jsx, frontend/src/pages/stock/FournisseurFiche360.jsx, frontend/src/pages/stock/FournisseursStock.jsx, frontend/src/pages/stock/InventairesAnnuels.jsx, frontend/src/pages/stock/LotsEntrepot.jsx, frontend/src/pages/stock/ModelesBcf.jsx, frontend/src/pages/stock/ReceptionsFournisseur.jsx, frontend/src/pages/stock/RetoursFournisseur.jsx, frontend/src/pages/stock/RevalorisationsStock.jsx, frontend/src/pages/stock/StockList.jsx, frontend/src/pages/stock/MouvementsPage.jsx, frontend/src/pages/stock/OcrStockImport.jsx, frontend/src/features/magasin/MagasinCockpit.jsx. (ROUTINE — M, haiku) (@lane: stock-sweep) (@after: APX22)

**D — CHANTIERS/INTERVENTIONS : l'app du TEMPS et de la PREUVE :**
- [x] APX25 — **La fiche chantier respire : onglets au lieu du mur de 13 sections.** État vérifié : `InstallationDetail.jsx` (1 678 lignes) = un Sheet géant :753-1527, 13 sections empilées :811-1480, 4 bannières d'état au même patron :763-798 + une 5ᵉ générique `actionError` :801-808. Fix : 5-6 onglets (Aperçu / Gates & jalons / Matériel & BC / Photos / Facturation / Interventions), CollapsibleSection conservées dedans, UNE bannière de synthèse hiérarchisée (la plus grave visible, compteur « 3 alertes » en popover). **Done =** matrice section→onglet committée (aucun contenu perdu), scroll par onglet ÷3, tests adaptés. Files: frontend/src/pages/installations/InstallationDetail.jsx. (ROUTINE — L, sonnet) (@lane: chantiers-fiche) (@coord NTNRG4 — son onglet « Actifs » devient un onglet de CETTE structure ; le second arrivé rebase) (@coord ODY18)
- [x] APX26 — **UNE timeline de chantier (fusion des deux) + « Prochaine action » partagé.** État vérifié : deux timelines redondantes empilées (`ChantierTimeline` points flottants, rendu :847 ; `ChantierGateTimeline` stepper :857), gate timeline SANS progression (:182-186) alors que ChantierChecklist a une `Progress` (:121) ; bandeau « Prochaine action » dupliqué (ChantierGateTimeline:189-212, MaJourneePage:364-373 — duplication auto-documentée dans le code). Fix : UN stepper fusionné (jalons+gates, `Progress` « 3/7 » en tête), extraire `ui/NextActionBanner.jsx` adopté aux 2 sites. **Done =** une seule timeline dans la fiche, composant partagé aux 2 endroits, tests. Files: frontend/src/pages/installations/ChantierTimeline.jsx, frontend/src/pages/installations/ChantierGateTimeline.jsx, frontend/src/ui/NextActionBanner.jsx (nouveau), frontend/src/pages/interventions/MaJourneePage.jsx. (ROUTINE — M, sonnet) (@lane: chantiers-fiche) (@after: APX25)
- [x] APX27 — **Avant/Après appariés : la preuve qui vend.** État vérifié : 3 colonnes indépendantes, vignettes 64-96 px, AUCUNE comparaison (`ChantierPhotos.jsx:36-40,193-270`) ; photos chantier↔intervention reliées par un lien texte discret (:161-169). Fix : vue « Comparer » (paire avant/après par phase, séparateur glissant CSS-only) + galerie UNIFIÉE en lecture (chantier + interventions liées, badge d'origine, AUCUN nouveau modèle). **Done =** comparaison rendue (capture), galerie lit les deux sources, reduced-motion, tests. Files: frontend/src/pages/installations/ChantierPhotos.jsx. (ROUTINE — M, sonnet) (@lane: chantiers-photos)
- [x] APX28 — **Le planning devient un VRAI planning : grille horaire par technicien.** État vérifié : le « Calendrier techniciens » (`PlanificationPage.jsx:227-331`) = kanban colonnes-techniciens SANS axe temps ; conflits FG300 en liste texte à part (:479-504) ; Gantt minimal (:104-146). Fix : vue Jour (+ Semaine condensée) en grille 7h-19h × techniciens — VÉRIFIER d'abord les champs heure/durée réellement servis par l'API interventions (si la durée manque : blocs séquencés à hauteur fixe, JAMAIS de données inventées), chevauchements surlignés destructive, drag dnd-kit = réassignation technicien/créneau via le PATCH existant avec confirmation, « aujourd'hui » marqué, mobile = lecture seule propre. NB : cette page porte `.lp-page` SANS `data-view` — le scoping APX3 (`[data-view]`) la protège, ne pas y toucher ici. **Done =** poser/déplacer une intervention au bon créneau en drag (test), conflit VISIBLE dans la grille, zéro écriture serveur nouvelle. Files: frontend/src/pages/installations/PlanificationPage.jsx, frontend/src/index.css. (ARCH — XL, opus) (@lane: chantiers-planning)
- [x] APX29 — **« Ma tournée » sur la CARTE (le GPS dort).** État vérifié : `itineraire_url` est CALCULÉ par l'endpoint « Ma tournée » (`installations/views/intervention.py:1900` — pas un champ de modèle) et la tournée s'affiche en liste numérotée SANS carte (`PlanificationPage.jsx:349-386`, dupliqué `MaJourneePage.jsx:246-322`) ; `MapView` existe (lazy dans 2 autres pages). Fix : MapView avec arrêts numérotés dans l'ordre + tracé simple, bouton « Itinéraire » par arrêt (lien renvoyé par l'endpoint), liste à côté (desktop) / dessous (mobile) ; composant de tournée DÉDUPLIQUÉ entre les 2 pages ; les données viennent de l'endpoint tournée DÉJÀ appelé (vérifier PlanificationPage — sinon le seul ajout est cet appel existant, rien de nouveau côté serveur). **Done =** carte des arrêts aux 2 endroits (captures), zéro endpoint nouveau, tests. Files: frontend/src/pages/installations/PlanificationPage.jsx, frontend/src/pages/interventions/MaJourneePage.jsx. (ROUTINE — M, sonnet) (@lane: chantiers-tournee) (@after: APX28)

**E — SAV : l'INBOX (le SLA se voit, se compte, et reste rouge) :**
- [x] APX30 — **SLA par ticket : échéance + compte-à-rebours, rouge PERSISTANT — 100 % frontend (les champs existent déjà).** État vérifié : badge texte « ouvert depuis X j » seul (`TicketsPage.jsx:179-190,1628-1639`) ; les % SLA n'existent qu'en agrégat admin (`SavSlaReportPage.jsx:84-89`) ; MAIS le serializer expose DÉJÀ `sla_due_at`, `sla_due_at_effectif`, `sla_breach`, `jours_pause`, `en_attente_client`, `attente_depuis`, `date_premiere_reponse` (vérifié `apps/sav/serializers.py:359-415`) — AUCUN backend requis. Fix : chip d'échéance par ligne ET carte (« répondre avant 16h » / « −2 h » destructive), distinction 1ʳᵉ réponse vs résolution, dépassement qui RESTE marqué après résolution (« SLA dépassé » persistant — la traçabilité Odoo Helpdesk), tri par échéance ; au passage élargir le panneau détail `w-[26rem]` → `w-[32rem]` (:1166, vérifié hit unique). **Done =** échéance visible ligne+carte+détail, rouge persistant post-résolution (test), tri OK, multi-tenant testé. Files: frontend/src/pages/sav/TicketsPage.jsx. (ROUTINE — M, sonnet) (@lane: sav-sla) (@coord ODY19)
- [x] APX31 — **Checklist % partagée + j/k sur les tickets (l'agent qui en traite 40/jour).** État vérifié : `pages/sav/TicketChecklistPanel.jsx:119-121` affiche « X/Y points » en texte plat alors que `ChantierChecklist.jsx` a UNE `Progress` (:121 — la :130 est le texte %) ; `pages/sav/TicketWorksheetPanel.jsx:16-42` sort du kit (`form-label`/`form-control`/checkbox natif) contrairement à ses voisins ; aucune navigation clavier de liste côté SAV (le pattern j/k existe côté leads LW). Fix : extraire `ui/ChecklistProgress.jsx` (barre + « X/Y ») adopté par les DEUX panels, worksheet sur `Input`/`Checkbox`/`Label` du kit, et j/k + Entrée/Échap sur la liste tickets (mêmes gardes champ-de-saisie que LW). **Done =** composant partagé aux 2 endroits, worksheet cohérent, navigation clavier testée, aide `?` à jour. Files: frontend/src/ui/ChecklistProgress.jsx (nouveau), frontend/src/pages/sav/TicketChecklistPanel.jsx, frontend/src/pages/sav/TicketWorksheetPanel.jsx, frontend/src/pages/installations/ChantierChecklist.jsx, frontend/src/pages/sav/TicketsPage.jsx. (ROUTINE — M, sonnet) (@lane: sav-sla) (@after: APX30)

**F — COMPTA + LE KIT : l'app de CONFIANCE (et le bug d'en-tête qui casse 56 écrans) :**
- [x] APX32 — **LE bug transverse : l'en-tête `ListShell` est en hex — cassé en sombre sur 56 écrans — + les 5 poches d'hygiène de la même nature.** État vérifié : `ui/module/ListShell.jsx:2` importe `components/layout/PageHeader` (56 consommateurs) dont le CSS est en hex NU sans override dark (`.pageheader-title{color:#0f172a}` index.css:1062, `.pageheader-subtitle{color:#64748b}` :1068 — sur fond nuit oklch(15.7%) les titres sont quasi invisibles, vérifié) ; 5 pages compta empilent DEUX titres (Tresorerie:368/386, Rapprochements:521/536, Immobilisations:147/156, Fiscalite:338/356, Engagements:557 + :115-116/527-528). Fix : tokeniser `pageheader-*` (`var(--foreground)`/`var(--muted-foreground)`), prop `hideHeader` (ou titre optionnel) sur ListShell consommé par les 5 pages, échantillon de 6 écrans consommateurs vérifié clair+sombre. MÊME COMMIT, même nature : (a) `CartePage.jsx:83` hex fallback contredisant son commentaire :21-22 → token ; (b) Calendar/Carte : EmptyState+Spinner cohérents des deux côtés (CalendarPage:244,250 vs CartePage:141,145-150) ; (c) `BalanceAgeePage.jsx:74` style inline → classe ; (d) `features/ged/advanced/ApprobationPage.jsx:524-651` : exactement 9 styles inline px → Tailwind/tokens ; (e) `features/ged/GedNavigator.jsx:263` + `features/ged/NumeriserPage.jsx:127` : 4ᵉ idiome `<h1>` nu → `ui/PageHeader`. **Done =** plus aucun hex dans `pageheader-*`, dark lisible sur ListShell (captures), plus de double titre compta, les 5 poches grep-vides, tests verts. Files: frontend/src/index.css, frontend/src/ui/module/ListShell.jsx, frontend/src/features/compta/pages/TresoreriePage.jsx, frontend/src/features/compta/pages/RapprochementsPage.jsx, frontend/src/features/compta/pages/ImmobilisationsPage.jsx, frontend/src/features/compta/pages/FiscalitePage.jsx, frontend/src/features/compta/pages/EngagementsPage.jsx, frontend/src/pages/CartePage.jsx, frontend/src/pages/CalendarPage.jsx, frontend/src/pages/reporting/BalanceAgeePage.jsx, frontend/src/features/ged/advanced/ApprobationPage.jsx, frontend/src/features/ged/GedNavigator.jsx, frontend/src/features/ged/NumeriserPage.jsx. (ARCH — L, opus) (@lane: kit-header) (@coord ODY20, ODY22)
- [x] APX33 — **Plus une table nue en compta : les 6 (dont la 6ᵉ oubliée).** État vérifié : `<table>` à la main là où le comptable a le plus besoin de tri/export — `TresoreriePage.jsx:144` (Position consolidée), `:174` (Prévisionnel 13 semaines) ET `:272` (6ᵉ, non documentée avant), `EngagementsPage.jsx:464` (Provisions FNP/FAE), dialogs `RapprochementsPage.jsx:85` et `ImmobilisationsPage.jsx:67` — alors que `pages/reporting/Table.jsx` (15 consommateurs réels, pas « EtatsPage seul ») existe pour ça. Migrer les 6 (tri + CSV), chiffres `.num` à droite. **Done =** zéro `<table className=` nu dans features/compta/pages/ (grep), Position/Prévisionnel triables/exportables, tests. Files: frontend/src/features/compta/pages/TresoreriePage.jsx, frontend/src/features/compta/pages/EngagementsPage.jsx, frontend/src/features/compta/pages/RapprochementsPage.jsx, frontend/src/features/compta/pages/ImmobilisationsPage.jsx. (ROUTINE — M, sonnet) (@lane: compta-tables) (@after: APX32)
- [x] APX34 — **Le dernier trou de densité : les tables CSS legacy `.data-table` obéissent enfin à la préférence globale.** État RE-vérifié (deux fausses pistes tuées par la critique) : `useDensity()` pilote DÉJÀ les 56 écrans DataTable (32/40 px, `DataTable.jsx:382-394`), la densité par vue est LIVRÉE (NTUX17, `densityOverride` :378), Mes préférences a DÉJÀ les libellés (PreferencesPanel:82-83) — le SEUL trou réel : les tables CSS legacy `.data-table td/th` (`index.css:1212,1227`, ~66 sites d'usage) et rien d'autre. Fix : `padding-block: var(--row-py)` dérivé de `[data-density]` (tokens.css:427,436 — les tokens morts prennent vie), mobile toujours confort (44 px). NE PAS toucher `densityOverride` ni DataTable. **Done =** basculer la préférence change les tables legacy (test), mobile inchangé, zéro nouveau système de densité. Files: frontend/src/index.css, frontend/src/design/tokens.css. (ROUTINE — S, sonnet) (@lane: kit-header) (@after: APX32)
- [x] APX35 — **Le cockpit finance parle Stripe : UN chiffre héros + aging en buckets colorés cliquables.** État vérifié : `CockpitPage.jsx` (post-VX115) propre mais plat — aucune hiérarchie « un chiffre en grand » ; la balance âgée n'a AUCUN paramètre d'URL (bucket en `useState('all')` local, vérifié) — l'ajouter fait partie de la tâche (lecture d'un `?bucket=` via useSearchParams, additif). Fix : chiffre héros (trésorerie dispo ou encaissé du mois — champ DÉJÀ servi au cockpit, vérifier lequel) en `text-display .num`, KPI secondaires en `<Stat>` avec tendance, rangée de buckets aging (0-30/31-60/61-90/90+) colorés warning→destructive, chaque bucket → `/reporting/balance-agee?bucket=…`. AUCUN endpoint nouveau si les données cockpit suffisent — sinon complément LECTURE au selector (additif). **Done =** hiérarchie héros nette (capture), buckets exacts vs balance âgée (test), navigation filtrée OK. Files: frontend/src/features/compta/pages/CockpitPage.jsx, frontend/src/pages/reporting/BalanceAgeePage.jsx. (ROUTINE — M, sonnet) (@lane: compta-cockpit) (@after: APX33)
- [x] APX36 — **Trois goldens qui prouvent « le plus beau » (pas trente — la taxe de baselines est une décision livrée).** Étendre la régression visuelle existante (VX70/LB34) de TROIS écrans seulement, clair+sombre : board leads compact (APX2), catalogue Stock jauges+icônes (APX19), cockpit finance héros (APX35) — la densité, elle, est prouvée par les ASSERTIONS MESURÉES d'APX8 (robustes), pas par des pixels. Masques sur zones variables, données seedées fixes. **Done =** 3 goldens ×2 thèmes verts et stables (2 runs), un changement non intentionnel les fait rougir. Files: frontend/e2e/visual.spec.js. (ROUTINE — S, sonnet) (@lane: e2e-apx) (@after: APX8, APX19, APX35)

**NE PAS FAIRE (Groupe APX) — frontières vérifiées contre l'état réel :**
- RE-construire ce qui est LIVRÉ : densité par vue (NTUX17, `densityOverride`), densité globale des
  écrans DataTable (`useDensity()` 32/40 px), migration DataTable DevisList/FactureList (ARC49/53),
  libellés de densité des Préférences. Virtualisation react-window → toujours rejetée (dep).
- Plan/heat-map d'entrepôt → NTWMS1 ; nouveaux modes scanner → NTWMS5/7/8 (APX23 = feedback seul).
  Inbox SAV multi-canal → NTSRV4 ; « Ma file » → NTSRV9 ; onglet RMA GarantiesPage → NTFSM16
  (fichier non touché). Tracking « vu le » des devis → signaux QW/QX, jamais un tracker ad-hoc.
- Bip de succès au scan ou tout SON nouveau (NE PAS FAIRE VX « pas de son » — flash+vibration
  suffisent). Étoiles de priorité tickets → non. Drag qui change un statut d'ARGENT → jamais
  (APX15 l'interdit). Refactors invisibles coupés au verdict : découpage du monolithe TicketsPage
  et fusion MonthGrid des 2 calendriers (drag HTML5 vs dnd-kit à réconcilier — une vague d'hygiène
  dédiée, pas celle-ci).
- Les acquis LB/LW/VX sont des invariants nommés dans les Contraintes — toute tâche APX qui les
  touche adapte leurs specs DANS la même tâche.
...
- [x] EZ1 — **Le popover d'appel devient LE planificateur (fin des 3 surfaces rivales et de l'écrasement silencieux).** État vérifié : `features/crm/CallLogPopover.jsx:22-27,75` n'écrit qu'un `relance_date` via 4 offsets figés (J+0/1/3/7) et ÉCRASE en silence une relance existante (jamais lue ni affichée) ; 3 mécanismes rivaux (chips popover / `LeadsPage.jsx:562-566` qui ouvre juste la fiche / `pages/crm/leads/PlanActiviteDialog.jsx` qui applique un GABARIT de plan prédéfini, sans date libre). Fix : ajouter au popover une DATE LIBRE (DatePicker — `relance_date` et `records.Activity.due_date` sont des DateField : PAS d'heure ici, une éventuelle heure = future tâche SCHEMA distincte) + un objet optionnel (si objet → crée une vraie activité datée via `POST /records/activities/` existant, cible `crm.lead` déjà déclarée `crm/platform.py:33`, client prêt `recordsApi.createActivity` `frontend/src/api/recordsApi.js:12` — zéro backend) ; si une relance existe, l'AFFICHER avec choix « Remplacer / Garder » ; `onPlanifierRelance` de LeadsPage pointe ce même popover ; PlanActiviteDialog garde les gabarits (rôle documenté). **Done =** date libre + objet en ≤3 taps de plus que le chip, plus aucun écrasement silencieux (test), une seule surface de planification rapide. Files: frontend/src/features/crm/CallLogPopover.jsx, frontend/src/pages/crm/leads/LeadsPage.jsx, frontend/src/api/recordsApi.js. (ROUTINE — M, sonnet) (@lane: ez-crm)
- [x] EZ2 — **Le nudge « noter l'appel » marche aussi au bureau.** État vérifié : `CallLogPopover.jsx:183-193` dépend de `visibilitychange` — sur poste fixe le tap `tel:` ne masque pas l'onglet, le nudge n'apparaît JAMAIS et le repli coûte 7 clics via la fiche. Fix : au clic ☎ (`ListView.jsx:276-281`), armer AUSSI un nudge temporisé (~45 s, délai INJECTABLE pour que la gate EZ17 l'avance via `page.clock`) et au retour de focus fenêtre — le premier déclencheur gagne, jamais deux nudges. **Done =** bandeau « Appel terminé — noter ? » sur desktop sans changement d'onglet (test des 2 chemins), mobile inchangé. Files: frontend/src/features/crm/CallLogPopover.jsx, frontend/src/pages/crm/leads/views/ListView.jsx. (ROUTINE — S, sonnet) (@lane: ez-crm) (@after: EZ1)

**B — LE TRAJET DU DEVIS (plus jamais d'abandon après 20 min de saisie) :**
- [x] EZ3 — **Créer un devis mène à l'ENVOYER (fin de l'abandon post-création).** État vérifié : `DevisGenerator.jsx:225-228` — `finish()` en pleine page fait `navigate('/ventes/devis')` et JETTE le `devisId` (le mode embarqué reçoit `onDone(devisId)`). BONNE NOUVELLE vérifiée : `DevisList` lit DÉJÀ `?devis=<id>` et surligne+scrolle (`:1214-1220`, test `DevisListDeepLinks.test.mjs`) — il suffit de passer l'id. Fix : panneau de succès (numéro + total) avec actions immédiates — Envoyer WhatsApp (flux existant `DevisList:1522`), Aperçu PDF (@coord APX14 — même dialog), « Retour à la liste » → `?devis=<id>` (surligné). **Done =** création → envoi WhatsApp en ≤2 clics sans re-chercher (test), retour liste = devis en évidence. Files: frontend/src/pages/ventes/DevisGenerator.jsx. (ROUTINE — S, sonnet) (@lane: ez-devis)
- [x] EZ4 — **Le brouillon n'a plus d'angle mort (les LIGNES ne sont pas couvertes).** État vérifié : le prédicat `dirty` de l'autosave VX62 (`DevisGenerator.jsx:436-439`) ignore `lines`, `discountPct`, `tauxTva`, `villaGroups` — or ces 4 champs sont DÉJÀ dans `draftSnapshot` (`:408-431`) : seul `dirty` est à corriger. Fix : `dirty` couvre tout l'état substantiel + indicateur discret « Brouillon enregistré à HH:MM » (la confiance vient de la continuité visible — pattern Docs/Notion). **Done =** ajouter 3 lignes puis recharger → tout est là (test), indicateur visible. Files: frontend/src/pages/ventes/DevisGenerator.jsx. (ROUTINE — S, sonnet) (@lane: ez-devis) (@after: EZ3)
- [x] EZ5 — **Dimensionner en kWc, pas en nombre de panneaux.** État vérifié : aucun champ « puissance cible » — on tape un NOMBRE DE PANNEAUX (`:2358`) et on relit le kWp calculé (`:2370`) ; la conversion kWc→panneaux EXISTE déjà (`DevisGenerator:704-719` `tailleKwc`, `autoQuote.js:109-112`) — la réutiliser, pas la réécrire ; le « Devis automatique » du workspace (`features/crm/workspace/DevisTab.jsx:152`, ≈5 taps) n'a pas de ciblage kWc. Fix : champ « Puissance cible (kWc) » bidirectionnel avec nbPanneaux (garde `step="any"` intacte), kWc cible optionnel sur le Devis automatique. QX43-52 livrées : ne rien défaire. **Done =** taper « 3 » kWc remplit les panneaux (et inversement), tests generator + DevisTab, garde noValidate verte. Files: frontend/src/pages/ventes/DevisGenerator.jsx, frontend/src/features/crm/workspace/DevisTab.jsx. (ROUTINE — M, sonnet) (@lane: ez-devis) (@after: EZ4) (@coord NTMFG18 — même fichier, additif)

**C — LE TRAJET DU TECHNICIEN (40-45 taps → ≤15 hors photos, zéro photo perdue) :**
- [x] EZ6 — **La paperasse de statut disparaît : les horodatages FONT le statut (10 taps/jour rendus).** État vérifié : `MaJourneePage.jsx:433-442` + `apps/installations/views/intervention.py` (depart_depot :629-649, checkin :661-696 — AUCUN ne touche `statut`) : le technicien fait 5 changements manuels de Select que l'app peut déduire ; le Select propose les 6 statuts sans filtre. ATTENTION deux gardes : `canMoveStatus` (`statuses.js:74`) est la machine des CHANTIERS, PAS des interventions — le filtre du Select = adjacence dans `Intervention.STATUT_ORDER` (rang ±1, un ordre de liste, pas une règle métier) ; et la décision VX105 (`MaJourneePage.jsx:387-389`) interdit de dupliquer `transition_block_reason` côté client — elle reste seule autorité. Fix : chaque horodatage applique la transition correspondante AUTOMATIQUEMENT avec toast-undo 6 s (le recul est explicitement autorisé serveur, `field_services.py:405-406`) ; si le serveur refuse (ex. `tout_charge` manquant — pré-vérifié côté écran quand l'info est là), NO-OP silencieux + indice inline, JAMAIS un toast rouge ; le Select reste pour les cas manuels, filtré par adjacence. **Done =** journée type = ≤3 interactions de statut au lieu de 10 (gate EZ17), transition non-adjacente non proposée, refus serveur sans toast rouge (test), undo testé. Files: frontend/src/pages/interventions/MaJourneePage.jsx. (ROUTINE — M, sonnet) (@lane: ez-terrain)
- [x] EZ7 — **La signature ne peut plus être oubliée.** État vérifié : `FLOW_TABS`/`NEXT_ACTION` vivent dans `MaJourneePage.jsx:351-362/:367-373` (rail `overflow-x-auto` :465) — la signature est l'onglet 9/10, ABSENTE de NEXT_ACTION, hors de la garde « Terminée » (`field_services.py:417-426`), et `SignatureClientPanel.jsx:85` étiquette le nom « optionnel ». Fix : la signature entre dans la séquence NEXT_ACTION (proposée quand photos OK + statut Sur site) ; réglage société ADDITIF « Signature client obligatoire pour clôturer » (booléen sur `CompanyProfile`, `parametres/models_company.py:12` — patron additif déjà utilisé 4×, défaut OFF = rien ne bloque) vérifié par la garde serveur existante étendue ; libellé « optionnel » revu. NOTE propriété : `apps/installations` = contrat PLAN_SERVICE (conflit de code possible, aucune migration chez eux — la migration va dans parametres). **Done =** Prochaine action mène à la signature au bon moment, réglage ON → clôture refusée sans signature (test backend additif), défaut OFF byte-identique. Files: frontend/src/pages/interventions/MaJourneePage.jsx, frontend/src/features/installations/SignatureClientPanel.jsx, backend/django_core/apps/installations/field_services.py, backend/django_core/apps/parametres/models_company.py. (SCHEMA — M, sonnet) (@lane: ez-terrain) (@after: EZ6)
- [x] EZ8 — **AUCUNE photo perdue : l'outbox EXISTANT s'étend aux binaires (jamais un 2ᵉ outbox — décision VX105 ×3).** État vérifié : `InterventionFieldExecution.jsx:39-54` — un échec réseau pendant l'upload = photo PERDUE (toast `duration: Infinity`) ; l'infra existe : `features/installations/offline/{outbox.js,idbStore.js}` (clés d'idempotence) + badge `OfflineSyncIndicator` (compteur + « Synchroniser ») ; NTMOB1 (ouvert) réserve `frontend/src/lib/offlineOutbox.js` — NE PAS y toucher. Fix : étendre CET outbox aux charges binaires — `ArrayBuffer` (pas des Blob : IndexedDB Safari instable), photos COMPRESSÉES avant mise en file (réutilise la compression VX77 — un toit = 3-5 Mo), plafond de file explicite + erreur visible si quota (`navigator.storage.estimate()`), honnêteté iOS documentée (Safari purge après ~7 j d'inactivité), retry auto à la reconnexion, badge TOUJOURS visible, purge au logout (patron LW45). **Done =** coupure réseau pendant 3 photos → 0 perdue, badge exact, sync au retour (test), quota dépassé = message clair, purge logout testée. Files: frontend/src/features/installations/offline/outbox.js, frontend/src/features/installations/offline/idbStore.js, frontend/src/features/installations/InterventionFieldExecution.jsx. (ROUTINE — L, opus) (@lane: ez-terrain) (@after: EZ6) (@coord NTMOB1/2/3 — la file générique des mutations JSON leur appartient ; les BINAIRES sont couverts ICI, pas de double pile)
- [x] EZ9 — **Mode « Plein soleil » (le technicien PV travaille littéralement au soleil).** Recherche terrain : le contraste perçu s'effondre en lumière directe ; les leaders ont un mode dédié qui compense le glare sans grossir la police. Fix : toggle « ☀ Plein soleil » sur Ma journée + Interventions (+ Mes préférences), implémenté en attribut `[data-sunlight='1']` sur le patron EXISTANT `[data-density]` (`tokens.css:421-430` — pas un 3ᵉ thème) : fond blanc pur, encre noire pure, bordures renforcées, ombres/dégradés coupés ; persisté par utilisateur ; AA largement dépassé. **Done =** bascule instantanée sur les 2 écrans terrain, tokens only, captures soleil vs normal, persistance testée. Files: frontend/src/design/tokens.css, frontend/src/pages/interventions/MaJourneePage.jsx, frontend/src/pages/interventions/InterventionsPage.jsx, frontend/src/pages/preferences/PreferencesPanel.jsx. (ROUTINE — S, sonnet) (@lane: ez-terrain)

**D — LE TRAJET DU MAGASINIER (deux modules enfin reliés — le montage du scan appartient à NTWMS5) :**
- [x] EZ10 — **Réception sans surprise : défauts + recherche + « Ranger maintenant ».** État vérifié : `ReceptionsFournisseur.jsx:53` — la date vaut `''` ; le sélecteur BCF (`:130-141`) est le `Select` Radix maison SANS recherche (le kit renvoie explicitement vers le `Combobox` existant pour ça — zéro composant nouveau) ; quantité reçue non pré-remplie ; la confirmation (`:107`) est une bannière inline qui n'offre pas la suite ; réception (Stock) et rangement (Magasin) = deux navigations sans lien. Fix : date = aujourd'hui, quantité reçue = commandée par défaut (valider bat retaper — Fiori/Zuko), BCF en Combobox, bannière de succès → bouton « Ranger maintenant » (deep-link simple vers `/magasin/rangement` — SANS pré-filtre : ni l'écran ni `PutAwayViewSet` ne le supportent aujourd'hui, et 1 tap suffit au budget). **Done =** réception standard = 3 clics + 1/ligne d'écart (gate EZ17), enchaînement en 1 tap, tests. Files: frontend/src/pages/stock/ReceptionsFournisseur.jsx. (ROUTINE — M, sonnet) (@lane: ez-magasin) (@coord NTWMS5 — le poste scanner monte le flux scan construit-mais-jamais-monté (`features/magasin/scan/*` : 0 site de montage vérifié) ; EZ n'y touche pas)
- [x] EZ11 — **Le casier EFFECTIF redevient vrai (fin de la donnée fausse en silence) — frontend PUR, l'API est prête.** État vérifié : `PutAwayScreen.jsx:38` appelle `rangerPutAway(row.id)` SANS casier alors que TOUT existe déjà : `installationsApi.rangerPutAway(id, binId)` envoie `{bin}` (`installationsApi.js:307-308`), `PutAwayViewSet.ranger` (`apps/installations/views/putaway.py:86-111`) valide un `bin` optionnel company-scopé et pose `bin_effectif`, et `getBinLocations` liste les casiers — l'écran n'a juste jamais branché le paramètre (sa docstring `:13-14` promet pourtant « éventuellement vers un autre casier »). Fix : sélecteur de casier au rangement (suggéré pré-choisi = 1 tap inchangé, liste de la zone sinon). **Done =** ranger ailleurs enregistre le casier réel (test bout-en-bout), défaut 1 tap intact. Files: frontend/src/features/magasin/PutAwayScreen.jsx. (ROUTINE — S, sonnet) (@lane: ez-magasin) (@coord NTWMS2 — son « casier suggéré modifiable » vit À LA RÉCEPTION avec moteur de règles ; EZ11 = au RANGEMENT sur l'écran existant — frontière nette) (@coord NTWMS5)

**E — LE TRAJET DU COMPTABLE (le meilleur parcours de l'app, terminé jusqu'au bout) :**
- [x] EZ12 — **L'encaissement offre sa suite (la bonne : les Encaissements, pas le rapprochement bancaire).** État vérifié et PRÉMISSE CORRIGÉE par la critique : `/comptabilite/rapprochements` = rapprochement relevé↔grand-livre, « STRICTEMENT DISTINCT de l'import de paiements clients » (`compta/models.py:1227`) — un paiement fraîchement encaissé n'y apparaît PAS (il faut l'import du relevé) ; aucune occurrence du CONCEPT de lettrage côté ventes (grep — 2 faux positifs « lettre de relance »). La suite honnête d'un encaissement : le toast de succès de `PaiementDialog` → « Voir l'encaissement » (deep-link `/ventes/paiements?…` — `PaiementsPage` lit déjà `useSearchParams`). ET : « Suggestions » + « Accepter les non-ambiguës » (`RapprochementsPage.jsx:314`, handler `:60-69`) — le chemin 3-clics — sont ENFERMÉS dans la modale de détail (1er bouton du footer, mais il faut ENTRER dans un rapprochement pour les voir) : les promouvoir en action de premier niveau de la page. Un éventuel pont paiements↔rapprochement bancaire = future tâche BACKEND, hors périmètre. **Done =** encaissement → encaissement visible en 1 clic, Suggestions accessibles sans ouvrir une modale, tests. Files: frontend/src/pages/ventes/FactureList.jsx, frontend/src/features/compta/pages/RapprochementsPage.jsx. (ROUTINE — M, sonnet) (@lane: ez-compta)
- [x] EZ13 — **Le bouton dangereux dégradé : « ✓ Payée » ne gagne plus contre « ⚡ Encaisser ».** État vérifié : `FactureList.jsx:394-398` vs `:377-382` — « Payée » (marquage sec + `window.confirm`, DÉTRUIT mode/date/référence) côte à côte avec « Encaisser » (3 clics, tout capturé) : sur une liste chargée le bouton pauvre gagne. Fix : « Encaisser » devient l'unique action rapide, avec option « paiement simple (sans détail) » DANS le dialog ; « Payée » relégué au menu ⋯ avec AlertDialog expliquant la perte. Ordre FIXÉ avec APX17 (qui liste `FactureList:921`) : APX17 d'abord, EZ13 rebase. **Done =** le chemin riche est LE chemin rapide, l'ancien atteignable mais expliqué, tests FactureList adaptés. Files: frontend/src/pages/ventes/FactureList.jsx. (ROUTINE — S, sonnet) (@lane: ez-compta) (@after: EZ12, APX17)

**F — LES DEUX ABSENTS SYSTÉMIQUES + LES GARDES :**
- [x] EZ14 — **Undo universel : « appliquer TOUT DE SUITE + inverse à l'annulation » (jamais le commit différé).** Carte de couverture : seuls des câblages ponctuels existent (VX95, NTUX6) — aucun mécanisme générique dans aucun plan. ATTENTION (critique vérifiée) : `toastWithUndo` (`lib/toast.js:108`) est un COMMIT DIFFÉRÉ (setTimeout 6 s) — sur un board, il se perd au démontage = écriture perdue ; et `resilientMutation` n'est pas un rollback. Fix : util unique `lib/mutateWithUndo.js` — applique la mutation immédiatement (optimiste), toast « Annulé ↩ » 6 s (durée exposée en paramètre — @coord NTUX27 `duree_undo_toast` tenant), à l'annulation exécute l'appel INVERSE (PATCH inverse propre vérifié pour : étape lead hors funnel-argent, assignation, tags, priorité/canal, archivage, statut d'intervention — recul autorisé serveur) ; effet chatter documenté (l'annulation crée une 2ᵉ ligne d'historique, libellé « annulé ») ; adoption première vague sur le REGISTRE FERMÉ ci-dessus ; JAMAIS pour l'argent, les suppressions dures, les envois. Doctrine publiée dans UIShowcase (« quand undo, quand confirmer »). **Done =** ≥6 mutations passées à l'undo (liste committée), aller-retour serveur testé, zéro undo sur l'argent (grep du registre), navigation pendant le toast ne perd RIEN (test anti-commit-différé), doctrine publiée. Files: frontend/src/lib/mutateWithUndo.js (nouveau), frontend/src/lib/toast.js, frontend/src/pages/crm/leads/views/KanbanView.jsx, frontend/src/pages/crm/leads/views/ListView.jsx, frontend/src/pages/ui/UIShowcase.jsx. (ARCH — L, opus) (@lane: ez-systemique)
- [x] EZ15 — **Dictée inline au BUREAU (frontière nette avec NTMOB30 qui possède le terrain).** Carte de couverture + critique : NTMOB30 (ouvert, `new_tasks_plan.md:978`) = enregistrement + transcription SERVEUR (Whisper) pour les surfaces terrain/mobile — EZ15 = dictée INLINE navigateur (Web Speech `webkitSpeechRecognition`, fr-FR, streaming dans le champ), zéro backend, zéro clé, pour le BUREAU : composer du chatter lead (workspace) + note ticket SAV + motif de perte — PAS l'intervention (terrain = NTMOB30 ; frontière écrite dans les deux fichiers, jamais deux boutons micro sur un même champ). HONNÊTETÉ (corrigée) : l'audio est traité par le service de reconnaissance DU NAVIGATEUR (Google pour Chrome/Edge) — documenté dans le HelpTip, jamais présenté comme local ; Firefox non supporté, Safari partiel (bouton masqué si API absente — sur iPhone le clavier natif dicte déjà), HTTPS + permission micro requis, redémarrage auto après coupure ~60 s de silence. **Done =** dicter 2 phrases dans le chatter fonctionne (test manuel documenté + test de rendu conditionnel), champ inchangé si API absente, HelpTip de confidentialité posé. Files: frontend/src/ui/DictationButton.jsx (nouveau), frontend/src/features/crm/workspace/LeadWorkspace.jsx, frontend/src/pages/sav/TicketsPage.jsx. (ROUTINE — M, sonnet) (@lane: ez-systemique)
- [x] EZ16 — **Garde CI anti-jargon : plus jamais de JSON brut dans un toast (8 sites réels à purger d'abord).** État vérifié (critique — pas 1 site, HUIT dans 7 fichiers) : `ParametresEntreprise.jsx:875`, `MouvementsPage.jsx:408` et `:496`, `OcrStockImport.jsx:693`, `StockList.jsx:915`, `VentesKanban.jsx:385` (@coord APX15 — le premier arrivé nettoie, l'autre rebase), `features/ia/copilotMessages.js:43`. Fix : corriger les 8 sites (pattern `frenchError`), puis `scripts/check_frontend_errors.py` qui échoue sur `JSON.stringify(err` dans pages/+features/ et sur `toast.error(err)` nu (0 occurrence aujourd'hui — garde anti-régression), allowlist vide à la fin ; ENREGISTREMENT DOUBLE obligatoire : `.github/workflows/ci.yml` (job stage-names, étapes explicites — aucune auto-découverte) ET `scripts/preflight.ps1:113-123` (liste codée en dur) ; noter au DONE LOG que « 10 sous-checks » devient 11 (mention CLAUDE.md, décision fondateur de l'y refléter). Il n'arbitre PAS le contrat d'erreur unique (VX203, gaté). **Done =** 8 sites purgés, garde rouge sur un `JSON.stringify(err` planté exprès (test du test), verte sur le repo, câblée aux DEUX endroits. Files: scripts/check_frontend_errors.py (nouveau), .github/workflows/ci.yml, scripts/preflight.ps1, frontend/src/pages/parametres/ParametresEntreprise.jsx, frontend/src/pages/stock/MouvementsPage.jsx, frontend/src/pages/stock/OcrStockImport.jsx, frontend/src/pages/stock/StockList.jsx, frontend/src/pages/ventes/VentesKanban.jsx, frontend/src/features/ia/copilotMessages.js. (ROUTINE — L, sonnet) (@lane: ez-gardes)
- [ ] EZ17 — **La gate des trajets : les budgets de clics deviennent des specs CI (méthode de comptage ÉCRITE).** Nouvelle spec `frontend/e2e/parcours-budget.spec.js` : rejoue les 5 trajets sur `manage.py seed_demo` (identifiants `e2e/helpers.js`) et COMPTE les interactions. MÉTHODE (imposée par la critique) : état de DÉPART et d'ARRIVÉE déclarés par trajet ; unité = 1 par `fill()` et 1 par « choisir une option » quel que soit le widget ; comptage par compteur d'événements `isTrusted` injecté via `page.addInitScript` CROISÉ avec un helper de test qui incrémente à chaque action (échec si divergence) ; le nudge 45 s d'EZ2 avancé par `page.clock` (Playwright ≥1.45, jamais `waitForTimeout`) ; projet/viewport DÉCLARÉS + `testIgnore` sur les autres projets ; mesure avec réglage signature OFF (EZ7). BUDGETS : appel+note+rappel — chemin rapide (chip) ≤4, chemin daté libre ≤6 ; création devis (formulaire déjà saisi) → WhatsApp prêt ≤6 ; clôture intervention ≤15 hors prises de photo ; réception ≤3 + 1/ligne d'écart ; encaissement ≤3 puis suite en +1. **Done =** 5 parcours verts et stables (2 runs), rouge si un clic superflu s'ajoute (test du test), méthode documentée en tête de spec. Files: frontend/e2e/parcours-budget.spec.js (nouveau). (ROUTINE — L, sonnet) (@lane: ez-gardes) (@after: EZ2, EZ3, EZ6, EZ7, EZ10, EZ13)
...
- [x] VAO1 — **PRÉ-REQUIS D'INSERTION (fait dans le commit qui ajoute ce groupe, jamais en lane) : déclarer le préfixe `VAO` + annoter les deux tâches AOF que ce groupe croise.** Constat : `scripts/check_build_order.py` (job `stage-names`) fait échouer la CI sur tout préfixe présent dans un fichier plan et absent de `docs/BUILD_ORDER.yml` — dès que les lignes VAO entrent dans `docs/PLAN2.md`, le job passe ROUGE si le préfixe n'est pas déclaré. `VAO` rejoint `unmapped_ok` (comme ODY/APX/EZ/VX/QX : groupe PLAN2 sans prérequis de vague — il ne compose que des primitives plateforme livrées à 100 %). Dans le MÊME commit : annoter **AOF169** (`docs/PLAN.md`) — sa ligne « JAMAIS de scraping du portail marchespublics.gov.ma » reste vraie **pour `apps/ao`** et son test de grep reste valable, la voie automatique vivant dans `apps/veille_ao` sous gate règle #5 ; et **AOF1** — la fonction ajoutée par VAO30 en fin d'`apps/ao/services.py` doit voyager avec le relogement du corps depuis `compta`. Puis rafraîchir §10 « Plan status » de `docs/CODEMAP.md` et re-stamper `codemap_fingerprint.py --write`. **Done =** `check_build_order.py`, `codemap_fingerprint.py --check` et `check_stages.py` verts au moment de l'insertion ; `plan_lanes.py docs/PLAN2.md` émet les lanes VAO sans refus d'ordre de vague. Files: `docs/BUILD_ORDER.yml`, `docs/PLAN.md`, `docs/CODEMAP.md`, `docs/PLAN2.md`. (ROUTINE) (@lane: gov/veille-ao) (@model: haiku)
...
- 2026-08-01 — **Retour fondateur round 3 (balayage « the best », barre grise, étape encore visible).** APX6 INVERSÉ sur ordre fondateur (« enlève ça ») : la barre segmentée d'activité des en-têtes de colonne est retirée (JSX + CSS + mécanisme de filtre par colonne), contrat d'ABSENCE posé dans KanbanActivityBar.apx6.test.mjs ; la somme LB9 reste. « Nom de l'étape encore visible » : le tap iOS donne le focus au wrapper tabbable dnd-kit et `:focus-within` dépliait le StageMover sous le doigt → `:has(:focus-visible)` (chemin clavier préservé), ET au pointeur grossier le StageMover n'est plus MONTÉ du tout (`onInlineSave=undefined` → 9 nœuds + un <select> natif rendus par carte). Balayage « the best » (audit Opus) : RENDER_CAP_TACTILE=10 (240→60 cartes montées, ~15 000→~4 000 nœuds sous le doigt — le poids n°1), base de « Charger plus » alignée sur le cap actif, `scroll-padding-left: 0.5rem` (fin de la traction de 8 px à la pose), et RÉSURRECTION du haptique de pose : usePanScroll renvoie une FONCTION callback-ref, `boardRef.current` était undefined en silence (classe #43 inversée) — le nœud vit désormais sur `attach.node` (posé à la création, `react-hooks/immutability`), sondes mises à jour pour rendre la panne impossible à répéter. Faux remèdes explicitement rejetés par l'audit : content-visibility sur .kb-col (efface le scroll vertical des colonnes + pop-in au snap), will-change (~30 Mo de backing store), pager JS. 170 node:test + 42 vitest + lint 0 + build OK. PR dev-lead-feel round 3.

- 2026-08-01 — **Retour fondateur téléphone (feel du board + fenêtre lead).** Board leads mobile : le swipe de carte ne s'arme plus sur le bruit du pouce (verrou d'axe décidé une fois, 12 px + dominance 1,5×, transition réservée à l'aimantation), TouchSensor 300 ms/5 px et surtout DÉMONTÉ par défaut au doigt (son écouteur touchmove non-passif window ralentissait toute la page) — le glisser passe par « Réorganiser par glisser » du menu ⋯ mobile, le StageMover reste le chemin sans-drag ; snap `x proximity`/`start` (le geste diagonal n'est plus volé), momentum iOS + overscroll contenu sur la colonne, plus de « pop » :active ni de dépliage au tap iOS. Fenêtre lead : les 6 pastilles d'étape deviennent UNE pilule+menu ≤44 px (SIGNED garde SigneDialog, raccourcis 1-4 intacts) et le pavé de 6 boutons (~300 px sans info, doublons de la thumbbar) devient la bande « Faits clés » (montant du dernier devis + statut, canal, dernier échange humain depuis chatter_recent — ZÉRO fetch nouveau) + 2 CTA + menu ⋯. 47 tests vitest + tests swipe verts, eslint 0, build OK. PR dev-lead-feel.

- 2026-08-01 — **Groupes APX + EZ (34 taches APX sur 36, 16 EZ sur 17) — l'INTERIEUR des apps et les 5 trajets quotidiens.** APX1 pose la decision fondateur : la PORTE du CRM est `/crm/leads` (le cockpit n'est plus jamais `items[0]` ni la cible du fil d'Ariane) — corrige du meme coup les 4 surfaces qui derivaient toutes du meme `items[0]` (lanceur, epingles, atterrissage, breadcrumb) et 2 liens morts `/crm/clients/<id>`. APX2-9 : la carte de lead passe de 120-150 px a la densite Odoo, chrome vertical reduit, 6 colonnes visibles, plafond de rendu (500 leads testes), anatomie tactile. APX11-17 : les 9 ecrans Ventes prennent l'identite du module, KPI en `<Stat>`, chaine documentaire devis→BC→facture (un lien amont pointait sur `?ref=` que la liste ne lit pas — corrige en `?devis=<id>`), apercu PDF inline, kanbans, et 12 `window.confirm/alert` remplaces par les dialogues maison. APX18-21 : photo produit — **le plan demandait un `ImageField`, la garde plateforme ARC26 l'interdit** : rebati sur `records.Attachment` (validation par octets magiques, plafond 10 Mo, stockage cloisonne par societe, telechargement same-origin herites au lieu d'etre reinventes) ; jauge de stock, rupture distinguee de sous-seuil, fiche technique, courbe de pompe. APX22-24, APX32-35 : cockpits stock/inventaire, 15 ecrans sur `PageHeader`, correction du bug de mode sombre sur 56 ecrans (hex codes en dur → tokens), densite des tables legacy, cockpit tresorerie avec 4 tranches d'age cliquables. APX25-29 : fiche chantier en 6 onglets (15 sections empilees), timeline unifiee, **vraie grille horaire — `date_prevue` ne porte AUCUNE heure et il n'existe pas de champ duree, donc seules les interventions avec fenetre de RDV sont placees sur l'axe, les autres restent dans une bande « Sans creneau » assumee plutot qu'a une heure inventee** ; tournee sur carte. EZ1-2, EZ14-15 : appel+note+relance en 4 clics, undo universel, dictee inline. EZ3-5 : creation de devis enchainee, brouillon a angle mort ferme, kWc cible. EZ6-9 : les horodatages font le statut (avec annulation de 6 s qui rejoue l'appel INVERSE), signature client rendue obligatoire par societe, aucune photo perdue hors ligne (ArrayBuffer + plafonds explicites), mode « Plein soleil ». EZ10-13 : reception, rangement au casier, encaissement enchaine. EZ16 : garde CI des erreurs frontend — `stage-names` passe de 10 a 11 sous-verifications. APX8/APX36 : gates de densite et captures visuelles. **NON TERMINE : EZ17** (budget de clics) — 2 trajets sur 5 sont joues et verts, les 3 autres n'ont AUCUNE couverture e2e existante a reutiliser et sont livres en `test.fixme` avec leur budget declare ; la tache reste `[ ]` car sa clause « 5 parcours verts » n'est pas remplie.

- 2026-08-01 — **APX21 — Les courbes de pompe sont enfin TRACÉES : 11 pompes avaient leurs données constructeur, personne ne les voyait.** Les pompes OSP 30 embarquent depuis leur mise au catalogue la courbe débit→HMT du fabricant. Elle servait UNIQUEMENT au dimensionnement (choix de la pompe pour une HMT donnée) et n'apparaissait à l'écran que sous forme d'un badge texte « courbe constructeur » : impossible de voir à quoi ressemblait la pompe qu'on vendait. Le nouvel onglet Fiche technique (APX20) affiche désormais le graphe débit (m³/h) en abscisse, HMT (m) en ordonnée — grille discrète du kit maison, couleurs par tokens donc lisible en clair comme en sombre, animation coupée sous `prefers-reduced-motion`, et un repli tabulaire accessible (« Voir le tableau ») pour que la donnée chiffrée reste lisible sans les pixels. Lecture seule : rien n'est modifiable depuis là. **Une pompe sans courbe ne rend RIEN** — pas de carte vide sur les dizaines de produits qui n'en ont pas. La lecture de la courbe applique EXACTEMENT les mêmes garde-fous de forme que le dimensionnement (`debitAtHmt`) — longueurs égales, ≥ 2 points, valeurs numériques — pour que le graphe ne puisse jamais montrer autre chose que ce qui sert à dimensionner. Zéro dépendance nouvelle (recharts et le kit `ui/charts` étaient déjà là), aucune migration. **Note technique :** les fonctions pures d'APX19/20/21 (`severiteStock`, `jaugeStock`, `estPompage`, `pointsCourbePompe`) vivent dans `features/stock/catalogue.js` et non dans les écrans — la règle eslint `react-refresh/only-export-components` est en ERREUR sur ce dépôt et refuse qu'un fichier de composant exporte autre chose que des composants.

- 2026-08-01 — **APX20 — La promesse de la fiche produit est enfin tenue.** La création rapide d'un produit annonce depuis toujours « vous pourrez compléter la fiche complète (catégorie, marque, garantie…) plus tard depuis Stock » — sauf que Stock ne le permettait pas : `marque` et le `garantie` TEXTE existaient au modèle et partaient déjà sur les fiches produits des PDF de devis, mais aucun écran ne les exposait (seules les DURÉES numériques `garantie_mois`/`garantie_production_mois` l'étaient). On ne pouvait donc les remplir que par l'admin Django, ou pas du tout. Les deux champs sont maintenant dans le formulaire produit — la marque à côté du SKU, le texte de garantie en tête de la section Garantie, clairement distingué des durées en mois qui alimentent les horloges du parc d'équipements. En face, la fiche produit gagne un 3ᵉ onglet **Fiche technique** (à côté des deux onglets achats) qui MONTRE ce qui partira chez le client : marque, garantie, description, plus puissance et tension pour un produit de pompage seulement. Un champ vide y dit « Non renseigné » plutôt que de disparaître — sinon on ne saurait pas qu'il manque. Aucune migration, aucun backend touché : le sérialiseur acceptait déjà les deux champs.

- 2026-08-01 — **APX19 — Le niveau de stock se lit sans le lire, et le catalogue n'a plus qu'UNE hauteur de ligne.** Avant : le stock était un chiffre nu qui passait en gras-rouge quand il était bas, et le MÊME badge « stock bas » servait pour une rupture (0 en stock, on ne peut plus vendre) et pour un sous-seuil (il en reste, il faut recommander) — deux urgences très différentes, un seul signal. Maintenant : une mini-jauge colorée (vert/orange/rouge, le composant `Progress` déjà utilisé par « Santé catalogue »), et deux badges vraiment distincts, **Rupture** (rouge) et **Sous seuil** (orange). La jauge vise 2× le seuil — exactement la cible de la suggestion de réassort déjà affichée, donc aucun second barème inventé ; sans seuil renseigné elle ne promet rien (pleine si le stock est non nul). **La hauteur de ligne, elle, ne dépend plus des données** : la cellule Seuil empilait jusqu'à quatre éléments (seuil + badge + « commander ~N » + bouton Réapprovisionner) et la cellule Stock jusqu'à trois — chaque zone a désormais une hauteur RÉSERVÉE même vide, et le réassort a quitté la grille pour le menu de ligne, où il est la PREMIÈRE action (donc révélée au survol : 1 clic à la souris, 2 par le menu, toujours visible au toucher) avec la quantité suggérée dans son libellé. Rien n'a disparu : le détail réservé/disponible et la ventilation par emplacement gardent leur ligne, tronquée au lieu de faire grandir la ligne. Aucune migration, aucun backend touché. **Hors périmètre déclaré :** la barre d'action groupée mentionnée par la tâche vit dans `StockList.jsx`/`BulkProductBar`, hors de la liste `Files:` d'APX19 — non touchée.

- 2026-08-01 — **APX18 — La photo produit existe enfin (dégatée par mot fondateur « photos ok »).** Le catalogue montrait des SKU, plus jamais du matériel : un produit peut désormais porter une photo, visible en vignette 40 px dans la liste et en en-tête de sa fiche. **Ce que le plan demandait et ce qui a été fait autrement, à raison :** le plan écrivait « `Produit.image` (ImageField → MinIO) », mais le garde-fou plateforme ARC26 interdit tout NOUVEAU `FileField`/`ImageField` — toute pièce jointe passe par `records.Attachment` (MinIO). La photo EST donc une pièce jointe plateforme, et `Produit.photo` n'est qu'un pointeur vers LA photo canonique (sans lui, n'importe quelle image attachée à un produit deviendrait « la » photo). Bénéfice réel : on hérite gratuitement de la validation par octets magiques, du plafond 10 Mo, du stockage MinIO scopé société et de l'endpoint de téléchargement même-origine — au lieu de réinventer les quatre. **MIGRATION** `stock/0084_apx18_produit_photo` : additive, FK nullable `SET_NULL`, révertable, aucun produit existant touché, seed intact. Un SEUL aller-retour client (`POST /stock/produits/<id>/photo/` téléverse ET rattache dans la même transaction : jamais de pièce jointe orpheline si le client coupe) ; remplacer une photo supprime l'ancienne de MinIO ; `DELETE` la retire. La photo est INTERNE : elle n'entre dans aucun PDF ni sortie client (règle #4 — vérifié par test sur le rendu PDF), et n'est jamais rendue à côté de `prix_achat` (vérifié par test sur les deux surfaces d'affichage). Au passage, le bug latent annoncé par le plan est corrigé : `description` était déclaré DEUX fois dans le corps de `Produit` — Python gardait silencieusement la seconde, la première était morte depuis toujours ; suppression sans effet de base ni migration. Repli construit d'office pour APX19/APX36 : sans photo, la vignette rend l'ICÔNE de la catégorie du produit dans la MÊME boîte 40 px — la hauteur de ligne ne dépend jamais des données. Compression cliente VX77 avant envoi (bord long 1600 px, JPEG), refus serveur relayé en français dans le formulaire, cible tactile 44 px sur le sélecteur de fichier, tokens clair/sombre. **Note d'hygiène CI :** `scripts/on_delete_allowlist.txt` re-tamponné — 29 entrées `stock/models.py` décalées par l'ajout du champ, MÊME jeu de FK, aucun nouveau CASCADE.

- 2026-08-01 — **Groupe ODY, vague 2 (ODY2-ODY14, ODY27, ODY28, ODY30, ODY34) — la bascule du paradigme : j'ouvre l'ERP, je vois MES apps ; j'entre, j'y suis vraiment.** ODY2/ODY3 : le Menu d'accueil `/apps` (grille plein ecran, type-ahead a la Odoo, Favoris puis Recents, halo « Lumiere sur Nuit ») devient la porte d'entree ; l'atterrissage passe par UNE seule resolution partagee par `Login.jsx` et la nouvelle garde `/`, avec les exceptions prevues (preference VX46, mono-app, deep-link/F5, `?next=`). **Correction fondateur apportee a l'atterrissage** : « dernier module visite » etait le defaut IMPLICITE (preference vide == VX11), si bien que tout utilisateur de retour serait entre directement dans son dernier module et n'aurait JAMAIS vu le nouveau Menu d'accueil — il devient un choix EXPLICITE (`__dernier__`) et le Menu d'accueil devient le vrai defaut. ODY4 : le mode immersion — `ActiveAppContext` derive l'app active de la route (fonction pure du pathname, donc deep-link/F5/retour rendent la meme coquille), la Sidebar ne rend plus la pile globale de ~106 destinations mais l'en-tete de l'app + SES items + la sortie « ⊞ Toutes les apps » ; les 4 sections encore codees en dur (tete Dashboard/Ma file/Messages, DOCUMENTS, INTELLIGENCE, ADMINISTRATION) sont supprimees — ce qui resout la duplication reelle que la vague 1 avait rendue mesurable (deux liens /dashboard, deux liens /messages faisaient echouer trois tests). ODY5 : topbar d'app (pastille icone+nom, logo repointe vers `/apps`, le ⊞ devient LA sortie canonique) + breadcrumb hierarchique ; au passage `titleFor` prenait le PREMIER prefixe correspondant, laissant un prefixe generique masquer silencieusement tout sous-titre declare apres lui — il prend desormais le PLUS LONG. ODY7 : audit des liens croises — aucun point d'appel a corriger (la coquille etant derivee de la route, rendre un ecran Ventes « dans » la coquille CRM est devenu structurellement impossible) ; ce qui manquait etait la garde, ajoutee sur 6 parcours reels + 1 e2e. ODY8 : la route d'une app non activee ouvre un vrai ecran « App non activee » (CTA Activer pour l'admin) au lieu d'un renvoi muet. ODY9-ODY14 : `AppIcon` unique sur 4 surfaces, transition signature (View Transitions + repli, instantane sous reduced-motion), prefetch au survol, reordonnancement au glisser (dnd-kit/core seul), etat vide + onboarding. ODY10 : badges vivants derives de l'endpoint federe ARC40 existant — **aucune migration**, une seule requete agregee, cloisonnement multi-societes prouve par test. ODY27 : filtrage « app installee » sur la palette ⌘K, la recherche globale, la cloche et le Dashboard ; corrige au passage SIX liens morts vers `/sav/tickets` (route inexistante) et une action `navigate(undefined)`. ODY28 : fin d'une vraie collision — `AppLauncher` installait un second listener `g a` en plus de celui de `ShortcutsProvider` (les deux tiraient) ; gestionnaire unique, lanceur deplace sur `g o`, `g g` ouvre le Menu d'accueil, les 10 destinations historiques epinglees par un test. ODY34 : changer de societe renvoie vers ses propres apps au lieu de recharger une URL potentiellement interdite dans la nouvelle societe.

- 2026-08-01 — **Groupe ODY, vague 1 (ODY1 + ODY15-ODY23 + ODY30) — les apps deviennent une vraie source unique, chaque app devient un monde complet.** ODY1 : creation de `frontend/src/lib/apps/useInstalledApps.js`, LA source unique « mes apps » (registre moduleConfigs ∩ modules actifs societe ODX6 ∩ role/permission ARC47) ; le lanceur VX9 et les epingles VX10 y sont branches — ils affichaient jusqu'ici TOUTES les apps, y compris celles desactivees par la societe et celles interdites au role (bug reel, jamais cable depuis ODX6). ODY15/ODY16 : passes CRM et Ventes — cockpits `/crm/cockpit` et `/ventes/cockpit` (ModuleHero + actions rapides + KPI), parite route↔nav bouclee (le vieux trou `/crm/payloads-site-web` est comble), sous-groupe `facturation` etiquete pour qu'ODX18 n'ait qu'a le deplacer. ODY17/ODY18 : passes Stock/Magasin et Chantiers/Interventions — ModuleHero sur Stock et Magasin, sous-groupe `achats` pret pour ODX20 ; le ModuleHero de `/chantiers` a ete VOLONTAIREMENT omis (l'e2e mobile MB6 asserte deja un titre « Chantiers » — un second titre aurait casse la spec en strict mode). ODY19 : passe SAV — cockpit `/sav/cockpit` ; decision documentee : PAS de tuile Monitoring separee, les ecrans `/production*` sont deja rattaches a l'app Chantiers. ODY20 : Compta + AO verifies conformes (frais = section de Compta, portail = route publique a jeton, AO backend-only) — tests de verrouillage ajoutes. ODY21 : RH/Paie/Flotte/QHSE verifies complets, aucun orphelin. ODY22 : creation de l'app « Messages » (`features/messaging/module.config.jsx`, cle `chat` = manifeste backend existant) et GED complete — la cle `ged_advanced` devient `ged`, ce qui corrige un vrai bug de gating (l'ancienne cle ne correspondait a aucun manifeste, desactiver la GED ne masquait donc rien). ODY23 : creation de l'app « Intelligence » (`features/ia/module.config.jsx` + cockpit `/ia`, alias `ia`→`agent` dans `check_modules.py`), Rapports gagne le Journal, Parametres absorbe les ecrans d'Administration, Tableau de bord devient sa propre app. ODY30 : kill-switch `VITE_APPS_SHELL` (defaut ON) pose AVANT la bascule, declare dans le triplet complet (`.env.example`, `docker-compose.yml`, `Dockerfile.prod`) — flag build-time, rebuild d'image requis, retrait queued en ODY33.

- 2026-07-31 — **Lane LW8R (LW41-LW45) — espace de travail Lead.** LW41 : le shell ne refait plus le GET `/historique/` au montage quand le détail embarque déjà `chatter_recent` (et l'état mort `dups` du shell est purgé). LW42 : `changeStage` remonte le vrai message serveur (`detail`) au lieu d'un toast unique « Retour d'étape non autorisé », et ne reste plus muet hors-400 (un clic hors-ligne parle enfin). LW43 : gardes d'identité sur `historique`/`dups`/`clientMatch`/compteurs d'onglets — une réponse lente du lead A ne peut plus se peindre sur le lead B après un J/K rapide. LW44 : les « Récents » du ⌘K portent le bon nom dès la première ouverture (clé sur `state.leadId` après `LOAD_LEAD`) et le focus du composer WhatsApp vise le vrai bouton Radix au lieu d'un `input[type=checkbox]` inexistant. LW45 : le cache de préchargement se vide à la déconnexion (fuite inter-session de 60 s fermée) et les chips de préparation QX28 affichent un état « manquant » discret au lieu de disparaître. **Note :** l'écouteur LW45 attend un événement `taqinor:auth-logout` que personne n'émet encore — le nettoyage est correct mais inerte tant que `authSlice.logoutUser`/`SessionProvider` ne le déclenchent pas (câblage requeué dans ce même run). Aucune migration, aucun backend touché.
- 2026-07-31 — **Lane LB8R (LB35-LB40, +LB45 déjà présent) — kanban des leads.** LB35 : `ForecastView` retrouve les trois invariants du kanban (`memo()` sur la carte, retour de focus après un drop clavier, `usePanScroll` en callback-ref) — le retour de focus est chaîné sur la résolution du save, la vue n'étant pas optimiste. LB36 : « Effacer les filtres » pendant le débounce ne ressuscite plus la recherche (détection de reset complet pendant le rendu, volontairement étroite pour ne pas effacer le texte à chaque changement d'étape). LB37 : trois commentaires menteurs supprimés (le polling kanban inexistant, et 3 blocs `perduMotif`/`perduBusy` périmés dans `ListView` — un de plus que prévu). LB38 : `selectionActive` était déclaré, stylé, mais jamais transmis — `KanbanView` passe désormais un booléen (jamais le `Set`, pour préserver `memo`) ; le CSS n'avait besoin de rien. LB39 : l'undo VX95 de changement d'étape remarche — champ `undo` en écriture seule NON persisté (aucune migration), et le serveur re-vérifie dans le chatter que le dernier mouvement d'étape est bien l'inverse exact et date de moins de 5 min ; garde « perdu », garde de tunnel et cloisonnement société inchangés (9 tests d'API). LB40 : la barre d'actions groupées ne couvre plus la scrollbar du board (jeton `--kb-scrollbar-h`), l'overlay de glissement passe au-dessus, et les radios icône-seule du sélecteur de vues ont une infobulle. **LB45 non construit — déjà résolu par LB47 :** sous 768 px il n'existe plus AUCUNE rangée de sélecteur de vues (les 6 vues vivent dans le menu `⋯`), donc construire le dropdown demandé ajouterait un SECOND sélecteur mobile. Deux fichiers hors périmètre déclaré touchés volontairement (`KanbanView.jsx`, `ui/Segmented.jsx`) ; ajouts `index.css` en fin de fichier uniquement.
...
- 2026-08-01 — **VAO1 (insertion du Groupe VAO — « Veille appels d'offres », 42 tâches) :** préfixe `VAO` déclaré dans `docs/BUILD_ORDER.yml` (`unmapped_ok` — app satellite sans prérequis de vague), **AOF169 et AOF1 annotés dans `docs/PLAN.md`** (AOF169 reste vrai et son test de grep valable, sa portée étant `apps/ao` — la collecte automatique vit dans l'app séparée `apps/veille_ao` sous gate règle #5 ; AOF1 doit faire VOYAGER la fonction que VAO30 ajoute en fin d'`apps/ao/services.py` lors du relogement depuis `compta`), §10 de `docs/CODEMAP.md` rafraîchie et re-stampée. **Aucun code construit** — le groupe est une file de tâches, pas une livraison. Provenance : 13 agents de recherche + vérification hands-on de la recette d'accès au portail + synthèse Opus. **Découverte majeure consignée en tête de groupe : l'AO FRDISI n'a jamais été publié** (fondation de droit privé, hors décret 2-22-431) — c'était une consultation privée restreinte, qu'aucun dispositif de veille n'aurait captée ; d'où les DEUX portes du groupe (automatique + manuelle) et l'interdiction de promettre l'exhaustivité dans l'UI. (ROUTINE)
