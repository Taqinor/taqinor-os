...
- [ ] APX36 — **Trois goldens qui prouvent « le plus beau » (pas trente — la taxe de baselines est une décision livrée).** Étendre la régression visuelle existante (VX70/LB34) de TROIS écrans seulement, clair+sombre : board leads compact (APX2), catalogue Stock jauges+icônes (APX19), cockpit finance héros (APX35) — la densité, elle, est prouvée par les ASSERTIONS MESURÉES d'APX8 (robustes), pas par des pixels. Masques sur zones variables, données seedées fixes. **Done =** 3 goldens ×2 thèmes verts et stables (2 runs), un changement non intentionnel les fait rougir. Files: frontend/e2e/visual.spec.js. (ROUTINE — S, sonnet) (@lane: e2e-apx) (@after: APX8, APX19, APX35)
