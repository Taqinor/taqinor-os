...
Generated from commit `c296372` on 2026-06-17.
Structure fingerprint: 9b00546bbce7cdc9fe9c5b185065f0d6e8d72c9c99d6c310e911466efd18789e
...
│   │       ├── reporting/          Dashboards/KPIs/insights/audit-log/notifications/backup (read-only aggregation; one model: NotificationPreference)
...
- **ProductionReleve** (N51) — per-installed-system production reading: `company` FK; `installation` FK→Installation; `periode_debut/fin`; `kwh_produit`; `source` (manuel/monitoring, manual fallback by default — N50 monitoring stays gated); `note`. Drives an on-the-fly performance summary (measured vs expected = kWc × productible × days/365) and the configurable under-performance flag (N52).
- Endpoints (`/api/django/installations/`): `chantiers/` ViewSet + `creer-depuis-devis`, `regime-suggestion`, `{id}/historique|noter|mise-en-service|annuler|reactiver`, `{id}/checklist|cocher-checklist`, `{id}/besoin-materiel|commander-besoin`, `{id}/production` (GET summary+list / POST manual releve) + `{id}/supprimer-production`; `interventions/`; `types-intervention/`; `checklist-etapes/`.
...
### reporting — dashboards, KPIs, insights, audit log, notifications, backup
Read-only aggregation across crm/ventes/installations/sav/stock, role-filtered.
- **NotificationPreference** (N75) — per-user, per-event in-app notification toggle: `company` FK; `user` FK; `event_type` (one of 7 bell categories); `in_app` bool. Absent row = enabled (default: everything shows). Only model in the app.
- Endpoints (`/api/django/reporting/`): `dashboard/`, `search/`, `notifications/` (unified 7-category in-app bell, N75), `notification-preferences/` (GET list / POST upsert, N75), `calendar/` and `calendar/reschedule/` (agenda events + drag-reschedule), `pipeline/` (funnel value by STAGES, weighted forecast), `reports/sales|stock|service/` (+`?export=xlsx`), `insights/recurring-revenue|audit-log|job-costing|analytics|commissions/`, `backup/` (admin tenant data export → multi-sheet .xlsx, N97), `archive/client/{id}/` and `archive/chantier/{id}/`. `job-costing` (margin via internal `prix_achat`), `commissions` (sales commission per `CompanyProfile.commission_mode`) and `backup/` are admin-only.

### parametres — company profile, business settings, WhatsApp templates
- **CompanyProfile** (one per company) — identity + Moroccan legal IDs (`ice`, `identifiant_fiscal`, `rc`, `patente`, `cnss`, `rib`); branding (`logo_key`, `signature_key`, `couleur_principale`); `responsable_defaut_leads` FK (default lead owner); quote-gen knobs (`payment_terms` JSON, `quote_validity_days`, `tva_standard/panneaux`, ROI constants `onee_tarif_kwh`/`productible_kwh_kwc`/`rendement_global`, `remise_max_pct`, `discount_approval_threshold`, `agricole_pump_hours`); `default_installer` FK (default technician for new chantiers, N66; NULL = creator is responsable); sales commission (`commission_mode` off/pct_devis/par_kwc + `commission_valeur`, sensitive/admin-only, N99); referral toggle (`referral_enabled` bool + `referral_reward`, N98); under-performance rule (`seuil_sous_performance_pct` NULL=off + `auto_ticket_sous_performance` bool, N52); `doc_prefixes`/`doc_numbering` JSON.
...
- **installations/** — InstallationsPage, ParcInstallePage, InstallationDetail, ChantierChecklist/Photos/Timeline/Production.
...
7. **Reporting now has one model.** Historically the reporting app had no models;
   as of N75 it owns a single model, `NotificationPreference` (per-user in-app
   notification toggles). It remains otherwise a read-only aggregation layer.
...
- **Provenance window.** Generated from `main` at commit `7d255cd`. Work merged
