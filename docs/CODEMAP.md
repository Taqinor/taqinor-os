...
Structure fingerprint: a133508c13efdb84cd03f87955c742b9118f511c2b2bb23958e2236642400fab
...
- **crm** — leads/clients state; `crmSlice.js`, `bulk.js` (selection logic), `stages.js` (mirrors STAGES.py + CONVERSION_STAGE + per-stage scalars + `isStageMoveAllowed`/`isStageMoveBackward` — CI-checked), `confirmRecul.js` (the ONE backward-stage confirmation, shared by board/list/lead window); **`workspace/`** — the lead cockpit (ex-`pages/crm/LeadForm.jsx`, removed LW40): shell `LeadWorkspace.jsx` (3 zones — `IdentityRail`/`SectionsPane`+`sections/`/`ContextRail` with `TimelineTab`/`DevisTab`/`StageControl`) over the pure autosave engine `draftCore.js` (no-loss reducer + section model: `SECTION_FIELDS`/`sectionAutoRepliee`) via `useLeadDraft.js`; `missingFields.js` (the « À compléter » banner model + `DEVIS_AUTO_FIELD_IDS`, re-exported by DevisTab), `jumpToField.js` (the ONE unfold+scroll+focus jump), `leadPrefetch.js`, `rotting.js`; consumers `ScoreBadge.jsx`, `CallLogPopover.jsx`, `components/ChatterTimeline.jsx`.
