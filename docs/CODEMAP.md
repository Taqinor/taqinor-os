...
Generated from commit `3c9f8eb` on 2026-06-17.
Structure fingerprint: 2cf87f0c2afeee4292a82760552afcb0bcd4986b5ea82408a112e28ad3ce1236
Plan fingerprint: 00166f47de78d8ed7cad0c5a95def90e1fb1fc2d95bf3130074ca8ca99af583e
...
├── .github/workflows/ci.yml      CI: backend-lint, backend-tests, frontend-lint, stage-names, web-build-test
...
- **CustomUser** (extends AbstractUser) — `company` FK→Company; `role` FK→roles.Role (nullable); `role_legacy` (deprecated CharField: admin/responsable/normal); `poste`, `phone_number`, `avatar_key` (MinIO); `is_protected` (owner-account guard), `is_active`, `is_superuser`.
- Endpoints (mounted at `/api/django/`): `POST /auth/register-company/` (public onboarding: new company + admin) · `POST /register/` (admin adds user to own company) · `GET /auth/me/` · `POST /auth/logout/` · `POST /auth/token/refresh/` · `GET/POST/PATCH/DELETE /users/…` + `POST /users/{id}/avatar/` · `GET/POST/PATCH/DELETE /companies/…` (superuser).
...
- Endpoints (`/api/django/parametres/`): `GET /`, `PUT/PATCH /update/`, `POST /upload-logo|upload-signature/`, `DELETE /delete-logo|delete-signature/`, `GET+PUT/PATCH /messages/`, `GET /audit/`.
...
- Endpoints: Role ViewSet (CRUD, admin-only; delete blocked if system or in-use) + `permissions-disponibles/`.
...
SPA built with React 19 + Redux Toolkit + react-router 7. `features/` holds Redux
slices and domain logic; `pages/` holds screens; `api/` holds one axios module per
backend area.
...
- **CI status checks that gate a merge** — `.github/workflows/ci.yml` defines
  **five** jobs: `backend-lint` (flake8), `backend-tests` (Postgres+pgvector +
  Redis + MinIO; runs `python manage.py test apps authentication`), `frontend-lint`
  (eslint + node `--test` solar/catalogue/stages parity), `stage-names`
  (`scripts/check_stages.py` **plus** `scripts/codemap_fingerprint.py --check`,
  which fails the build when this CODEMAP is stale vs the structural surface),
  and `web-build-test` (apps/web astro build +
  vitest). CLAUDE.md designates the **first four** as the required merge gate (0
  approvals, merge-commit self-merge); see §9 for the `web-build-test` caveat.
...
4. **CI has five jobs, not four.** CLAUDE.md and README describe four checks
   (lint/tests/frontend-lint/stage-names). `ci.yml` defines a fifth, `web-build-test`
   (Astro build + vitest for `apps/web`). Whether it is branch-protection-*required*
   is not visible from the repo (see §9), but the job exists and runs on every push.
...
- **Which CI jobs are "required".** The five job names come from `ci.yml`, but the
  ...
  "first four are required" claim as policy, not as a code-verified fact.
...
- **Provenance window.** Generated from `main` at commit `3c9f8eb`. Work merged
...
cross-checked against `main`. Generated from commit `3c9f8eb` on 2026-06-17. This
...
**Totals: 171 tasks — 93 done · 68 open · 10 blocked.**

**Done (93)**

- `N1` — Chantier (projet) object created from an accepted devis, linked to that devis, the lead…
- `N2` — Chantiers list view + kanban grouped by chantier status, same visual language and same…
- `N3` — "Créer chantier" action on a devis and on a lead that creates a chantier from that…
- `N4` — Per-chantier configurable execution checklist; default steps editable in Paramètres…
- `N5` — Photo & file attachments per chantier (reuse existing file-attachment feature) grouped…
- `N6` — Chantier timeline view per job showing signed, material-ordered, planned & actual…
- `N7` — Système installé (parc installé) record auto-created when a chantier reaches…
- `N8` — Parc installé list + map view, searchable/filterable by client, ville, brand, capacity…
- `N9` — Optional per-component serial-number capture on the relevant chantier checklist step so…
- `N10` — Per-installed-system detail screen showing components, warranties, linked SAV tickets…
- `N11` — Bon de commande fournisseur (purchase order): select supplier from existing supplier…
- `N12` — Bon de commande PDF for the supplier (French): SKU, description, quantity, unit buy…
- `N13` — Besoin matériel view per chantier listing components needed from its source devis with…
- `N15` — Multi-location stock (at least main dépôt + camionnette), transfers between locations…
- `N16` — Inventory count & adjustment: admin records a physical count per location and posts the…
- `N17` — Per-SKU multiple-supplier price lists (buy prices from several suppliers +…
- `N18` — Stock valuation per location using average cost from purchase history, shown only in…
- `N19` — Supplier return (retour fournisseur) record for defective/wrong items, decreasing stock…
- `N21` — PV de réception (procès-verbal de réception des travaux) PDF from a chantier (French…
- `N22` — Bon de livraison (delivery note) PDF from a chantier or devis (French): delivered items…
- `N23` — Client handover pack PDF (dossier de remise) per chantier (French): system summary…
- `N24` — Attestation generator (French) from a chantier or installed system (e.g. attestation…
- `N25` — Mark a devis accepted on a chosen date with the accepting person's name captured +…
- `N27` — Full set of Moroccan legal company identifiers in Paramètres company identity (raison…
- `N28` — Client ICE field on the client record, surfaced on devis & factures; carry the client…
- `N29` — Facture conformity check verifying every Article 145 CGI mention (seller…
- `N30` — Explicit delivery-or-prestation date + payment-terms-and-mode fields on factures…
- `N31` — Sequential, continuous, gap-free invoice/quote numbering per document type with no…
- `N32` — Documents archive view per client and per chantier gathering every generated…
- `N33` — Facture d'acompte + facture de solde workflow: chantier/devis generates a deposit…
- `N34` — Configurable default acompte percentage + default payment terms in Paramètres to…
- `N35` — Échéancier / installments option on a facture splitting TTC into dated instalments…
- `N37` — Per-line TVA on devis & factures with an editable TVA rate per line defaulting to a…
- `N38` — Local structured-invoice export for factures: UBL 2.1-shaped XML generated & stored…
- `N39` — Clearance-status placeholder field on factures (Non soumise/Soumise/Validée), purely…
- `N40` — Dossier réglementaire section on each chantier for loi 82-21 self-production: régime…
- `N41` — List + filter of chantiers and installed systems by regulatory dossier régime & statut…
- `N42` — Article 33 régularisation flag + status on installed-system and lead records, with a…
- `N43` — Configurable régime-suggestion: given a chantier kWc + grid-connection type, propose…
- `N44` — SAV ticket object linked to a Système installé (and thus client + chantier): type de…
- `N45` — SAV intervention report PDF on closing a ticket (French): reported issue, diagnosis…
- `N46` — Parts consumption on a SAV ticket optionally decrements stock for parts used and…
- `N47` — Contrat d'entretien object linked to one or more Systèmes installés (start date…
- `N48` — Warranty tracking on each Système installé and components: store install date +…
- `N49` — Recurring-revenue view summarising active contrats d'entretien, monthly/annual value…
- `N54` — Expand Paramètres into a structured settings hub with grouped sections (company…
- `N55` — Admin audit of settings changes (who/what/when, existing audit pattern)
- `N56` — Make every reference list editable by an admin from Paramètres (CRM tags, lead…
- `N57` — Deactivating a reference value preserves it on historical records and only removes it…
- `N61` — Message-template editor in Paramètres for WhatsApp/email/SMS templates (named…
- `N62` — Editable numbering-sequence configuration per document type…
- `N63` — Editable pricing & sizing engine in Paramètres exposing today's implicit quote…
- `N66` — Configurable default lead responsable, default installer, default acompte percentage…
- `N70` — Per-user activity & access view so Reda can see who did what
- `N71` — Admin-defined custom-fields system from Paramètres…
- `N77` — Tableau de bord home view (French): pipeline value/count by stage, close rate by canal…
- `N78` — Job costing per chantier: realised margin from captured buy prices vs invoiced amounts…
- `N80` — Business analytics section: lead-source ROI when ad-spend data is available, average…
- `N81` — Generic import-and-export framework (CSV & XLSX) for major objects…
- `N82` — Per-object export to CSV/XLSX from every list view respecting the user's column &…
- `N83` — Global search across every object (leads/clients/devis/factures/chantiers/installed…
- `N84` — Calendar/agenda view of planned installs, scheduled maintenance visits, SAV…
- `N90` — Installable PWA with Chantiers/Parc installé/SAV/calendar/bon-de-commande screens…
- `N95` — Comprehensive audit log across all objects (creates/updates/deletes + key actions…
- `N98` — Optional referral/parrainage program (referrer→referred-client links, configurable…
- `N99` — Optional sales-commission tracking (configurable commission per signed quote or per…
...
- `A1` — "Marquer comme accepté" control on a devis
- `A2` — Moving a lead into Signé prompts for the accepted devis + option
- `A3` — The chosen option is authoritative downstream
- `A4` — Inline next actions after acceptance
- `B1` — Fix file attachments end-to-end (and the broken devis-PDF icon)
- `C1` — iPhone: menu items cut off / unreachable
- `C2` — Desktop: cold first-load flakiness
- `D1` — Reorganize Paramètres into sections/tabs by domain
- `D3` — Per-document numbering configuration
- `D5` — Avancé: editable + versioned quote logic
...
- `E1` — Playwright + required CI job
- `E2` — Login is the app entry point
- `E3` — Lead lifecycle
- `E4` — Devis from a lead
- `E5` — Inline bill editing
- `E6` — Lead reassignment
- `E7` — Stage transitions
- `E8` — Employee management
- `E9` — Typed activities
- `E10` — File attachments
- `E11` — Duplicate detection
- `E12` — Credit notes (avoirs)
- `E13` — Payment follow-ups & receivables
- `E14` — Paramètres
- `E15` — Cross-cutting health
- `E16` — Mobile pass
...
- The visible `main` history is 85 commits deep. Every **done** task that landed
  inside that window is individually traceable to a merged feature commit whose
  subject names it (e.g. `feat(stock): N15 …`, `feat(stock): N17/N18/N19 …`,
  `feat(crm): N98 …`, `feat(reporting): N84 …`, `feat(parametres): N66 …`,
  `feat(parametres): D1 …`, `feat(numbering): D3 …`, `feat(quote-logic): D5 …`,
  `feat(ventes): A1/A3 …`, `feat(crm): A2/A4 …`, `fix(records): B1 …`,
  `fix(ui): C1 …`, `fix(pwa): C2 …`, and the `N49/N70/N78/N80/N95` insights batch).
  Done tasks that predate this window (T1–T17 and the early N-series) are confirmed
  by the DONE LOG and the "ALREADY LIVE" baseline rather than by an individually
  visible commit — older than the current log depth, not a discrepancy.
- **No done task is missing its feature on `main`.** The only DONE-LOG self-caveat —
  T3 noting it was "developed on branch … not merged to main" — is **superseded**:
  the bulk-actions feature is present on `main` today (`/crm/leads/bulk`,
  `frontend/src/features/crm/bulk.js`, `BulkActionBar.jsx`, `apps/crm/tests_bulk.py`),
  so T3 is genuinely done.
- **Substantial `main` feature commits not represented in `docs/PLAN.md`** are the
  `W*` marketing-site commits (`feat(web): W11…W22`) and the CODEMAP/guard
  infrastructure commits (`docs(codemap): …`, `chore(ci): self-enforce …`). Neither
  is an anomaly: the `W*` work is tracked in its own plan (`docs/WEB_PLAN.md`, scoped
  to `apps/web/**`), and the CODEMAP/guard commits are this freshness machinery, not
  a product feature.
- **Blocked tasks (10)** sit against non-negotiable rule #4 (the vendored
  premium quote engine must not be edited — N26/N36/N59/N60/N67), a calculation-model
  change the founder must validate first (N64/N65 ONEE tranche tables + per-region
  irradiation), or a gated scheduler/monitoring dependency (N53/N76/N79). They stay
  `[BLOCKED]` in the plan and count as open work, never as done.
