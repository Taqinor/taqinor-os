...
Structure fingerprint: dd14dae4ea90ea11215ecf3e87e946352af07d0deae52f463eff7967ce1655ad
Plan fingerprint: b8034e8e264833d4c6e6403aa5af7b7b5ccd602071bc1209238fa37658e11510
...
- **Models** — `VenteComptoir` + `LigneVenteComptoir` (XPOS1: a counter sale that, on `valider/`, becomes a real `ventes.Facture` + `Paiement`(s) + a stock movement via `ventes.services`/`stock.services` — never a direct model import), `SessionCaisse` (XPOS4: caisse open/close, backed by the EXISTING `compta.Caisse` via `compta.services` — no duplicate cash journal), `ShareLinkTicket` (XPOS3: tokenized public link to the ticket PDF, same pattern as `ventes.ShareLink`, kept local to `apps/pos`), `CommandeRetrait` + `LigneCommandeRetrait` (XPOS15: click-and-collect, à_preparer→pret→retire), `ConfigMaterielPOS` (XPOS18: ESC/POS printer config + TPE card-terminal reconciliation).
- Endpoints: `ventes/` (+ `{id}/lignes`, `{id}/valider`, `{id}/ticket-pdf`, `{id}/ticket-escpos`, `{id}/ticket-share-link`, `encaisser-facture/`, `factures-recherche/`, `dashboard/`, `dashboard-export/`), `sessions/` (+ `{id}/cloturer`, `{id}/rapport-z`), `retraits/` (+ `{id}/lignes`, `{id}/marquer-pret`, `{id}/remettre`), `config-materiel/`; public `GET /api/django/public/pos/ticket/<token>/` (tokenized ticket PDF, no auth, same pattern as `ventes.ShareLink`).
...
- **FavoriUtilisateur** — épinglage personnel (cible générique, `ordre`), strictement privé : 404 pour un collègue, même Directeur. **UxParametres** — réglages UX par société (durées, partage d'équipe, rôles autorisés à définir un défaut).

### ai_governance — assistants IA gouvernés  *( `/api/django/ai/` )*
- Rédaction assistée, CR d'intervention dicté, description produit, copilote de configuration, rapport périodique, surveillance de dérive (PSI). **Clé-gaté** sur les clés existantes ; sans clé → 503 explicite, aucun appel réseau. **`prix_achat` n'est JAMAIS lu** : le prompt produit est bâti sur une liste blanche de 4 champs (nom/marque/catégorie/garantie), pas sur une consigne au modèle.
...
- **PlafondNoteFrais** (XACC27) — per-company, per-`NoteFrais.categorie` spend ceiling referential (`montant_max`, optional `seuil_justificatif_obligatoire`); a category with no configured row is never flagged.
...
- Endpoints (dual-served, byte-identical, at BOTH `/api/django/frais/…` and the legacy `/api/django/compta/…`): `notes-frais/` (+ `soumettre|valider|rejeter|rembourser`), `rapports-notes-frais/`, `plafonds-notes-frais/`, `baremes-indemnite/` (auto-demotes prior default), `indemnites-chantier/` (+ lifecycle actions).
...
- `PACT1` — Garde de contrat : un appel frontend vers une route qui n'existe pas ne peut plus être…
- `PACT2` — Garde de forme : un test frontend ne peut plus inventer la forme d'une réponse
- `PACT3` — Les bases de référence ne peuvent que RÉTRÉCIR, et c'est le code qui l'impose
- `PACT4` — Les deux gardes tournent dans `stage-names`, PAS dans `backend-lint` — à dessein
- `PACT15` — L'écran qui plante : tableau de bord AO
- `PACT16` — Les chemins morts du module AO
...
- `LB5` — P1 : « ✗ Perdu » depuis une carte met l'UI à jour (chemin Redux)
- `LB6` — P1 : mémo réparé — une frappe ne re-rend plus tout le board
- `LB7` — P1 : plus de refetch intégral après un PATCH mono-lead + fin des catch muets +
- `LB8` — P2 : la sélection est élaguée contre la liste FILTRÉE
- `LB9` — Colonnes : en-têtes riches épinglés, régions nommées, colonne vide = zone de
- `LB10` — Repli de colonne persisté (rail droppable)
- `LB11` — autoScroll dnd-kit réglé + drag-to-pan sur l'espace vide
- `LB12` — Restauration du focus après un drop (souris ET clavier)
- `LB13` — Anatomie 4 zones de la carte (le visage du pipeline)
- `LB14` — Rampe « rotting » sur la carte (réutilise workspace/rotting.js TEL QUEL)
- `LB15` — Menu ••• au hover + PerduPopover PARTAGÉ (fin de la triplication)
- `LB16` — Tags tokenisés + token WhatsApp + tokens morts réparés
- `LB17` — Tactile + PII : cibles 44px, swipe inerte, cadenas PII
- `LB18` — Scrolleur unique + thead sticky + colonne nom sticky + colgroup
- `LB19` — Choix de colonnes persisté (vol de useColumnPrefs + ColumnManager)
- `LB20` — Option « Par étape » (groupes repliables avec agrégats)
- `LB21` — Lignes ouvrables au clavier + adoption du PerduPopover partagé
- `LB22` — URL partageable : ?view= + filtres dans l'URL (module pur)
- `LB23` — Recherche débouncée
- `LB24` — Bandeau KPI = filtres (le cockpit du matin)
- `LB25` — Barre bulk FLOTTANTE
- `LB26` — Vues enregistrées : « Copier le lien » + Express dans le ⋯ mobile
- `LB27` — Squelette EN FORME dans le shell
- `LB28` — Prévision : parité complète avec le kanban
- `LB29` — Calendrier + Carte : tons d'étape, aujourd'hui, hover câblé
- `LB30` — Graphique : cache session des insights + cohérence filtres
- `LB31` — Chasse aux hex des surfaces leads (dark mode honnête)
- `LB32` — Dédup useIsMobile + ViewSwitcher sur Segmented
- `LB33` — Spec de régression « le board tient dans l'écran »
- `LB34` — Goldens leads-kanban clair+sombre + passe axe finale (clôture du batch)
- `LB35` — ForecastView : parité des trois invariants du kanban
- `LB36` — FilterBar : « Effacer les filtres » pendant le débounce ressuscite la recherche
- `LB37` — Commentaires menteurs survivants (blueprint I2)
- `LB38` — `selectionActive` déclaré mais jamais câblé (comportement D3 différé en silence)
- `LB39` — Pré-existant (pas de cette branche) : l'undo VX95 de changement d'étape est mort en…
- `LB40` — Cosmétique : barre bulk vs scrollbar du board + infobulles ViewSwitcher
- `LB41` — Kanban : UN SEUL scroll vertical pour toutes les étapes (patron Odoo, retour fondateur)
- `LB42` — Kanban mobile : le patron Odoo exact (une étape à la fois, snap, colonne = scrolleur)
- `LB43` — Ligne de contrôle UNIQUE façon Odoo (recherche + facettes + « Filtres ▾ » + ⋯)
- `LB44` — Fenêtre lead mobile : le scroll-spy suit enfin le défilement (parité desktop)
- `LB46` — Desktop : TOUT le cockpit en UNE ligne large
- `LB47` — Phone : UNE seule ligne de chrome, le reste dans ⋯ (+ LB45 absorbé)
- `LB48` — Backend : vues enregistrées DE COMPTE (crm.SavedView)
- `LB49` — Frontend : vues liées au compte — défaut rang 1 à chaque connexion, continuité de…
- `LB50` — LeadViewPicker : le titre DEVIENT le sélecteur de vues (patron Twenty/Attio)
- `LB51` — QuickFilterChips : tuiles KPI + chips fréquentes = UN jeu de chips comptées
- `LB52` — Rangée facettes CONDITIONNELLE (Twenty ViewBarDetails / Linear)
- `LB53` — Contrat CSS chrome : ~52px au repos
- `LB54` — Mobile : `[picker|🔍|Filtres|⋯]`
- `LB55` — E2E/pins : « Pipeline » heading→button, tuiles→chips
- `LB56` — Régression continuité : URL > session > rang 1 > kanban
- `LW1` — P0 : le corps de la fiche lead ne scrolle pas, le bouton Enregistrer est hors…
- `LW2` — Perte de données #1 : le raccourci d'étape 1-4 « blanchit » les éditions non sauvées
- `LW3` — Perte de données #2/#4 + fuites inter-leads : l'état satellite survit à la navigation…
- `LW4` — Perte de données #3 : « a »/bouton Archiver jettent les éditions sans confirmation +…
- `LW5` — SigneDialog : la date « futur » se trompe d'un jour le soir (UTC vs local)
- `LW6` — AppointmentBooker : annulation avalée en silence + `<form>` imbriqué invalide
- `LW7` — normalizeMaPhone accepte n'importe quoi → bouton WhatsApp armé sur un numéro invalide
- `LW8` — Backend : N+1 confirmé sur le chatter `historique/`
- `LW9` — Le moteur d'état `useLeadDraft` : la perte de données devient structurellement…
- `LW10` — Le shell `LeadWorkspace` : une fenêtre, deux enveloppes, le scroll juste par…
- `LW11` — Le centre : `SectionsPane` + les 6 fichiers de sections, port 1:1 des champs
- `LW12` — Mode création : le formulaire rapide, défauts intelligents et « créer un autre » intacts
- `LW13` — Bascule des appelants : la liste, la route détail, et LeadForm devient un adaptateur
- `LW14` — `IdentityRail` : l'identité, la préparation et les actions à demeure
- `LW15` — La triade obligatoire : responsable · prochaine action · relance, toujours visibles et…
- `LW16` — `StageControl` : l'étape en StatusPill, le pourrissement visible, SIGNED toujours gardé
- `LW17` — Le score s'explique enfin : popover `score_reasons` + réutilisation `scoreTooltip`
- `LW18` — Bannières intelligentes : doublons, « déjà client ? », carte collée
- `LW19` — `ContextRail` : les onglets Historique · Devis · Activités · Pièces avec compteurs
- `LW20` — `TimelineTab` : le chatter devient le fil unique — filtre, épingle, parcours multi-touch
- `LW21` — `DevisTab` : la chaîne document en cartes + le CTA devis-auto qui dit ce qui manque
- `LW22` — WhatsApp dans DevisTab : multi-sélection, FR/Darija, aperçu — l'état dans le moteur
- `LW23` — Raccourcis propres : registre exact, « d » réparé, « n » pour noter
- `LW24` — Pré-chargement des voisins de file : J/K devient instantané
- `LW25` — Skeleton à l'ouverture, en forme de la vraie grille
- `LW26` — Palette de commandes : la fiche courante répond à ⌘K
- `LW27` — Chatter : +11 champs métier journalisés (`TRACKED_FIELDS`)
- `LW28` — `LeadActivity.pinned` : la note épinglée (UNE migration additive)
- `LW29` — `pii_masked` : le masquage PII devient VISIBLE au lieu de silencieux
- `LW30` — `chatter_recent` embarqué sur le GET détail : l'ouverture passe de 4 requêtes à 3
- `LW31` — Zéro hex dans la fenêtre : migration Badge/tokens des chips et bannières héritées
- `LW32` — Un seul Avatar : `components/Avatar.jsx` → `ui/Avatar` + `--owner-color-1..10` sur les…
- `LW33` — Rythme, motion et micro-feedback : la passe « premium » du workspace
- `LW34` — Responsive vrai : 768-1024 à 2 colonnes + Sheet contexte, mobile à barre-pouce
- `LW35` — A11y : la nav de sections et les onglets parlent aux lecteurs d'écran
- `LW36` — AppointmentBooker sort de l'inline : 30 blocs de styles → tokens/kit
- `LW37` — Le harnais unitaire migre vers LeadWorkspace : les 12 suites existantes + le moteur…
- `LW38` — Régression anti-perte : la suite « rafale » qui verrouille les 4 bugs pour toujours
- `LW39` — Contrat e2e mis à jour DÉLIBÉRÉMENT : la spec et le DOM bougent dans la même tâche
- `LW40` — Démontage final : LeadForm.jsx supprimé, cartes et plans à jour
- `LW41` — `chatter_recent` doit vraiment économiser la requête historique
- `LW42` — `changeStage` : messages d'erreur fidèles
...
- `VAO1` — PRÉ-REQUIS D'INSERTION (fait dans le commit qui ajoute ce groupe, jamais en lane) …
...
- `PACT148` — Atteignabilité exigée sur les 120 tâches §E — l'amendement qui empêche le 5ᵉ rattrapage
