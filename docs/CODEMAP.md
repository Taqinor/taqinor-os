...
The Odoo Portal equivalent: a client logs in to see their own devis/factures/chantiers/tickets. Originally built inside `compta`; **ODX12** moved the models here with `db_table = 'compta_<model>'` (`SeparateDatabaseAndState`, state-only — no data moved) + a re-export shim left in `apps/compta/models.py`. Links to `crm.Client`/devis/factures/chantiers/tickets are STRING-FK or opaque id only (never a `models` import of ventes/crm/sav/installations). Auth surface (portal tokens/accounts) preserved byte-identical — no access widened by the split.
- **Models** — `ComptePortailClient` (tokenized client access account, reuses the client's email — no 2nd identity copy per DC32), `AcceptationDevisPortail` (quote e-acceptance from the portal), `PaiementFacturePortail` (self-service invoice payment record), `DocumentClientPortail` (client-visible document refs), `JalonChantierPortail` (site-progress milestones surfaced to the client), `DemandeTicketPortail` (SAV ticket requests raised from the portal).
