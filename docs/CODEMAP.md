...
Structure fingerprint: d4979d76ef5e556a680f1aaabc60d9ce1ecd64fef5c4dd3e7f0c7778a2008bc1
Plan fingerprint: aeb64c54d7df757a93a7e4eba7f25d325aa98c7112464933366330ae1db1e3c3
...
└── frontend/                     React/Vite SPA
    └── src/
        ...
        ├── features/               Redux slices + domain logic per area (see §5); features/adsengine/ = ads-engine console (module.config.jsx-registered, 12 screens under `/publicite`)
...
- **SCA35** adopts the `core.documents` kit's TRANSITIONS read-contract on `Contrat` WITHOUT inheriting `DocumentMetier` itself: `Contrat` keeps its own `company` FK (historical `related_name='contrats'`) and `date_creation` rather than switching to `TenantModel`'s generic accessor (no functional gain, would break the historical accessor); a read-only `TRANSITIONS`/`transitions_permises` property exposes the EXISTING `machine_etats._transitions()` state graph (CONTRAT12) in the kit's expected shape — `services.changer_statut` (the `machine_etats.changer_statut` alias) stays the one write path, still enforcing the "≥2 parties" guard the generic kit doesn't know about. **ARC14** adds `Contrat.custom_data` (JSON), the first `customfields` pilot alongside `flotte.Vehicule`. **ARC28/33/40** add `apps/contrats/platform.py` (`agent_actions_module` — read-only list + `contrat_voir`; `kpi_providers`). **NTSUB1-4** add a recurring-revenue catalogue foundation, all `TenantModel`-scoped: **PlanAbonnement** (sellable offer, `code`/`nom`/`prix_base`/`engagement_mois` + `plan_recurrent` FK to the existing ZCTR1 cadence — distinct from `Contrat.montant`; `Contrat.plan_abonnement` FK, nullable, snapshots montant/plan_recurrent onto the contract at creation via `services.appliquer_plan_abonnement`, never a live link), **AddOnAbonnement** + **AbonnementAddOnLigne** (paid add-ons attached to a contrat/`sav.ContratMaintenance` via typed `(type_cible, cible_id)`, patron `CycleFacturationLog`; active add-ons add their amount into `services.facturer_ligne_echeance`'s generated Facture total), **PalierUsage** (tiered/volume usage pricing, addon XOR plan_abonnement) backed by the pure `core/pricing_paliers.py` `calculer_prix_paliers(usage, paliers, mode)` (volume/graduated, stdlib-only, reusable by `apps.sav`'s XCTR16 — not yet wired there), **CompteurUsage** (generic metering ingestion, idempotent `update_or_create` on `(company, type_cible, cible_id, code_compteur, periode_debut, periode_fin)`, endpoint `compteurs-usage/` + `compteurs-usage/import-csv/` [NTSUB31, batch idempotent] + `plans-abonnement/export/` [NTSUB21, 3-sheet xlsx]). **NTSUB5** adds **EssaiAbonnement** (trial: freezes billing until `date_fin_essai`, `services.demarrer_essai_contrat`/`convertir_essais_expires` — activates unless the contrat is résilié, J-3 notification) + beat `contrats.convertir_essais_expires_daily`. **NTSUB8** adds **SequenceDunning** + **EtapeDunning** (canal email/whatsapp/notification interne, `jour_offset`, `declenche_suspension`) + **EtapeDunningLog** (idempotence `(contrat, etape)`); `Contrat.sequence_dunning` FK (NULL = ZCTR2 unchanged); `services.executer_dunning_contrat/company` play due steps by days-overdue and the flagged step suspends via the extracted `_appliquer_suspension_impaye` (single ZCTR2 suspension path); viewsets `sequences-dunning/` + `etapes-dunning/`; beat `contrats.executer_dunning_daily`. **NTSUB7** adds `contrats/{id}/changer-plan/` (`services.changer_plan_contrat`: delta Avenant + plan snapshot, immediate-upgrade prorata XCTR6, no downgrade avoir). **NTSUB12** adds `contrats/metriques-saas/` (`selectors.arr_bridge`/`quick_ratio`/`rule_of_40` — MRR×12, div-by-zero guarded, marge via `compta.selectors`). **NTSUB33** journalizes add-on attach/detach to the contrat chatter; **NTSUB34** notifies the responsable on trial conversion / plan change / dunning entry. Pure `core/pricing_paliers.py` (FG364) backs tiered usage pricing.
...
- **adsengine** — « Publicité » console for the Meta Ads engine (§4), a `module.config.jsx`-registered coquille (not a Redux slice — `hooks.js`/`adsengineApi.js` + `adsengine.js` label maps) under `/publicite`, gated responsable/admin: DashboardScreen (+ Pacing/Reconciliation tabs), ConnectionScreen, CampaignsScreen, ApprovalsScreen (the flagship propose→approve box), BriefScreen, CreativeLibraryScreen, ActionsLogScreen, ExperimentsScreen, FlightPlanScreen, BacklogScreen, RulesScreen, SimulationScreen, ReportsScreen; `TenantBrand.jsx` (white-label-clean fallback, no hardcoded brand — SCA29); DOM test hooks prefixed `ae-*`.
