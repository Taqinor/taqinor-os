...
Generated from commit `HEAD` on 2026-06-19 (refinement-queue batch — additive structural surface: `stock.Categorie.type_equipement` (migration stock/0020) + per-company serial UniqueConstraint on `sav.Equipement` (migration sav/0006); new screens/routes `/ventes/paiements` (Encaissements), `/stock/categories`, `/stock/fournisseurs`, `/stock/retours-fournisseur`; new endpoints reporting balance-âgée xlsx export + ventes numerotation-preview + records attachment-phase PATCH + parametres message reset; shared xlsx builder `apps/records/xlsx.py`; read-only serializer fields on stock/ventes/sav. Prior: refinement batch wave 2 — crm.Client.created_by, ventes.FactureActivity, export-comptable).
Structure fingerprint: e6360327c91f0ab3ab38fba73261a31f1d0f667d0f910b64a7e1ee21d6b2e2ee
Plan fingerprint: cac6db2f38e41901838dc1a4add92b4647b44077f4b70b00f16d51ed8bb7d989
...
cross-checked against `main`; completed tasks are archived verbatim in `docs/DONE.md`, so a fully-drained queue shows 0 done here. Generated from commit `HEAD` on 2026-06-19 (plan-status refresh: F24 shipped — the field-execution/outillage module documentation task; the REFINEMENT QUEUE's ~114 ROUTINE tasks are id-less and outside this fingerprint). This
...
**Totals: 68 tasks — 56 done · 0 open · 12 blocked.**
...
- `F24` — (SHIPPED 2026-06-19 — voir DONE LOG) After F1–F23 are built,
