...
Structure fingerprint: 41b993bc1d41bc1b6cceb82566bd3da132bef27349b6c181e300d0bfb863ae00
Plan fingerprint: 68470e5de2d741af0c33dfb3fa0b7f7d4a2188509a7782865e44d8bf9e39d1a2
...
- **Models** — `MetaConnection` (per-company Meta creds/ad-account, encrypted at rest), `GuardrailConfig` (spend/CPA/frequency ceilings gating every write), `EngineAction` (the propose→approve→apply spine — every mutating step is a row here first), `EngineAlert` (guardian-detected anomalies), `CreativeAsset` + `CreativePolicy` (ad creative library + compliance rules), `AdCampaignMirror`/`AdSetMirror`/`AdMirror` (read-mirror of live Meta objects), `InsightSnapshot` (daily performance pull), `WeeklyBrief` (auto-generated FR ops summary), `Experiment`/`ExperimentArm`/`ArmDailyStat`/`DecisionLog` (A/B test + bandit decision trail), `RulePolicy`/`AnomalyEvent`/`PacingState` (rules-engine + budget pacing), `CreativeGenerationBatch`/`CreativeBacklogItem` (creative production queue), `FlightPlan`/`FlightPhase` (launch sequencing + preflight), `ReconciliationSnapshot` (Meta-vs-ERP spend/lead reconciliation).
- **Key modules** — `meta_client.py` (the PAUSED-only Meta Marketing API client); `services.py` (the propose→approve→apply `EngineAction` spine, guardrail-gated dispatch); `guardrails.py`; decision engine (`bandit.py` Thompson beta-binomial, `allocation.py`, `rewards.py`, `authority.py`, `mde.py`); guardian (`rules_engine.py`, `anomaly.py`, `rule_templates.py`, `watchdog.py`, `alerts.py`); treasury (`pacing.py`, `budget_applier.py`, `identity.py`, `launch_templates.py`); creative (`rotation.py`, `recombine.py`, `backlog.py`, `flightplan.py`, `dco.py`, `creative_factory.py`); attribution (`reconciliation.py`, `capi_crm.py`, `reporting.py`, `attribution.py`, `metrics.py`); runner (`flightrunner.py` state machine, `simulator.py`, `preflight.py`); `platforms/` (the `AdsPlatform` interface — ADSENG48/49, Google/Snapchat/TikTok adapters GATED behind founder budget/product decisions).
- Endpoints (`/api/django/adsengine/`): `status/`, `metrics/cout-par-signature/`, `wiring-health/`, `reporting/variantes|entonnoir|cohortes|export/`; ViewSets `connexions/`, `garde-fous/`, `actions/`, `alertes/`, `creatifs/`, `policy-creative/`, `experiences/`, `bras/`, `stats-bras/`, `decisions/`, `regles/`, `anomalies/`, `pacing/`, `lots-creatifs/`, `backlog-creatif/`, `plans-vol/`, `phases-vol/`, `reconciliations/`.
