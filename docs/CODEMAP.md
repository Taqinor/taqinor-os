...
Generated from commit `3c2a979` on 2026-06-17.
...
- **TicketActivity** — ticket chatter. **ContratMaintenance** — preventive contracts (`periodicite`, `date_debut`, `derniere_visite`, `actif`).
...
- Endpoints (`/api/django/reporting/`): `dashboard/`, `search/`, `notifications/`, `pipeline/` (funnel value by STAGES, weighted forecast), `reports/sales|stock|service/` (+`?export=xlsx`), `insights/recurring-revenue|audit-log|job-costing|analytics/`, `archive/client/{id}/` and `archive/chantier/{id}/`. `job-costing` (margin via internal `prix_achat`) is admin-only.

### parametres — company profile, business settings, WhatsApp templates
- **CompanyProfile** (one per company) — identity + Moroccan legal IDs (`ice`, `identifiant_fiscal`, `rc`, `patente`, `cnss`, `rib`); branding (`logo_key`, `signature_key`, `couleur_principale`); `responsable_defaut_leads` FK (default lead owner); quote-gen knobs (`payment_terms` JSON, `quote_validity_days`, `tva_standard/panneaux`, ROI constants `onee_tarif_kwh`/`productible_kwh_kwc`/`rendement_global`, `remise_max_pct`, `discount_approval_threshold`, `agricole_pump_hours`); `doc_prefixes`/`doc_numbering` JSON.
...
- **crm/** — ClientList, LeadForm, LeadsPage + `leads/` (ViewSwitcher, FilterBar, BulkActionBar, DoublonsPanel, SigneDialog, views/Kanban|List|Calendar|Charts).
...
- **admin/** — UsersManagement, RolesManagement. **parametres/** — ParametresEntreprise. **activities/** — MesActivitesPage. **ia/** — AgentChat, OcrUpload. Top-level: Dashboard, Landing, Login, Reporting, Rapports.
...
- **CI status checks that gate a merge** — `.github/workflows/ci.yml` defines
  **five** jobs: `backend-lint` (flake8), `backend-tests` (Postgres+pgvector +
  ...
  (`scripts/check_stages.py`), and `web-build-test` (apps/web astro build +
...
- **Provenance window.** Generated from `main` at commit `3c2a979`. Work merged
