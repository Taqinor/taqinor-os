...
Roof design studio (`/api/django/calepinage/`). Models: `Calepinage`, `CalepinageVariante`, `CalepinageVersion`, `ReleveTerrain`, `PhotoSite`, `PoseReelle`, `DossierReglementaire`, plus two linked registries (`ParametreSimulation` from `apps.parametres`, `PresetCalepinage` from `apps.stock`). Pure engines in `core/calepinage/` (io, stateless pose/DP) and `core/electrique/` (pure topology layer) are imports by both this app and `apps.ao/` (pre-SOLMVP15). **Form of URL (CAL233):** business object under `calepinages/<pk>/…` (sub-resources as `@action`), company settings under `parametres/…`, stateless engine under `moteur/…`. Services: `simulation.py::simuler_calepinage` (CALX5), `chaine_pertes.py` (CALX147), `pvgis_serie.py::ClientPvgis` (PVGIS v5_3, CALX150), `ombrage_chaines.py`, `agregation_electrique.py`, `rapport/` (documents hub), plus domain-specific (`pompage.py`, `consommation.py`, `batterie.py`, `hors_reseau.py`, `economie.py` — the last one is READ-ONLY, wrapping `apps.ventes.economie`). Contracts: `roof_layout_v2.schema.json` (document persistence), `parametres_calepinage.json` (settings registry), `calepinage_simulation.json` (result fusion), `calepinage_pertes_cascade.json`, `calepinage_meteo.json`, plus electrical schema samples. No price keys ever leave the generator (D-CALX 5).
