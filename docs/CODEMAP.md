...
Structure fingerprint: 29fa7c5ca3d9627f20c6490783e8fb04481ea9272887bc0413b307033a1bd832
Plan fingerprint: 760196e3064349fdf8e906c0cbd98b01d84fb48fdbfc14b017da3adcdad23357
...
- `WOW1` — Enable `--parallel 4` on the CI backend test run
- `WOW2` — Fast password hasher under tests
- `WOW3` — Remove the dead coverage tax from the merge gate
- `WOW4` — Cache Python deps in CI like npm already is
- `WOW5` — Make the slow/pdf tier REAL
- `WOW6` — Shard backend-tests 4× (AFTER WOW1 is proven green twice)
- `WOW7` — Deduplicate the tripled CI environment setup
- `WOW8` — (STRETCH — only if after WOW1-6 the gate is still > 30 min) Cached pre-migrated test…
- `WOW9` — Commit `scripts/test-backend.ps1` (+ `.sh` twin): the ONE way to run backend tests…
- `WOW10` — Commit `scripts/probe_queries.py`: the query-count probe that pinpointed every N+1 on…
- `WOW11` — Shift validation left in the plan-run itself: amend `docs/PLAN.md` HOW-TO-RUN (and the…
- `WOW12` — Write `docs/TESTING.md` — the test-authoring contract every lane agent must follow (≤…
- `WOW13` — Convert the top-10 heaviest per-test `setUp` files to class-scoped `setUpTestData`…
- `WOW14` — Introduce `backend/django_core/core/factories.py` (homegrown, NO new dependency)…
- `WOW15` — Fix the parallel-unsafe tests (prereq of WOW1): convert…
- `WOW16` — Run "clean the plans" NOW (archive the 223 inline `[x]` tasks + fold DONE-LOG bodies…
- `WOW17` — Hoist the ~170-line HOW-TO-RUN/STANDING-RULES preamble into `docs/PLAN_HOWTO.md`…
- `WOW18` — Commit `scripts/land.ps1` — the end-of-run landing sequence, automated in order: `git…
- `WOW19` — Harden `scripts/plan_lanes.py` against silent task loss: (a) emit a WARNING for any…
- `WOW20` — Actually set the model floor CLAUDE.md already documents: add to…
- `WOW21` — Reconcile CLAUDE.md's tooling claims with reality (
- `WOW22` — Recalibrate the MERGE FLOOR once the CI gate is ≤ 45 min (
- `WOW23` — Stop the mid-batch Dependabot tax: (a) configure `.github/dependabot.yml` to GROUP…
- `WOW24` — OneDrive as DISASTER-RECOVERY backup (GitHub remains the source of truth — decision…
- `WOW25` — Workspace GC — one-time + repeatable: commit `scripts/gc-workspace.ps1` that (a) `git…
- `WOW26` — Fix the deploy's recurring silent-partial-failure (
