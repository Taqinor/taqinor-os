...
- [ ] WN1 — **Blog reads as brand-new.** `content/blog/bienvenue-blog-taqinor.md` ("Vous lisez le premier…", "Bienvenue sur le blog / pourquoi nous ouvrons un blog") announces post #1 of a 1-post blog; `pages/blog/index.astro:86` renders "Les premiers articles arrivent très bientôt" as an empty-state. Reframe the welcome post as evergreen (no "first post"); never render a "coming soon" blog index. @files: apps/web/src/content/blog/bienvenue-blog-taqinor.md, apps/web/src/pages/blog/index.astro (+en/ar)
- [ ] WN2 — **Small-count headline stats.** `pages/realisations/index.astro` renders `villesCount` (≈3) as a monumental stat; low counts read tiny. Replace bare count badges with a qualitative city list. @files: apps/web/src/pages/realisations/index.astro (+en/ar)
- [ ] WN3 — **Garanties shows unfinished policy.** `garanties.astro` has "à confirmer — liste en cours de validation" and "ce plancher… pas encore fixé — en attente d'un engagement du fondateur". Use definitive standard-exclusions language / OMIT the unfinished production-guarantee section until the figure exists. @files: apps/web/src/pages/garanties.astro (+en/ar)
- [ ] WN4 — **Maintenance pricing/SLA shown "pending founder validation".** `maintenance-monitoring.astro` renders "en attente de validation du fondateur" (SLA + price ×3 tiers). Switch to contact-for-pricing; hide the pending fields. @files: apps/web/src/pages/maintenance-monitoring.astro (+en/ar)
- [ ] WN5 — **Parrainage published before its mechanic exists.** `parrainage.astro`: "montant/déclencheur… en cours de finalisation". Hold or reframe as "récompense sur mesure". @files: apps/web/src/pages/parrainage.astro (+en/ar)
- [ ] WN6 — **"Avis clients — bientôt disponibles" empty-state.** `StarRating.astro` renders a "coming soon" reviews placeholder wherever `GOOGLE_RATING` is null. OMIT the component until real reviews exist. @files: apps/web/src/components/StarRating.astro
- [ ] WN7 — **"suivi depuis plus d'un an" vs a weeks/months-old fleet.** `garanties.astro:248` vs realisations.ts — reconcile before using either claim (founder to confirm). @files: apps/web/src/pages/garanties.astro
- [ ] WN8 — **V2H "à venir / rares" dwell.** `recharge-voiture-electrique-solaire.astro` dwells on V2H/V2G being "à venir… rares". Keep honesty but shorten. @files: apps/web/src/pages/recharge-voiture-electrique-solaire.astro (+en/ar)
