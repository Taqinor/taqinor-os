...
- [x] WJ130 — **buildIdempotencyKey en CSPRNG** : `apps/web/src/lib/lead.ts:1227` génère la clé d'idempotence (seul credential du lookup public `lead-ref` qui renvoie `client_ref` avec le nom de famille) via `Math.random()` — passer à `crypto.getRandomValues` (les 3 appelants de mon-toit inchangés, la signature garde le paramètre injectable pour les tests). Constat de l'audit CRM L3 du 02/09 (vérifié 2×), coupé du groupe CRX car hors contrat d'apps (site web). Files: `apps/web/src/lib/lead.ts`.
...
- [x] QJW17 — **`restaurer` cesse d'aplatir le bloc « total avec » à chaque chargement de page** : `src/lib/proposition/liaisons.ts:248` déclare `totalAvec` avec la capture par défaut `texte` sur `[data-detail-eco-total-avec-bloc]`, or ce nœud (`pages/proposition/[...token].astro:3648-3651`) **contient deux `<span>` enfants** — l'étiquette `data-i18n` et le montant stylé `dir="ltr"` ; `swap.reposer` (`swap.ts:95`) fait `el.textContent = orig.contenu`, ce qui les **détruit**, et `restaurer` tourne **au chargement** via `appliquer()` (`[...token].astro:8669`) → `chargerDetail()` → `restaurerDetail()` (`:8510`). Conséquence corrigée par R3 — le défaut est **pire que décrit** et vaut HAUT : le client voit un texte **FR+EN+AR mélangé dès le premier affichage** (le crochet de traduction ayant disparu), et le sélecteur de langue ne pilote plus ce bloc. La garantie « identité byte-à-byte après un aller-retour » annoncée à `[...token].astro:8418` et `liaisons.ts:29-30` est donc fausse (origine: fresh-bugs-3, medium→**HAUT** par R3). Capturer/restaurer en **préservant les nœuds enfants** (capture de type « fragment », pas `texte`), et faire que le crochet i18n survive à l'aller-retour. **Done =** test ROUGE d'abord : aller-retour capture/restauration sur `[data-detail-eco-total-avec-bloc]` → les deux `<span>` disparaissent ; vert après = DOM identique à l'octet, `data-i18n` intact, bascule FR/EN/AR pilotant encore le bloc après restauration ; `npm run build` vert. Files: `apps/web/src/lib/proposition/liaisons.ts`, `apps/web/src/lib/proposition/swap.ts`, `apps/web/tests/propositionLiaisons.test.ts`. (ROUTINE) (@lane: qjw/proposition-contrevisite) (@model: opus)
- [x] QJW18 — **Le pourcentage DANS le donut se repeint quand le client charge une taille Eco/Max** : `src/lib/proposition/liaisons.ts:328-335` — la liaison de couverture déplace bien l'arc (`stroke-dasharray` recalculé sur `data-detail-donut-r`) mais le nombre au centre passe par `const val = unHtml(noeuds, 'valeur')`, et `unHtml` **renvoie `null` pour le nœud `<text>` SVG** : la peinture est silencieusement sautée, donc **l'arc décrit la nouvelle taille pendant que le chiffre garde le pourcentage du devis officiel** — deux chiffres contradictoires dans la même figure, sous les yeux du client (origine: fresh-bugs-2, medium confirmé sans réserve par R3). Étendre `unHtml` / la liaison au texte SVG (le `<text>` SVG n'a pas d'`innerHTML` : passer par `textContent`), sans changer le format d'affichage. **Done =** test ROUGE d'abord : chargement d'une taille Eco → arc à jour, chiffre inchangé ; vert après = arc et chiffre décrivent la même taille ; un cas où la donnée manque OMET le chiffre au lieu d'en peindre un faux ; `npm run build` vert. Files: `apps/web/src/lib/proposition/liaisons.ts`, `apps/web/tests/propositionLiaisons.test.ts`. (ROUTINE) (@lane: qjw/proposition-contrevisite) (@model: opus) (@after: QJW17)
- [x] QJW19 — **Une réponse de détail en retard ne déclare plus « chargé » une section encore en vol** : `src/pages/proposition/[...token].astro:8535` — une requête de détail qui revient tard efface `aria-busy` sur des chapitres qu'une requête **plus récente et toujours en vol** a légitimement marqués occupés ; ces chapitres restent **masqués**, si bien que la technologie d'assistance annonce une région stabilisée alors qu'elle est vide et encore en chargement (origine: fresh-bugs-13, medium confirmé sans réserve par R3). Attacher l'effacement d'`aria-busy` à l'identité de la requête qui l'a posé (jeton de requête), comme le fait déjà la garde anti-réponse-périmée du reste de la page. **Done =** test ROUGE d'abord : requête A lente + requête B rapide → à l'arrivée de A, `aria-busy` retiré alors que B est en vol ; vert après = seul le retour de la requête courante l'efface, les chapitres masqués redeviennent visibles au bon moment ; `npm run build` vert. Files: `apps/web/src/pages/proposition/[...token].astro`, `apps/web/tests/propositionAriaBusy.test.ts`. (ROUTINE) (@lane: qjw/proposition-contrevisite) (@model: sonnet) (@after: QJW18)
- [x] QJW20 — **`proposal_data` reçoit sa table de couverture des FEUILLES, comme `taille_detail`** : `apps/web/tests/propositionPayload.test.ts:125` porte bien une table `CLES_LUES` (« la garde qui justifie tout le module »), mais c'est une table des clés que la page **lit**, pas une table de **couverture des feuilles** : une clé ajoutée à `proposal_data` — la principale charge utile client de la page proposition — peut donc arriver **sans qu'aucun test ne force une décision** à son sujet (lue, ou refusée avec une raison écrite). `taille_detail` a cette machinerie depuis QJW10 et c'est la deuxième meilleure du dépôt ; `proposal_data` est le seul surface non gardée **des deux côtés à la fois** (origine: addition-walkthrough-8, confirmé par R3). Construire la table par énumération des **feuilles** de l'échantillon de contrat commité **`apps/ventes/contract_samples/proposal_data.json`** — le même fichier auquel s'apparie la moitié serveur (QJR228 dans `PLAN2.md`), chaque moitié tournant seule. **Done =** une feuille ajoutée à `proposal_data.json` et non déclarée (ni lue, ni refusée avec raison) fait ROUGIR le test (cas négatif exécuté) ; les refus actuels écrits un par un ; suite web verte. Files: `apps/web/tests/propositionPayload.test.ts`. (ROUTINE) (@lane: qjw/proposition-contrevisite) (@model: sonnet)
- [x] QJW21 — **La garde de parité du tunnel passe par le `lireEtatTunnel` de CHAQUE page** : `apps/web/tests/tunnelParite.test.ts:108-125` construit un `etatComplet()` **à la main** puis appelle `construireCorps(etat, …)` pour les trois locales : la garde compare donc trois fois la MÊME lecture et **ne peut pas voir** la seule omission qu'elle existe pour attraper — une locale dont la page ne LIT jamais la nouvelle question reste verte (origine: addition-walkthrough-2, HAUT confirmé). Faire partir chaque locale de son propre `lireEtatTunnel()` (le lecteur réel de sa page, `mon-toit.astro` fr/en/ar) sur un DOM peuplé, puis comparer les corps émis. **Done =** test négatif EXÉCUTÉ : un champ retiré du seul DOM anglais fait rougir la garde (aujourd'hui elle reste verte) ; les 12 tests de locale existants toujours verts ; `npm run build` vert après chaque `.astro` touché. Files: `apps/web/tests/tunnelParite.test.ts`, `apps/web/src/pages/devis/mon-toit.astro`, `apps/web/src/pages/en/devis/mon-toit.astro`, `apps/web/src/pages/ar/devis/mon-toit.astro`. (ROUTINE) (@lane: qjw/tunnel-contrevisite) (@model: opus)
- [x] QJW22 — **La liste blanche de `lead.ts` est appariée au registre du tunnel, et le test porte sur le corps APRÈS liste blanche** : `src/pages/api/capture-lead.ts:71` — un champ correctement déclaré dans le registre du tunnel est **silencieusement jeté avant le webhook** s'il n'a pas AUSSI été ajouté à la main à la liste blanche de `lib/lead.ts`, et les tests de parité assertent sur le corps **d'AVANT** la liste blanche : ils ne peuvent structurellement pas le voir. R3 abaisse à **medium** — les 69 clés du registre sont toutes traitées aujourd'hui, c'est donc un **piège de maintenance latent**, pas une perte de données en production (origine: addition-walkthrough-4). Écrire la garde de parité registre ↔ liste blanche, et **déplacer l'assertion sur le corps POST-liste-blanche** (celui qui part réellement au webhook). **Done =** test négatif EXÉCUTÉ : une clé ajoutée au registre et absente de la liste blanche fait rougir la garde (aujourd'hui verte) ; les assertions de parité portent sur le corps réellement émis ; suite web verte. Files: `apps/web/src/lib/lead.ts`, `apps/web/src/pages/api/capture-lead.ts`, `apps/web/tests/tunnelParite.test.ts`. (ROUTINE) (@lane: qjw/tunnel-contrevisite) (@model: sonnet) (@after: QJW21)
- [x] QJW23 — **Une passe de vérification de types couvre enfin les scripts `.astro`** : `apps/web/tsconfig.check.json:14` — **aucune vérification de types ne tourne sur les scripts des pages `.astro`**, si bien que le contrat typé entre le registre du tunnel (`src/lib/tunnel/champs.ts`) et les trois pages n'est appliqué qu'à l'intérieur de `src/lib` : côté page, tout est du JavaScript non vérifié (origine: addition-walkthrough-3, medium confirmé). Ajouter la passe (`astro check` ou l'extension du périmètre `tsconfig.check.json`) et la brancher au gate web existant ; les erreurs préexistantes se traitent, ou se consignent une par une dans une liste gelée avec leur raison — **jamais un gate désarmé** pour passer. **Done =** la passe tourne dans le gate web et rougit sur une erreur de type introduite dans un script `.astro` (cas négatif exécuté) ; l'inventaire des erreurs préexistantes est soit vidé, soit gelé avec ses raisons ; `npm run build` vert. Files: `apps/web/tsconfig.check.json`, `apps/web/package.json`. (ROUTINE) (@lane: qjw/typecheck) (@model: sonnet) (@after: QJW21)

> **Suite du Groupe QJW — la moitié `apps/web` de la RONDE 4 du parcours devis (31/08/2026).** La ronde 4
> (29 confirmés / 5 partiels / 0 réfuté en bloc) a produit **deux** constats qui vivent sous `apps/web/**` ; tout
> le reste part dans `docs/PLAN2.md` sous le **Groupe QJR4**. Rapport complet :
> https://claude.ai/code/artifact/e18dd134-8974-4871-bdd4-3cf81f5361e2
>
> **MÊME VAGUE QUE LE GROUPE QJR4 — mais indépendantes de forme.** Ces deux tâches ne consomment aucun contrat
> ni aucun endpoint nouveau : **il n'est PAS requis de vérifier qu'un merge QJR4 est sur `origin/main` avant de
> démarrer**, et aucune ne porte de `@after` vers une tâche `PLAN2.md` (un `@after` inter-fichiers n'est pas
> lisible par `plan_lanes.py`). Ce qui est requis est calendaire : **les deux moitiés doivent atterrir dans la
> même période** — QJW24 épingle la parité avec la moitié ERP QJR405, QJW25 est la moitié web de QJR416 (la
> primitive d'adresse IP), et une preuve légale de signature réparée côté ERP ne vaut rien si le proxy web ne
> transmet toujours pas l'IP du signataire.
>
> **Périmètre :** ces tâches n'éditent QUE `apps/web/**`. `npm run build` dans `apps/web` après chaque `.astro`
> touché ; la suite vitest existante reste verte avant de passer à la suite. Le site **se déploie tout seul via
> Cloudflare au merge** — ne jamais lancer de commande de déploiement ni `wrangler`.
>
> **Vérifié avant rédaction (à ne pas re-chercher) :** `apps/web` **n'a AUCUN jumeau de la salle de vente CRM**
> (`grep salle-vente / SalleVente / salle_vente` sous `apps/web` : zéro occurrence) — la page publique
> `src/pages/proposition/[...token].astro` est une **autre** fonctionnalité (la proposition client en ligne). Et
> la correction d'affichage post-signature DR1 se fait **à la source** côté ERP (`display_totals`), si bien que
> la page proposition en hérite **sans modification web**. Il n'y a donc pas de troisième tâche web : les deux
> ci-dessous sont exhaustives.

- [x] QJW24 — **Le miroir « facture → kWh » entre le site et l'ERP est ÉPINGLÉ par un test, au lieu d'être seulement promis par un commentaire** : `billToAnnualKwh` (`apps/web/src/lib/estimatorBrainV2.ts:807`, `export function billToAnnualKwh(monthlyBillMad: number, grid: TariffGrid = REGIE_TARIFF): number`) inverse la facture par **bissection** de `billMAD()` (`estimatorBrainV2.ts:345`, barème à tranches progressives puis sélectives), et non par une division plate ; il est consommé par `estimateFromBill()` (`src/lib/billEstimate.ts:165`). Côté ERP, `kwh_from_bill` (`backend/django_core/apps/ventes/quote_engine/pricing.py:469`) délègue sa branche à table sélective à `_kwh_from_bill_bisect` (`pricing.py:429`), dont les docstrings (`:430-432` et `:487-489`) déclarent noir sur blanc être un **« miroir EXACT »** de la fonction web. **Vérification faite pour cette tâche : les deux implémentations concordent aujourd'hui** — même bissection sur 60 itérations, même recherche par doublement de la borne haute. Le défaut n'est donc PAS une divergence de formule (ne pas en « corriger » une) : c'est qu'**une relation de miroir déclarée uniquement dans un commentaire n'est appliquée par rien**. Les deux fichiers vivent dans deux dépôts logiques, sont édités par deux sessions différentes, et le jour où l'un des deux barèmes bouge — un tarif ONEE, une tranche, une constante — **rien ne rougit** : le prospect lit un kWc sur le site et le commercial en lit un autre dans l'ERP, exactement la classe de défaut que QJW13 a déjà dû corriger une fois sur ce même site. Écrire la garde de parité : une table de points de contrôle (factures mensuelles couvrant **chaque tranche** du barème, plus les marches entre tranches et le cas nul) dont les valeurs de référence sont **dérivées du barème partagé** — jamais des nombres écrits à la main (règle zéro-chiffre-inventé). **Done =** cas négatif EXÉCUTÉ : modifier une tranche du barème d'un seul côté fait **rougir** la garde (aujourd'hui rien ne bouge) ; la garde passe sur les points de contrôle actuels sans qu'aucune valeur attendue n'ait été codée en dur ; si un écart réel apparaît, il est **rapporté et non « corrigé » au jugé** — la moitié ERP (QJR405 dans `docs/PLAN2.md`) tranche laquelle des deux formules fait foi ; suite vitest verte, `npm run build` vert. Files: `apps/web/tests/billInversionParite.test.ts`, `apps/web/src/lib/estimatorBrainV2.ts`. (ROUTINE) (@lane: qjw/parite-bareme) (@model: sonnet)

- [x] QJW25 — **Le proxy d'acceptation transmet enfin l'IP du signataire — c'est le seul des quatre relais du site à ne pas le faire** : `apps/web/src/pages/api/proposition-accept.ts` relaie l'acceptation d'une proposition vers l'ERP avec, pour tout en-tête, `headers: { 'content-type': 'application/json', accept: 'application/json' }` (appel `fetch` `:109-113`) — **ni `CF-Connecting-IP`, ni `X-Forwarded-For`**. L'ERP enregistre donc, comme **preuve légale de signature** (loi 53-05), l'adresse du proxy et non celle du signataire : le champ de preuve du registre immuable est vide de sens. Le motif correct existe **déjà trois fois dans ce même dépôt** et doit être repris tel quel, sans troisième formulation — `src/pages/api/visite.ts:86-99` : `headers: { 'content-type': …, accept: …, 'X-Webhook-Secret': secret, 'X-Webhook-Timestamp': …, ...(clientIp ? { 'X-Forwarded-For': clientIp, 'CF-Connecting-IP': clientIp } : {}) }`, motif également présent dans `src/pages/questionnaire/[token].astro:69` et `src/pages/proposition/[...token].astro:301`. `proposition-accept.ts` est le **seul dissident**, et c'est précisément le relais qui porte l'acte juridique. **Moitié ERP jumelle : QJR416 dans `docs/PLAN2.md`** — elle unifie la primitive de lecture d'IP côté serveur et lui fait prendre le **dernier saut de confiance** ; les deux moitiés sont indépendantes de forme (celle-ci peut partir immédiatement) mais **doivent atterrir dans la même période** : transmettre un en-tête que le serveur ne lit pas encore ne casse rien, et ne pas le transmettre laisse la correction serveur sans effet. **Done =** test ROUGE d'abord : l'appel sortant de `proposition-accept.ts` porte `CF-Connecting-IP` **et** `X-Forwarded-For` avec l'IP du client (aujourd'hui aucun des deux) — rouge, puis vert ; second test : quand l'IP cliente n'est pas déterminable, **aucun** des deux en-têtes n'est envoyé (jamais une valeur vide ni inventée, exactement comme le fait `visite.ts`) ; troisième test : le corps transmis et le reste des en-têtes sont **inchangés à l'octet** ; suite vitest verte, `npm run build` vert. Files: `apps/web/src/pages/api/proposition-accept.ts`, `apps/web/tests/propositionAcceptEnTetes.test.ts`. (ROUTINE) (@lane: qjw/proposition-ip) (@model: sonnet)
...
- 2026-09-06 — **WJ130/QJW24/QJW25 (lane web du drain AUD)** : clé d'idempotence en CSPRNG (crypto.getRandomValues, Math.random prouvé jamais appelé) ; parité facture↔kWh épinglée par test (grille gelée attribuée, 16 points de contrôle + aller-retour ±1e-6, cas négatif réellement exécuté) ; le proxy d'acceptation transmet X-Forwarded-For/CF-Connecting-IP (sentinelle 'unknown' exclue — jamais de valeur inventée, rouge→vert exécuté). Exécution réelle des sources TS via Node 24 strip-types (aucun node_modules dispo) ; npm run build = à confirmer par la CI.
...
- 2026-08-31 — QJW23 : `npm run check` chaîne désormais `astro check` (la CI l'exécute déjà, zéro edit de workflow) — 113 erreurs préexistantes gelées par (fichier, code TS) avec plafond dans `astro-check-frozen-errors.json` (raisons écrites, dépassement OU nouveau couple = rouge, jamais un gate désarmé) ; cas négatif exécuté (erreur injectée dans mon-toit → exit 1). NOUVELLE DÉPENDANCE (gratuite) : `@astrojs/check@^0.9.10` en devDependency, lock régénéré via npm@10, diff purement additif, entrées Linux intactes.
- 2026-08-31 — Orchestrateur : l'épingle-source de `offresTaillesTAILLES.test.ts` recalée sur la forme QJW17 de `swap.ts` (ligne `const cache` extraite — même invariant, comportement couvert 30/30 par propositionSwap) ; 102/102 verts.
- 2026-08-31 — QJW13 : le repli local de `runSimulation` dérive ses bornes kWc d'`estimateFromBill` (le moteur de l'accueil) au lieu d'une table à la main (+78 % d'écart mesuré sur le plafond à 3 000 MAD/mois) ; `LOCAL_BANDS` reste la table éditoriale des pages segment, différence documentée ; test de parité `leadBandParite.test.ts` ; tree-shaking du bundle client vérifié.
- 2026-08-31 — QJW14 : DÉCISION (inversable d'un mot) — « Refuser » explicite sur la bannière BLOQUE le beacon Meta CAPI (loi 09-08) ; aucune interaction = comportement inchangé (beacon lié à l'acte explicite du prospect), bascule = une ligne documentée ; la capture du lead vers le CRM n'est JAMAIS gatée (épinglé par test) ; gate unique dans `fireCapi`, lu par simulate/preview-lead/capture-lead ; seuls les 2 formulaires diagnostic émettent le flag (tunnel inchangé, extensible d'une ligne) ; rouge 4 échecs puis 7/7.
- 2026-08-31 — QJW15 : jetons `idempotencyKey`/`clientRef` persistés en sessionStorage (survivent au rechargement) sur les 2 formulaires diagnostic ; `aria-invalid` posé/retiré avec focus (erreurs serveur incluses) ; honeypot `website_url` rendu par les 2 composants ET câblé dans `simulate.ts` (la garde preview était un no-op) ; rouge 10/12 puis 12/12.
- 2026-08-31 — QJW21 : cause racine supprimée — la LECTURE du DOM (33 champs) unifiée dans `src/lib/tunnel/lecture.ts`, chaque page l'applique à SON document ; la garde de parité part du DOM réel de chaque locale (cécité de l'ancienne garde démontrée : champ retiré du DOM anglais → verte ; nouvelle → rouge 56≠57 clés) ; 19/19 puis 23/23.
- 2026-08-31 — QJW22 : garde d'appariement registre ↔ liste blanche + assertions de parité déplacées sur le corps POST-liste-blanche (celui qui part au webhook) ; exceptions épinglées une par une (honeypot voulu, `phone→phoneE164`, `utm_*→utm`) ; cas négatif exécuté (clé au registre absente de la liste → rouge).
- 2026-08-31 — Orchestrateur : 2 commentaires périmés « 1,41–1,62 » corrigés en « 1,38–1,62 » (billRange.ts, index.astro) — les valeurs affichées venaient déjà des constantes corrigées par QJW12.
- 2026-08-31 — QJW19 : jeton de demande sur l'effacement d'`aria-busy` — une réponse en retard ne déclare plus « chargé » des chapitres encore en vol (le raccourci cache et le retour Recommandé lèvent l'attente eux-mêmes) ; le harnais du test LIT la politique de péremption dans le `.astro` réel (retirer la garde rougit) ; rouge 6 échecs puis 10/10 + 104/104 voisins.
- 2026-08-31 — QJW20 : couverture des FEUILLES de `proposal_data` — 169 feuilles sur les 3 exemples : 51 lues par `lireProposal`, 118 refus écrits UN PAR UN (chacun cite le lecteur alternatif réel : frontmatter, visionneuse 3D, curseur batterie, contrats PACT10 dédiés qui font foi à leur racine) ; cas négatif exécuté en mémoire, l'échantillon jamais touché (parité QJW1 intacte) ; 80/80.
- 2026-08-31 — QJW17 : nouveau mode de capture `fragment` (les nœuds enfants RÉELS, jamais des clones — le sélecteur de langue garde des références vives) : le bloc « total avec » revient à l'octet après restauration, le triplet FR/EN/AR ne s'affiche plus bout à bout au premier chargement ; cause racine `prepareI18n` documentée, rouge d'abord (4 échecs, DOM aplati en diff) puis 30/30.
- 2026-08-31 — QJW18 : le chiffre au centre du donut suit enfin la taille chargée — `unHtml` rendait `null` sur le `<text>` SVG (peinture silencieusement sautée) ; correctif `unEl`+`textContent`, format inchangé ; le faux-vert historique épinglé (le DOM de test mettait le `<text>` HORS du `<svg>` → `HTMLUnknownElement`) ; rouge « expected 48 to be 62 » puis 95/95 + 108/108.
