...
- [ ] WJ96 — **Make the roof map actually render, and make a bad key VISIBLE.** (a) Add `https://api.mapbox.com` to BOTH `img-src` and `connect-src` in `worker/headers.mjs` (+ assert in `tests/headers.test.ts`) so the sharper-satellite path can't silently CSP-block every tile; (b) log a clear Worker warning + a non-prod diagnostic when `PUBLIC_MAPTILER_KEY` resolves empty (client fallback stays graceful) so a mis-named secret isn't invisible forever; (c) call `ensureMapBooted()` for a sessionStorage-resumed visitor who lands past step 0. **NEEDS YOUR INPUT (not blocking the code):** confirm the Cloudflare secret is named EXACTLY `PUBLIC_MAPTILER_KEY` — a secret named `MAPTILER_KEY` is read as absent, which alone makes the map never load. **Why:** the founder reports the map never shows to clients. @files: apps/web/worker/headers.mjs, apps/web/tests/headers.test.ts, apps/web/src/pages/devis/mon-toit.astro, apps/web/src/pages/api/roof-config.ts
