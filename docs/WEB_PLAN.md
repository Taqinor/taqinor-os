...
- **BLOCKED (left in the queue, moved to NEEDS YOUR INPUT):** WJ115 (`/suivi/<token>` page) waits on PLAN2 QX34 and WJ116 (parrainage real links) waits on PLAN2 QX35 — both backend endpoints are still unbuilt on `main`, and the composition guard forbids building the web consumer against an invented contract. They unblock automatically once QX34/QX35 land; no founder action needed.
