...
> accessreview, contact. **49 apps SORTENT** : agriculture ai_governance ao assurances btp_chantier chat compta
> contacts contrats conversation_ai cpq credit dataquality datarooms douane ecommerce_connect education einvoice
> esg extensions fiscal flotte fpa frais ged gestion_projet grc hospitality immobilier innovation juridique kb
> litiges marketing migration mlops mrp paie pos promotions qhse rh sante scm statuspage territoires transport
> veille_ao voip. **PHASE 2 (premiers à revenir — à retenir, jamais à re-décider)** : chat (Messages), ged (GED),
> ao + veille_ao + btp_chantier (édition « Solaire C&I / EPC »), pack Maroc paie/einvoice/fiscal, compta.
> Reda a dit « fully out » : pas de toggle, le code et les tests PARTENT ; « whenever I want one back » :
> chaque module doit revenir par une recette écrite, sans perte de données.
>
...
> VIDE ; elle RESTE dans `INSTALLED_APPS` (le graphe de migrations des apps gardées reste valide sans squash),
...
> Les 7 liens FK gardé→parqué (stock→flotte ×2, stock→qhse, stock→rh, sav→rh, portail→ged, + rien d'autre —
> inventaire du 20/09) → `RemoveField` (destructif-revertable, seules les colonnes de LIEN partent, noté au
> DONE LOG). Les ~300 appels function-local gardé→parqué sont SUPPRIMÉS avec la fonctionnalité (jamais une
