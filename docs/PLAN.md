...
- [ ] N94 — Translation-management surface in settings so interface strings can be reviewed/adjusted per language without a code change. (UNGATED 2026-07-01 — Reda; BUILD IT. Depends on the N93 i18n framework.)
...
- [ ] DC40 — **Décision modèle `Equipe`** : introduire un modèle d'équipe unique (membres = M2M→User) réutilisé par roster (FG169)/plan de charge (FG299)/planning camionnette (FG303), OU garder le M2M `Intervention.equipe` ad-hoc — une seule définition, pas une par feature. (DECISION) (UNGATED 2026-07-01 — Reda : trancher pour UN modèle `Equipe` canonique unique et migrer roster/plan-de-charge/camionnette dessus, migration de données réversible.)
...
- [ ] UX1 — **Kit « ERP module shell » + standard de design** : confirmer/extraire les briques partagées que tous les écrans de module réutilisent — `ModuleDashboard` (bande de `Stat` + grille recharts + drill-down), `ListShell` (PageHeader + `DataTable` + filtres + vues sauvegardées + export), `DetailShell` (entête + `Tabs` + panneau activité/chatter optionnel), `EcheanceCenter` (liste d'alertes d'échéances agrégées, tri par urgence), `moduleNav` helper (enregistrement d'une section Sidebar gatée), et un helper `statusPill(map)`. Ajouter une entrée dans le showcase `/ui`. Fait = kit documenté + utilisé par au moins le module Comptabilité, look identique d'un module à l'autre, tests de la logique pure. (ARCH) (@lane: frontend/ui)

### UX · Comptabilité (`/api/django/compta/…`, `IsResponsableOrAdmin`)

- [ ] UX2 — **Cockpit financier** (`GET /compta/pilotage/cockpit/`) : page `/comptabilite` — cartes résultat du mois, trésorerie nette, DSO/DPO, marge %, créances en retard ; courbes recharts (résultat 12 mois, trésorerie) ; drill-down vers grand-livre/balance âgée. Fait = cockpit lisible, chiffres alignés `tabular-nums`, drill-down fonctionnel, test. (@lane: frontend/compta)
- [ ] UX3 — **Plan comptable + journaux** (`comptes` filtrable `?classe=`, `journaux`, `plans`) : arbre/liste des comptes CGNC par classe 1–8, CRUD comptes ; liste des journaux (VTE/ACH/BNK/CSH/OD/AN). Fait = navigation par classe, création/édition compte, tests. (@lane: frontend/compta)
- [ ] UX4 — **Saisie d'écriture (grille débit=crédit)** (`ecritures` filtrable `?journal=&date_debut=&date_fin=`, actions `valider`/`extourner`) : éditeur d'écriture multi-lignes avec **équilibre débit/crédit calculé en direct** (blocage tant que déséquilibré), sélection compte (Combobox) + tiers, valider (verrou audit) et extourner (contre-passation). Fait = on ne peut enregistrer qu'une écriture équilibrée, valider/extourner câblés, tests. (@lane: frontend/compta)
- [ ] UX5 — **États financiers CGNC** (`GET /compta/etats/{grand-livre,balance,cpc,bilan,esg,etic}/`) : écran « États » avec sélecteur exercice/période, rendu tableau (Table réutilisable) du grand-livre, balance, CPC (compte de produits & charges), bilan (date d'arrêté), ESG/ETIC ; export. Fait = chaque état s'affiche et s'exporte, tests. (@lane: frontend/compta)
- [ ] UX6 — **Trésorerie & prévisionnel** (`etats/position-tresorerie`, `etats/previsionnel-tresorerie`, `tresorerie`, `caisses`, `virements`, `previsionnel`) : position par compte/caisse + total + projection nette AR/AP/paie/TVA (recharts waterfall), CRUD comptes de trésorerie/caisses/virements internes, lignes prévisionnelles. Fait = position + projection lisibles, CRUD trésorerie, test. (@lane: frontend/compta)
- [ ] UX7 — **Fiscalité & déclarations** (`etats/{export-fec,liasse-fiscale,export-fiduciaire,releve-deductions-tva,declaration-honoraires,aide-is,balance-agee-fournisseurs}`, `declarations-tva`, `retenues-source`, `timbres-fiscaux`) : centre « Déclarations » avec génération/téléchargement FEC, liasse, export fiduciaire, relevé déductions TVA, état 9421 honoraires, aide IS ; CRUD déclarations TVA / RAS. Fait = chaque export se télécharge (blob), CRUD déclarations, test. (@lane: frontend/compta)
- [ ] UX8 — **Immobilisations & amortissements** (`immobilisations`, `dotations`, `cessions`) : registre des immobilisations avec plan d'amortissement, dotations, cessions. Fait = liste + détail immobilisation avec échéancier d'amortissement, tests. (@lane: frontend/compta)
- [ ] UX9 — **Rapprochements, budgets & analytique** (`rapprochements`, `rapprochements-3voies`, `budgets`, `centres-cout`, `provisions-creances`, `comptes-auxiliaires`, `mappings-compte`, `pieces-justificatives`, `exercices`, `periodes`) : rapprochement bancaire + rapprochement 3 voies (bon-à-payer bloqué si écart), budgets par centre de coût (réel vs budget, recharts), comptes auxiliaires tiers, mapping comptable, pièces justificatives, gestion exercices/périodes (verrouillage). Fait = écrans câblés + verrou exercice, tests. (@lane: frontend/compta)

### UX · Paie (`/api/django/paie/…`, `IsResponsableOrAdmin` ; coffre-fort = tous rôles)

- [ ] UX10 — **Assistant de run de paie** (`periodes` + `POST /paie/periodes/generer/`, `bulletins` + `changer-statut`/`cloturer`) : wizard « lancer la paie du mois » review-before-commit — 1) créer/ouvrir période, 2) générer les bulletins, 3) **revue** (tableau des bruts/nets/écarts, repérer les anomalies avant validation), 4) valider, 5) clôturer (verrou). Fait = enchaînement guardé, on revoit avant de valider, statuts progressifs respectés, tests. (@lane: frontend/paie)
- [ ] UX11 — **Bulletin de paie (aperçu)** (`GET /paie/bulletins/{id}/bulletin/`, `/attestation/`, `importer-elements-rh`) : détail bulletin (rubriques brut → cotisations CNSS/AMO/IR → net), import des éléments RH, attestation PDF. Fait = bulletin lisible fidèle, import RH, PDF, test. (@lane: frontend/paie)
- [ ] UX12 — **Paramètres & barèmes de paie** (`parametres` + `seed-defaults`, `baremes` IR, `rubriques` + `seed-defaults`/`seed-standard`, `profils`, `rubriques-employe`) : édition des constantes (SMIG/CNSS/AMO/frais pro, versionnées `date_effet`), barème IR + tranches, catalogue de rubriques (seed en un clic), profils de paie par employé. Fait = seed idempotent depuis l'UI, édition versionnée, tests. (@lane: frontend/paie)
- [ ] UX13 — **Déclarations & virements de paie** (`ordres-virement` + `valider`/`rectifier`, `bulletins/{id}/{declaration-cnss,fichier-damancom,etat-ir,livre-de-paie}`, `bulletins/etat-ir-annuel`, `avances`, `saisies`, `cumuls-annuels`) : ordres de virement (fichier bancaire), déclaration CNSS + fichier DAMANCOM, état IR + IR annuel, livre de paie, avances & saisies-arrêt, cumuls annuels. Fait = fichiers générés, CRUD avances/saisies, tests. (@lane: frontend/paie)
- [ ] UX14 — **Self-service « Mes bulletins »** (`GET /paie/mes-bulletins/`, `authLoader` tous rôles) : espace employé — liste + téléchargement de ses propres bulletins (coffre-fort), jamais ceux des autres. Fait = un employé voit uniquement ses bulletins, test d'isolation. (@lane: frontend/paie)

### UX · Flotte (`/api/django/flotte/…`)

- [ ] UX15 — **Cockpit flotte (4 KPI north-star)** (`GET /flotte/releves-telematiques/tableau-bord/`, `vehicules/{id}/tco`) : page `/flotte` — coût/km, taux d'utilisation, immobilisation non planifiée, conformité entretien ; bande d'alertes (entretien dû, échéances) ; drill-down vers le véhicule. Fait = 4 KPI + alertes + drill-down, test. (@lane: frontend/flotte)
- [ ] UX16 — **Véhicules & engins (liste + fiche)** (`vehicules`, `engins`, `actifs`, `referentiels` ; actions `tsav`/`tco`/`eco-conduite`/`amortissement`) : liste filtrable (statut actif/entretien/réformé, énergie), fiche véhicule avec onglets (identité/carte grise, coûts TCO, éco-conduite CO₂, amortissement, TSAV). Fait = liste + fiche multi-onglets, calculs affichés, tests. (@lane: frontend/flotte)
- [ ] UX17 — **Conducteurs & affectations** (`conducteurs`, `affectations`, `reservations`, `demandes-vehicule`, `etats-des-lieux`) : conducteurs (permis + validité), affectation conducteur↔véhicule (contrôle permis valide), réservations/demandes, états des lieux. Fait = affectation bloquée si permis invalide (message clair), CRUD, tests. (@lane: frontend/flotte)
- [ ] UX18 — **Entretien & réparations** (`plans-entretien`, `echeances-entretien`, `garages`, `ordres-reparation`, `pneumatiques`, `pieces`) : plans d'entretien préventif + échéances (timeline), garages, ordres de réparation (main-d'œuvre + pièces), pneus, pièces. Fait = échéancier d'entretien + OR, tests. (@lane: frontend/flotte)
- [ ] UX19 — **Conformité & documents** (`echeances-reglementaires`, `assurances`, `visites-techniques`, `cartes-grises`, `baremes-vignette`) : centre d'échéances réglementaires (assurance/vignette-TSAV/visite technique/carte grise) trié par urgence, couleurs J-7/15/30/échu. Fait = centre d'échéances agrégé multi-sources, test. (@lane: frontend/flotte)
- [ ] UX20 — **Carburant, sinistres & télématique** (`pleins`, `cartes`, `sinistres`, `infractions` + `alertes-echeances`, `releves-telematiques`, `trajets-telematiques`, `trajets-chantier`) : pleins + cartes carburant (anomalies), sinistres, infractions (PV en attente), trajets télématiques. Fait = suivi carburant + sinistres/infractions, tests. (@lane: frontend/flotte)

### UX · RH (`/api/django/rh/…`, `IsResponsableOrAdmin` ; portail = tous rôles ; salaires = `salaires_voir`)

- [ ] UX21 — **Cockpit RH + centre d'échéances** (`GET /rh/cockpit/`, `rh/echeances`, `rh/tableau-bord-hse`) : effectifs, absentéisme, échéances (CDD, certifs, visites médicales), KPI HSE ; alertes triées. Fait = cockpit + échéances agrégées, test. (@lane: frontend/rh)
- [ ] UX22 — **Dossier employé (master)** (`employes` + `verifier-habilitation`/`registre-formation`/`cdd-a-echeance`/`expirant-bientot`, `documents`, `remunerations` derrière `salaires_voir`) : liste employés (statut/contrat), fiche master multi-onglets (identité, contrat, documents, rémunération gatée, habilitations, formations). Fait = fiche complète, rémunération masquée sans `salaires_voir`, tests. (@lane: frontend/rh)
- [ ] UX23 — **Congés & absences** (`types-absence`, `soldes-conge`, `demandes-conge`) : soldes de congé, workflow de demande (soumettre → valider/refuser), vue calendrier d'équipe. Fait = workflow congés + calendrier, test. (@lane: frontend/rh)
- [ ] UX24 — **Temps & présence** (`pointages` + `calendrier-equipe`/`export-paie`/`pointager-arrivee`/`pointager-depart`, `feuilles-temps`, `heures-supp`, `roster`, `presences-chantier`, `incidents-presence`) : pointage arrivée/départ, calendrier équipe, feuilles de temps, heures supp, roster (planning d'équipes), présence chantier ; export paie. Fait = pointage + roster + export paie, tests. (@lane: frontend/rh)
- [ ] UX25 — **Compétences, habilitations & formation** (`competences`, `competences-employe`, `habilitations`, `certifications`, `visites-medicales`, `sessions-formation`, `besoins-formation`) : matrice de compétences, habilitations/certifications (validité), visites médicales, sessions & besoins de formation. Fait = matrice + suivi validité, tests. (@lane: frontend/rh)
- [ ] UX26 — **EPI, recrutement & évaluations** (`epi-catalogue`, `dotations-epi`, `ouvertures-poste`, `candidatures`, `campagnes-evaluation`, `evaluations-employe`, `sanctions`) : catalogue & dotation EPI (émargement), ATS léger (postes → candidatures → embauche), campagnes d'évaluation, sanctions. Fait = pipeline recrutement + EPI + évaluations, tests. (@lane: frontend/rh)
- [ ] UX27 — **HSE RH** (`accidents-travail`, `presqu-accidents`, `causeries-securite`, `analyses-risques-chantier`) : registre accidents du travail + presqu'accidents, causeries sécurité, analyses de risques chantier. Fait = registres HSE, test. (@lane: frontend/rh)
- [ ] UX28 — **Portail self-service employé** (`GET /rh/portail/`, `authLoader` tous rôles ; `ordres-mission`, `notes-frais`, `avances-salaire`) : tableau de bord employé (mes congés, mes documents, mes notes de frais, mes ordres de mission). Fait = espace employé isolé par utilisateur, test. (@lane: frontend/rh)

### UX · QHSE (`/api/django/qhse/…`, `IsResponsableOrAdmin`)

- [ ] UX29 — **Cockpit QHSE + calendrier** (`qhse/calendrier`, `qhse/iso9001-readiness`, taux fréquence/gravité) : KPI sécurité (TF/TG), checklist ISO 9001, calendrier des échéances QHSE. Fait = cockpit + ISO readiness, test. (@lane: frontend/qhse)
- [ ] UX30 — **Non-conformités & CAPA** (`non-conformites` + `depuis-reserve`/`noter`/`historique`, `capa` + `en-retard`/`relancer-retards`/`verifier-efficacite`) : registre NCR (créer depuis réserve chantier, chatter/historique), actions correctives/préventives (en retard, relance, vérif efficacité). Fait = NCR→CAPA avec chatter + retards, tests. (@lane: frontend/qhse)
- [ ] UX31 — **Inspections & audits** (`plans-inspection`, `points-controle`, `plans-chantier`, `releves`, `grilles-audit`, `criteres-audit`, `audits`, `reponses-critere`, `notations-fin-chantier` + `peut-cloturer`, `procedures-qualite`, `retours-client`) : modèles d'inspection → plan chantier → relevés ; grilles d'audit → audits → réponses ; notation de fin de chantier (gate de clôture). Fait = chaîne inspection + audit + gate clôture, tests. (@lane: frontend/qhse)
- [ ] UX32 — **Risques, permis & incidents** (`evaluations-risque`, `lignes-evaluation-risque`, `permis-travail`, `consignations-loto`, `inductions-securite`, `plans-urgence`, `contacts-urgence`, `secouristes`, `incidents`, `declarations-cnss`, `analyses-incident`, `causes-incident`, `inspections-securite`) : document unique (matrice de risques), permis de travail + consignation LOTO, inductions, plans d'urgence/secouristes, incidents + déclaration CNSS + analyse causes (arbre). Fait = document unique + permis + incidents, tests. (@lane: frontend/qhse)
- [ ] UX33 — **Environnement & ESG** (`dechets`, `bordereaux-dechets`, `recyclage-modules`, `conformites-environnementales`, `bilans-carbone`, `lignes-bilan-carbone`, `indicateurs-esg`) : suivi déchets + bordereaux, recyclage modules PV, conformité environnementale, bilan carbone (recharts), indicateurs ESG. Fait = suivi déchets + bilan carbone, test. (@lane: frontend/qhse)

### UX · Contrats (`/api/django/contrats/…`, `IsResponsableOrAdmin`)

- [ ] UX34 — **Contrats (liste + fiche cycle de vie)** (`contrats` + `preavis`/`a-renouveler`, `parties`, `contrat-liens`) : liste par type/statut (brouillon→approbation→signé→actif→suspendu→résilié→expiré), fiche avec parties (FK client/fournisseur/employé), pills de statut, confidentialité. Fait = liste + fiche + machine à états lisible, test. (@lane: frontend/contrats)
- [ ] UX35 — **Modèles, clauses & versions** (`modeles`, `clauses`, `modele-clauses`, `clauses-contrat`, `versions`, `avenants`, `resiliations`) : bibliothèque de gabarits + clauses, versions immuables (diff), avenants, résiliations. Fait = composition depuis gabarit + historique versions, test. (@lane: frontend/contrats)
- [ ] UX36 — **Échéances & alertes contrats** (`alertes`, `jalons`, `obligations`, `sla`, `regles-approbation` + `preavis`/`a-renouveler`) : centre d'échéances (préavis, renouvellement), jalons, obligations (à faire), SLA, workflow d'approbation. Fait = centre d'échéances + obligations, test. (@lane: frontend/contrats)
- [ ] UX37 — **Finances de contrat** (`retenues-garantie`, `cautions`, `echeanciers`, `lignes-echeance`, `indexations`, `pieces-conformite`) : retenues de garantie, cautions, échéanciers de paiement, indexation prix, pièces de conformité. Fait = échéanciers + garanties, test. (@lane: frontend/contrats)

### UX · Gestion de projet (`/api/django/gestion-projet/…`, `IsResponsableOrAdmin`)

- [ ] UX38 — **Projets (liste + machine à états)** (`projets` + `planifier`/`demarrer`/`pausner`/`terminer`/`annuler`/`liens`, `projet-chantiers`, `projet-liens`, `clotures`) : liste multi-chantiers, fiche projet avec transitions gardées (brouillon→planifié→en cours→pause→terminé/annulé), liens CRM/chantiers, clôture. Fait = liste + transitions + liens, test. (@lane: frontend/gestion_projet)
- [ ] UX39 — **Planning Gantt** (`phases`, `taches`, `dependances`, `jalons`, `calendriers`, `jours-feries`, `baselines`) : vue Gantt (phases/tâches/dépendances/jalons) avec baseline, calendriers & jours fériés. Fait = Gantt lisible avec dépendances + baseline, test. (@lane: frontend/gestion_projet)
- [ ] UX40 — **Ressources & capacité** (`ressources`, `equipes`, `affectations`, `indisponibilites`, `timesheets`) : profils de ressources, équipes, affectations, indisponibilités, plan de charge (capacité vs affecté, recharts), timesheets. Fait = plan de charge + affectations, test. (@lane: frontend/gestion_projet)
- [ ] UX41 — **Budget & P&L projet** (`budgets`, `lignes-budget`) : budget par catégorie, réel vs budgété (recharts), coûts engagés vs réels. Fait = budget vs réel lisible, test. (@lane: frontend/gestion_projet)
- [ ] UX42 — **Risques, actions & CR** (`risques`, `actions`, `comptes-rendus`, `documents`, `commentaires`, `modeles`, `modele-taches`, `sous-traitants`, `lots-sous-traitance`) : registre des risques, plan d'actions, comptes-rendus de réunion, documents/commentaires, gabarits de projet, sous-traitants & lots. Fait = registres + gabarits, test. (@lane: frontend/gestion_projet)

### UX · Knowledge Base & Litiges

- [ ] UX43 — **Base de connaissances** (`kb/articles` + `publier`/`nouvelle-version`/`marquer-lu`, `kb/versions`, `kb/article-liens`, `kb/article-acls`) : navigateur d'articles (catégories/tags, recherche), éditeur (brouillon→publié, versions), suivi de lecture, ACL par rôle. Fait = lire/écrire/publier un article + suivi lu + ACL, tests. (@lane: frontend/kb)
- [ ] UX44 — **Litiges & réclamations** (`litiges/reclamations` + `tableau-bord`/`analyse-concurrents`) : registre des réclamations (type/gravité/statut, montant contesté, blocage relances), cockpit (ouverts, montant contesté, délai moyen), analyse concurrents (deals perdus). Fait = registre + cockpit + analyse concurrents, test. (@lane: frontend/litiges)

### UX · GED — surfaces avancées (le CRUD de base/upload est couvert par U14 dans PLAN2)

- [ ] UX45 — **GED : approbation & signature** (`ged/demandes-approbation`, `ged/modeles-document`, `ged/demandes-signature`) : workflow d'approbation de document (réviseur/décision/statut), génération depuis modèle, demandes de signature électronique. Fait = un document passe en approbation puis signature depuis l'UI, test. (@lane: frontend/ged)
...
- [ ] UX47 — **GED : tags & liens transverses** (`ged/tags`, `ged/tag-assignments`, `ged/liens`) : taxonomie de tags + application, liens polymorphes document↔lead/client/devis/facture/chantier/ticket affichés dans la fiche document ET côté entité liée. Fait = tag + lien visibles des deux côtés, test. (@lane: frontend/ged)
