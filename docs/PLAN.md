...
- [ ] N70 — Per-user activity & access view so Reda can see who did what.
...
- [x] F4 — **Intervention list view + kanban grouped by intervention statut**, reusing the same visual language, drag-to-change-status, and reassignment behaviour as the existing lead and chantier kanbans. Each card shows client, ville, type, planned date, équipe, statut.
...
- 2026-06-17 — F1 done (module field-execution, sur la branche `dev-field-exec` — non fusionné sur main tant que le module n'est pas complet) : catalogue **Outillage** (équipement durable), totalement séparé des SKU vendables (`Produit`). Nouveau modèle `stock.Outillage` (nom, catégorie, asset tag unique par société, n° série, emplacement Dépôt/Camionnette/En intervention, statut Disponible/En intervention/En réparation/Perdu, date d'achat) ; photo optionnelle via la pièce jointe générique (`records.Attachment`, cible `('stock','outillage')` ajoutée). API company-scopée `/api/django/stock/outillage/` (TenantMixin, filtre emplacement + statut, recherche), nouvelle page **Stock → Outillage** (liste + filtres + ajout/édition). Un outil n'est jamais vendu, consommé, ni mis sur un document client (testé). Tests backend (company-scoping, company forcée côté serveur, filtres, unicité asset_tag, séparation du stock) + tests front (helpers). Build front vert, lint vert.
- 2026-06-17 — F2 done (dev-field-exec, non fusionné) : **Kits d'outillage** — modèles réutilisables éditables dans Paramètres (Outillage → Kits). Nouveaux modèles `stock.KitOutillage` (+ `KitOutillageItem`, liste ordonnée d'outils du catalogue Outillage), sélectionnables par type d'intervention (clé texte, pas de FK inter-app). Trois kits par défaut seedés (Kit pose structure / raccordement électrique / mise en service, protégés, vides — le founder y ajoute ses outils). API company-scopée `/stock/kits-outillage/` (seed à la lecture, écriture admin, items en imbriqué writable avec garde multi-tenant sur chaque outil, kit protégé non supprimable → 409 mais désactivable). Éditeur Paramètres autonome (ajout/renommage/réordre/désactivation du kit + ajout/retrait/réordre des outils). Désactivation préserve l'historique (cohérent N57). Tests backend (seed idempotent, CRUD, garde inter-société, désactivation, protection). Build + lint front verts. **Backend non validé (CI billing bloqué + pas de Postgres local).**
- 2026-06-17 — F3 done (dev-field-exec, non fusionné) : **Intervention (sortie chantier)** — le modèle léger `installations.Intervention` (« Ordre de travail ») existant est ÉTENDU (pas de modèle parallèle) : nouveau `statut` propre (À préparer / Prête / En route / Sur site / Terminée / Validée), équipe (M2M employés, défaut = installeur du chantier posé côté serveur), camionnette (libellé), types ajoutés SAV + Visite. Le `statut` est une **machine à états SÉPARÉE** : le changer ne lit ni n'écrit JAMAIS le statut du chantier ni le pipeline lead (testé). Nouveau chatter `InterventionActivity` (création + changements de statut automatiques, notes manuelles) avec endpoints `/historique` et `/noter` (acteur + société côté serveur). Client, devis, ville et GPS du site exposés en lecture seule, tirés du chantier (jamais dupliqués). Migration additive (statut, camionnette, M2M équipe, +2 types, modèle activité). Tests (défauts, isolation du statut, filtre, chatter, scoping, garde inter-société). Pas de régression sur le test intervention existant. **Backend non validé (CI billing bloqué + pas de Postgres local).** UI = F4.
- 2026-06-17 — F4 done (dev-field-exec, non fusionné) : **Interventions — liste + kanban**. Nouvelle page (Chantiers → Interventions, `/interventions`) avec bascule Kanban/Liste. Kanban groupé par statut d'intervention (À préparer → Validée), même langage visuel et même glisser-déposer (@dnd-kit) que les kanbans lead/chantier ; glisser une carte change le statut (optimiste, PATCH) **sans jamais toucher le statut du chantier**. Réassignation d'équipe par carte (select → PATCH equipe). Cartes : client, ville, type, date prévue, équipe, statut. Vue liste filtrable par statut. Helpers de statut purs + testés. Lint + build + tests front verts.
- 2026-06-17 — SKIP/BLOQUÉS (laissés [ ], non construits sans décision du founder — dépendance/coût/auth/architecture nouvelle, conformément aux GATED) : N14 (consommation stock à la pose = G6), N20 (étiquettes QR/code-barres = nouvelle dépendance), N50 (intégration monitoring Huawei FusionSolar = service externe + identifiants), N68 (éditeur RBAC = G4), N69 (permission prix d'achat/marges, dépend RBAC/auth), N72 (moteur d'automatisation/planificateur = G9), N73 (étape d'approbation dans l'automatisation, dépend N72), N85 (carte = nouvelle dépendance carto), N86 (extension chatbot = clé IA G11 + portée), N87 (envoi email = G1 coût/auth), N88 (capture email entrant = externe/coût), N89 (API REST publique = nouvelle architecture + auth/jetons), N91 (capture offline = hors périmètre PWA + nouvelle architecture sync), N92 (push PWA = infra VAPID), N93 (i18n arabe/darija complet = G15), N94 (gestion de traductions, dépend N93), N96 (2FA/sessions = G8 auth), N100 (plateforme multi-tenant = nouvelle architecture), N101 (console tenants + signup = nouvelle architecture/auth). Restent ouverts et CONSTRUCTIBLES pour une prochaine session (additifs) : N15, N17, N18, N19, N26, N36, N46, N47, N49, N51, N52, N53, N58, N59, N60, N62, N64, N65, N66, N67, N70, N74, N75, N76, N78, N79, N80, N84, N95, N97, N98, N99 (+ N102 doc finale).
