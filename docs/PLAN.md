...
- [ ] N96 — Account security: optional 2FA, visible active sessions with revoke, forced credential-rotation flow; production DEBUG setting left unchanged. (UNGATED 2026-06-20 — auth change approved (this closes G8); pyotp dependency approved. Build 2FA as OPT-IN per user so it can NEVER lock existing users out.) [PARTIAL 2026-06-20: the **2FA** half is DONE — opt-in TOTP on `CustomUser` (`totp_secret`/`totp_enabled`/hashed recovery codes, default OFF → no lockout), setup/enable/disable/status endpoints, login requires `otp` ONLY when enabled (401 `otp_required`), "Sécurité du compte" settings tab + login code step, `pyotp==2.10.0`, 11 tests. STILL OPEN: visible active sessions with revoke + forced credential-rotation flow.]
