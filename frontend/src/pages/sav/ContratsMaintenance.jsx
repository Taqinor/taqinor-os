...
export function Component() {
  ...
  const [form, setForm] = useState({ client: '', periodicite: 'annuel', date_debut: '' })
  ...
  const create = async () => {
    ...
    try {
      await savApi.saveContrat(null, form)
      setForm({ client: '', periodicite: 'annuel', date_debut: '' })
      load()
  ...
  return (
    <div className="page" style={{ maxWidth: 1000 }}>
      ...
      <table className="data-table">
        <thead>
          <tr><th>Client</th><th>Périodicité</th><th>Début</th><th>Prochaine visite</th><th>Statut</th></tr>
        </thead>
        <tbody>
          {rows.map(c => (
            <tr key={c.id}>
          ...
          {!rows.length && <tr><td colSpan={5} style={{ color: '#94a3b8' }}>Aucun contrat.</td></tr>}
        </tbody>
      </table>
    </div>
  )
