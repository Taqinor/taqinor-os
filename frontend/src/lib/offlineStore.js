// NTMOB1 — persistance IndexedDB des files hors-ligne (TOUS modules).
//
// Déplacé depuis `features/installations/offline/idbStore.js` (N91/F21/EZ8) :
// la file n'est plus une affaire de capture terrain, elle sert crm/ventes/
// stock/installations/sav. L'ancien chemin RÉEXPORTE ce module — il n'y a
// qu'UNE implémentation, jamais deux magasins concurrents (décision VX105).
//
...
// NTMOB1 — une file JSON par MODULE, dans la MÊME base et le MÊME object store,
// sous la clé `module:<nom>` : une seule base, un seul badge, N files.
...
function localStorageAdapter(key = KEY) {
  const k = `${DB_NAME}:${key}`
  return {
    async load() {
      try {
...
// NTMOB1 — store JSON d'un MODULE (`crm`, `ventes`, `stock`…). Mêmes replis que
// la file terrain : une op JSON survit très bien à localStorage.
export function createModuleOutboxStore(module) {
  const key = `${MODULE_KEY_PREFIX}${module}`
  try {
    if (typeof indexedDB !== 'undefined' && indexedDB) return idbAdapter(key)
  } catch { /* repli */ }
  try {
