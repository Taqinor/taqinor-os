// NTSRV30 — Assistant de clôture de ticket SAV (2 étapes).
//
// POURQUOI : la clôture en UN clic laisse partir des tickets sans cause ni
// remède codifiés (XSAV14) et sans enquête de satisfaction. Cet assistant
// rend ces deux gestes VISIBLES sans les rendre obligatoires à la création
// rapide d'un ticket :
//   (1) cause + remède — exigés SEULEMENT ici, jamais à la création ;
//   (2) canal de résolution + déclenchement de l'enquête (lien share_token
//       déjà existant, FG86/XSAV10).
// SOLMVP41 — l'étape « Créer un article KB » (branchée sur NTSRV19) est
// partie avec l'app kb (Groupe SOLMVP) : son endpoint `creerArticleDepuisTicket`
...
export default function TicketClotureWizard({ ticket, onTermine, onAnnuler }) {
  ...
  const terminer = async () => {
    ...
    try {
      ...
      onTermine?.({ rapide: false, lienEnquete: lien })
    } catch (e) {
      setErreur(messageErreur(e, 'La clôture a échoué. Réessayez.'))
    } finally {
      setEnCours(false)
