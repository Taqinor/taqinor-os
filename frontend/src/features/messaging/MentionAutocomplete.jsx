...
export default function MentionAutocomplete({ items, activeIndex, onPick, onClose }) {
  useEffect(() => {
    const onDoc = (e) => {
  ...
  return (
    <ul className="chat-mention-pop" role="listbox" aria-label="Membres à mentionner">
      {items.map((m, i) => (
        <li key={m.value ?? m.id} role="option" aria-selected={i === activeIndex}>
          <button
            type="button"
