...
describe('PrixNegocies (PACT57)', () => {
  ...
  it('crée une commande-cadre avec un fournisseur', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Nouvelle commande-cadre/ }))
  ...
  it('crée un contrat de prix depuis l\'onglet dédié', async () => {
    ...
    expect(await screen.findByText('Aucun contrat de prix')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: /Nouveau contrat de prix/ }))
    await screen.findByRole('option', { name: 'Solar Import SARL' })
