...
// question « propriétaire ou locataire » ni de flux locataire (un locataire
// qui paie est installé), plus de question « abonnement et entretien du
// compteur » (toute facture les porte, toujours) ; chaque question s'affiche
// SEULE, ses remarques internes en retrait (`decouperQuestion`). Le panneau
// reste ouvert d'une réponse à l'autre : c'est la FRISE qui garde ses lignes
// montées pendant un rechargement (`CadenceFrise.jsx`), pas ce composant.
 ...
 *  champ (`fieldLabels`) plutôt que le `verbose_name` technique. Découpé en
 *  `{ question, consigne }` : la question seule se lit au client, la
 *  consigne (remarques internes du `help_text`) s'affiche en retrait. */
function texteDeLaQuestion(entree) {
  return decouperQuestion(
    entree?.question || fieldLabels[entree?.champ]?.label || texteQuestion(entree))
}
  ...
  return (
    <div className="flex flex-col gap-1" data-testid={`question-appel-${entree.champ}`}>
      {Array.isArray(entree.choix) ? (
        ...
        <label className="text-sm text-foreground" htmlFor={id}>{question}</label>
      )}
      {consigne && (
        <p className="text-xs text-muted-foreground" data-testid={`consigne-question-${entree.champ}`}>
          {consigne}
        </p>
      )}
      {Array.isArray(entree.choix) ? (
