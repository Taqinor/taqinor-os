...
// WIR234 — planifier un exercice d'urgence (drill) rattaché à un plan.
function PlanifierExerciceDialog({ onClose, onDone }) {
  ...
  async function save() {
    if (!plan || !datePrevue) { toast.error('Plan et date prévue requis.'); return }
    setSaving(true)
    try {
      await qhseApi.exercicesUrgence.create({
        plan: Number(plan), type_exercice: type, date_prevue: datePrevue,
      })
      toast.success('Exercice planifié.')
      onDone()
      onClose()
    } catch (e) {
      toast.error(e?.response?.data?.detail ?? 'Planification impossible.')
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onClose() }}>
      <DialogContent>
        ...
        <div className="flex flex-col gap-3">
          <div>
            <Label>Plan d'urgence (id)</Label>
            <Input value={plan} onChange={(e) => setPlan(e.target.value)} inputMode="numeric" />
          </div>
          <div>
            <Label>Type</Label>
            <Select value={type} onValueChange={setType}>
              ...
              <SelectContent>
                <SelectItem value="evacuation">Évacuation</SelectItem>
                <SelectItem value="incendie">Incendie</SelectItem>
                <SelectItem value="deversement">Déversement</SelectItem>
...
// WIR234 — réaliser un exercice planifié (chronométrage évacuation + participants).
function RealiserExerciceDialog({ exercice, onClose, onDone }) {
  ...
  async function save() {
    ...
    try {
      await qhseApi.exercicesUrgence.realiser(exercice.id, {
        duree_evacuation_secondes: duree || undefined,
        nb_participants: participants || undefined,
        observations,
      })
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onClose() }}>
      <DialogContent>
        <DialogTitle>Réaliser l'exercice</DialogTitle>
        <div className="flex flex-col gap-3">
          <div>
          ...
          <div>
            ...
            <Input value={participants} onChange={(e) => setParticipants(e.target.value)} inputMode="numeric" />
          </div>
          <div>
...
// WIR278 — création d'une partie intéressée pertinente (ISO 4.2).
const PERTINENCE_OPTIONS = [
...
function CreerPartieInteresseeDialog({ onClose, onCreated }) {
  ...
  async function save() {
    if (!partie.trim()) { toast.error('La partie intéressée est requise.'); return }
    setSaving(true)
    try {
      await qhseApi.partiesInteressees.create({
        partie: partie.trim(), attentes: attentes.trim(), pertinence,
      })
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <Textarea rows={2} aria-label="Attentes" value={attentes} onChange={(e) => setAttentes(e.target.value)} />
          </div>
          <div>
            <Label>Pertinence</Label>
            <Select value={pertinence} onValueChange={setPertinence}>
              <SelectTrigger aria-label="Pertinence"><SelectValue /></SelectTrigger>
              <SelectContent>
                {PERTINENCE_OPTIONS.map((o) => (
                  <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
...
// WIR277/278 — Contexte SMQ ISO 4.1 (SWOT + périmètre, singleton société,
// GET crée l'enregistrement vide au premier appel) + parties intéressées
// (ISO 4.2). `PartieInteresseeViewSet` filtre par `?pertinence=`.
function ContexteSmqPanel() {
  ...
  useEffect(() => {
    let vivant = true
    qhseApi.contexteOrganisation.get()
      .then((r) => {
        if (!vivant) return
        setSwot(r.data?.swot ?? '')
        setPerimetre(r.data?.perimetre_smq ?? '')
      })
      .catch(() => { if (vivant) toast.error('Contexte illisible.') })
  ...
  async function enregistrerContexte() {
    setSaving(true)
    try {
      await qhseApi.contexteOrganisation.update({ swot, perimetre_smq: perimetre })
  ...
  const partiesCols = useMemo(() => [
    { id: 'partie', header: 'Partie intéressée', accessor: (r) => r.partie },
    { id: 'attentes', header: 'Attentes', accessor: (r) => r.attentes || '—' },
    {
      id: 'pertinence', header: 'Pertinence', width: 120,
      accessor: (r) => r.pertinence,
      cell: (v, r) => (
        <Badge tone={v === 'forte' ? 'danger' : v === 'moyenne' ? 'warning' : 'neutral'}>
          {r.pertinence_display || v}
        </Badge>
      ),
    },
  ], [])
    ...
    <div className="flex flex-col gap-6">
      <Card className="p-4 sm:p-5">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            <div>
              <Label>Analyse SWOT</Label>
              <Textarea rows={4} aria-label="Analyse SWOT" value={swot}
                onChange={(e) => setSwot(e.target.value)} />
            </div>
            <div>
              <Label>Périmètre du SMQ</Label>
              <Textarea rows={3} aria-label="Périmètre du SMQ" value={perimetre}
                onChange={(e) => setPerimetre(e.target.value)} />
            </div>
            <div className="flex justify-end">
              <Button onClick={enregistrerContexte} disabled={saving}>
                {saving ? 'Enregistrement…' : 'Enregistrer'}
              </Button>
            </div>
          </div>
      ...
      <QhseResourceList
        title="Parties intéressées (ISO 4.2)"
        subtitle="Pertinence forte/moyenne/faible — rien n'est masqué par défaut"
        fetcher={() => qhseApi.partiesInteressees.list()}
        columns={partiesCols}
        exportName="qhse-parties-interessees"
        deps={[reloadNonce]}
        actions={(
          <Button size="sm" onClick={() => setCreatingPartie(true)}>
...
/* ============================================================================
   ...
   • Contexte SMQ (ISO 4) : SWOT/périmètre + parties intéressées (WIR277/278).
   ========================================================================== */
...
export default function Risques() {
  ...
  // WIR234 — drills : planification + réalisation + CAPA d'écart.
  const [planifiantExercice, setPlanifiantExercice] = useState(false)
  const [realisantExercice, setRealisantExercice] = useState(null)
  const [exercicesReload, setExercicesReload] = useState(0)

  const creerCapaExercice = async (exercice) => {
    try {
      await qhseApi.exercicesUrgence.creerCapa(exercice.id, {})
      toast.success('CAPA créée depuis l’écart.')
      setExercicesReload((n) => n + 1)
    } catch (e) {
      toast.error(e?.response?.data?.detail ?? 'Création de CAPA impossible.')
    }
  }
  ...
  return (
    <div className="page flex flex-col gap-4">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="preparation" className="mt-4 flex flex-col gap-6">
          ...
          <QhseResourceList
            ...
            deps={[exercicesReload]}
            actions={
              <Button onClick={() => setPlanifiantExercice(true)}>
                <Plus size={16} /> Planifier un exercice
              </Button>
            }
            rowActions={(r) => [
              ...(r.statut === 'planifie'
                ? [{ id: 'realiser', label: 'Réaliser', icon: CheckCircle2, onClick: () => setRealisantExercice(r) }]
                : []),
              ...(r.statut === 'realise'
                ? [{ id: 'creer-capa', label: 'Créer CAPA', icon: ShieldAlert, onClick: () => creerCapaExercice(r) }]
                : []),
            ]}
          />
        </TabsContent>
      ...
      {planifiantExercice && (
        <PlanifierExerciceDialog
          onClose={() => setPlanifiantExercice(false)}
          onDone={() => setExercicesReload((n) => n + 1)}
        />
      )}
      {realisantExercice && (
        <RealiserExerciceDialog
          exercice={realisantExercice}
          onClose={() => setRealisantExercice(null)}
          onDone={() => setExercicesReload((n) => n + 1)}
        />
      )}
    </div>
