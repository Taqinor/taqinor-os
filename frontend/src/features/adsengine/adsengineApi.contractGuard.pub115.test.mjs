...
function extractActions(src) {
  ...
  while ((m = re.exec(src))) {
    ...
    const urlPathMatch = argsBlob.match(/url_path\s*=\s*'([^']+)'/)
...
function resolvable(path) {
  ...
  if (routerPrefixes.has(first)) {
    ...
    // Formes irrégulières (ex. un sous-chemin à 3 segments) : aucune route de
    // ce type n'existe aujourd'hui dans ce ViewSet router — repli légitime
    // sur les patterns directs ci-dessous (généralement sans match, ce qui
    // est CORRECT tant qu'aucune telle route n'a été construite).
  }
...
const KNOWN_GAPS = new Map([
  ['/adsengine/signaux/', 'PUB1'],
  ['/adsengine/signaux/cohorte/', 'PUB1'],
  ['/adsengine/noeuds-hypothese/file-voi/', 'PUB2'],
  ['/adsengine/noeuds-hypothese/1/tests/', 'PUB2'],
  ['/adsengine/noeuds-hypothese/tests/1/leads/', 'PUB2'],
])
