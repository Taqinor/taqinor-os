...
// PUB11 — carte de croyance FR d'un nœud d'hypothèse : simple AFFICHAGE du
// modèle Beta-Bernoulli exposé par l'API (moyenne postérieure + nombre
// d'observations = (α+β) − prior), jamais un score recalculé. `null` si les
// compteurs ne sont pas exploitables (jamais un pourcentage fabriqué).
...
// PUB6 — diff pur entre deux jeux d'entrées de la table des faits (comparaison
// par `cle`) — aucune valeur inventée, seulement une comparaison des chiffres
// déjà chargés depuis l'API.
...
// PUB54 — contenu des info-bulles pédagogiques FR (« ? ») par métrique
// (MetricHelp.jsx ne garde que le composant — convention only-export-components).
