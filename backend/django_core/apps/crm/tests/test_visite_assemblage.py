...
class AssemblageBase(TestCase):
    ...
    def poster_photo(self, nom, slot='toiture_vue_generale'):
        ...
        resp = self.api.post(
            f'/api/django/crm/visites/{self.visite_id}/photos/',
...
class MachineEtatsTests(AssemblageBase):
    def test_etat_initial_est_aucun(self):
        detail = self.api.get(f'/api/django/crm/visites/{self.visite_id}/')
    ...
    def test_recouvrement_insuffisant_donne_un_message_honnete(self):
        ...
        with mock.patch('apps.crm.tasks._assembler',
    ...
    def test_assemblage_reussi_pose_la_cle_et_lurl(self):
        ...
        self.assertEqual(
            ...
            f'/api/django/crm/visites/{self.visite_id}/photo-toit/')
...
class ActionApiTests(AssemblageBase):
    def test_laction_lance_la_tache_et_le_get_sert_de_polling(self):
        ...
        self.poster_photo('toit-2.png')
        with mock.patch(
                'apps.crm.tasks.assembler_photos_toit_task.delay') as envoi:
            resp = self.api.post(
                f'/api/django/crm/visites/{self.visite_id}/assembler-photos/',
                {}, format='json')
        self.assertEqual(resp.status_code, 200, resp.data)
    ...
    def test_calage_exige_quatre_coins(self):
        resp = self.api.patch(
            f'/api/django/crm/visites/{self.visite_id}/calage/',
