"""ARC2 — pilote ``ClientViewSet`` converti à ``CompanyScopedModelViewSet``.

``apps/crm/views.py`` re-codait son propre scoping ; il hérite désormais de la
base transverse unique tout en CONSERVANT son ``get_queryset`` (portée de
visibilité) et son ``perform_create`` (company + created_by forcés serveur).
Ce test prouve que le comportement est INCHANGÉ : (a) la société B ne voit
jamais un client de la société A (liste + détail 404, jamais 403) ; (b)
``company`` reste forcée côté serveur ; (c) la matrice de droits est identique
(lecture tout rôle, écriture responsable/admin, suppression admin).
"""
...
class TestClientIsolationARC2(TestCase):
    ...
    def test_list_scoped_to_company(self):
        """La société A ne voit PAS le client de la société B."""
    ...
    def test_detail_of_other_company_is_404(self):
        ...
        r = _api(self.admin_a).get(f'{URL}{self.client_b.id}/')
...
class TestClientPermissionsARC2(TestCase):
    """La matrice de droits (401/403) reste identique après ARC2."""

    def setUp(self):
        self.company, _ = Company.objects.get_or_create(
            slug='arc2-client-perm', defaults={'nom': 'Société P'})
    ...
    def test_non_privileged_cannot_write(self):
        ...
        member = CustomUser.objects.create_user(
            username='arc2_client_member', password='x',
