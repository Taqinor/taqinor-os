...
# ── FG135/136 + ZACC6/XACC27/XACC28 — NOTES DE FRAIS (déplacé ODX15) ───────
# Ces modèles vivent désormais dans ``apps.frais`` (équivalent Odoo Expenses).
# ODX15 les a sortis de compta en préservant à l'IDENTIQUE leurs tables
# (``db_table = 'compta_<model>'``) via des migrations
# ``SeparateDatabaseAndState`` (state-only, zéro SQL, zéro donnée déplacée).
# Ce ré-export garde fonctionnels le code et les migrations historiques
# (``from apps.compta.models import NoteFrais``, serializers, views, admin,
# tests FG135/136) ; le POSTING COMPTABLE, lui, RESTE ici
# (``apps.compta.services`` — écritures 6143/4432/trésorerie + verrou de
# période FG115), c'est la frontière voulue.
from apps.frais.models import (  # noqa: E402,F401
    BaremeIndemnite,
    IndemniteChantier,
    NoteFrais,
    PlafondNoteFrais,
