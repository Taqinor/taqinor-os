...
# ── FG209–221 — CONFIGURATION DE VENTE — SORTIE (ODX14) ────────────────────
# Ces modèles (CodePromotion, ModeleDevis, SessionGuidedSelling,
# DemandeApprobationConfig, ECatalogue, DocumentProposition, SimulationPublique,
# SimulationFinancement, OffreFinancement, LigneIncitation, EcheancierPaiement,
# TranchePaiement — FG209–221) vivent désormais dans ``apps.ventes`` (chez Odoo :
# Sales — quotation templates/pricelists/online quotes). ODX14 les a sortis de
# compta en préservant à l'IDENTIQUE leurs tables (``db_table = 'compta_<model>'``)
# via des migrations ``SeparateDatabaseAndState`` (state-only, zéro SQL). Ce
# ré-export garde le code/serializers/services/migrations historiques
# (``from apps.compta.models import CodePromotion``…) fonctionnels ; à retirer en
# ODX22 une fois tous les appelants re-pointés sur ``apps.ventes``. Invariants :
# /proposal reste l'unique voie PDF devis (DocumentProposition = ANNEXE, jamais
# un 2ᵉ chemin, règle #4) ; l'e-catalogue n'expose jamais prix_achat.
from apps.ventes.models import (  # noqa: E402,F401
    CodePromotion,
