...
FG131 — Rapprochement 3 voies (BC ↔ réception ↔ facture fournisseur) ajouté
en bas de ce fichier. Cf. section ``# ── FG131``.
...
#
# Trois tables « source » (BC, réception, facture) portent les quantités/montants
# en jeu. Le ``Rapprochement3Voies`` en est l'agrégat de contrôle : il calcule les
# écarts entre ce qui était commandé, ce qui a été reçu et ce qui a été facturé par
# le fournisseur, et BLOQUE le paiement tant que le statut n'est pas ``approuve``.
# Tous les montants AP (facture fournisseur) étaient jusqu'ici saisis à la main ;
# ce modèle structure cette saisie et l'expose au contrôle 3 voies.

class BonCommandeFournisseur(models.Model):
    """Bon de commande fournisseur (BC) — premier pilier du rapprochement 3 voies.

    Porte la commande émise à un fournisseur : référence, date, tiers (string-FK
    pour ne pas importer un autre domaine) et ses ``LigneBonCommandeFournisseur``.
    ``company`` posée côté serveur.
    """
    class Statut(models.TextChoices):
        BROUILLON = 'brouillon', 'Brouillon'
        ENVOYE = 'envoye', 'Envoyé'
        PARTIELLEMENT_RECU = 'partiellement_recu', 'Partiellement reçu'
        RECU = 'recu', 'Reçu'
    ...
    reference = models.CharField(
        max_length=80, blank=True, default='', verbose_name='Référence BC')
    date_commande = models.DateField(verbose_name='Date de commande')
    # Tiers fournisseur en string-FK — jamais d'import cross-app de modèle.
    fournisseur_type = models.CharField(
        max_length=30, blank=True, default='',
    ...
    fournisseur_nom = models.CharField(
        max_length=200, blank=True, default='', verbose_name='Nom fournisseur')
    statut = models.CharField(
        max_length=20, choices=Statut.choices,
        default=Statut.BROUILLON, verbose_name='Statut')
    # Montant total HT figé après envoi (calculé depuis les lignes).
    ...
    def __str__(self):
        return f'BC {self.reference or self.id} ({self.date_commande})'

    @property
    def montant_ttc(self):
        ht = self.montant_ht or Decimal('0')
        taux = self.taux_tva or Decimal('0')
        return (ht * (1 + taux / Decimal('100'))).quantize(Decimal('0.01'))

    def recalculer_montant_ht(self):
        """Recalcule ``montant_ht`` depuis les lignes et sauvegarde."""
        total = sum(
            (lig.quantite or Decimal('0')) * (lig.prix_unitaire_ht or Decimal('0'))
            for lig in self.lignes_bc.all()
        ) or Decimal('0')
        self.montant_ht = Decimal(str(total)).quantize(Decimal('0.01'))
        self.save(update_fields=['montant_ht'])


class LigneBonCommandeFournisseur(models.Model):
    """Ligne d'un bon de commande fournisseur (article, qté, PU HT)."""
    company = models.ForeignKey(
        'authentication.Company',
        on_delete=models.CASCADE,
    ...
    # String-FK optionnel vers un produit du catalogue (jamais import cross-app).
    produit_id = models.PositiveIntegerField(
        null=True, blank=True, verbose_name='ID produit (catalogue)')
    ...
    def clean(self):
        ...
        if self.prix_unitaire_ht is not None and self.prix_unitaire_ht < 0:
            raise ValidationError(
                "Le prix unitaire HT doit être positif.")

    @property
    def montant_ligne_ht(self):
        qte = self.quantite or Decimal('0')
        pu = self.prix_unitaire_ht or Decimal('0')
        return (qte * pu).quantize(Decimal('0.01'))


class ReceptionMarchandise(models.Model):
    """Réception de marchandise fournisseur — deuxième pilier du rapprochement 3 voies.

    Constate la LIVRAISON physique d'un BC : ce qui a réellement été reçu,
    à quelle date, en quel état (``statut``). Porte ses
    ``LigneReceptionMarchandise``. Scopée société, tiers en string-FK.
    """
    class Statut(models.TextChoices):
        PARTIELLE = 'partielle', 'Partielle'
        COMPLETE = 'complete', 'Complète'
        AVEC_RESERVES = 'avec_reserves', 'Complète avec réserves'
    ...
    reference = models.CharField(
        max_length=80, blank=True, default='', verbose_name='Référence réception')
    date_reception = models.DateField(verbose_name='Date de réception')
    statut = models.CharField(
        max_length=15, choices=Statut.choices,
        default=Statut.COMPLETE, verbose_name='Statut')
    # Bon de livraison fournisseur (numéro du BL reçu).
    numero_bl_fournisseur = models.CharField(
        max_length=80, blank=True, default='', verbose_name='N° BL fournisseur')
    notes = models.TextField(blank=True, default='', verbose_name='Notes / réserves')
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True, blank=True,
    ...
    def __str__(self):
        return (f'Réception {self.reference or self.id} '
                f'BC {self.bon_commande_id} ({self.date_reception})')

    def clean(self):
        super().clean()
        if (self.company_id and self.bon_commande_id
                and self.bon_commande.company_id != self.company_id):
            raise ValidationError(
                "Le bon de commande appartient à une autre société.")


class LigneReceptionMarchandise(models.Model):
    """Ligne de réception : quantité effectivement reçue pour un article du BC."""
    company = models.ForeignKey(
        'authentication.Company',
        on_delete=models.CASCADE,
    ...
    notes = models.CharField(
        max_length=255, blank=True, default='', verbose_name='Notes / réserves')
    ...
    def __str__(self):
        return f'{self.ligne_bc.designation} reçu × {self.quantite_recue}'
...
class FactureFournisseur(models.Model):
    """Facture fournisseur (AP) — troisième pilier du rapprochement 3 voies.

    Saisie aujourd'hui À LA MAIN (montants et lignes saisis par l'équipe compta
    à la réception de la facture papier). Ce modèle structure cette saisie pour
    la comparer au BC et à la réception dans le ``Rapprochement3Voies``.

    Le tiers est en string-FK. ``company`` posée côté serveur. Strictement additif.
    """
    class Statut(models.TextChoices):
        BROUILLON = 'brouillon', 'Brouillon'
    ...
    statut = models.CharField(
        max_length=15, choices=Statut.choices,
        default=Statut.BROUILLON, verbose_name='Statut')
    # Montants saisis à la main (reflètent exactement la facture papier).
    montant_ht = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal('0'),
    ...
    class Meta:
        ...
        constraints = [
            # Référence unique par société (une facture fournisseur ne se duplique
            # pas dans les AP d'une même société).
            models.UniqueConstraint(
    ...
    def __str__(self):
        return f'Facture {self.reference} ({self.date_facture})'

    def clean(self):
        super().clean()
        if self.montant_ht is not None and self.montant_ht < 0:
            raise ValidationError(
                "Le montant HT doit être positif.")
        if (self.date_facture and self.date_echeance
                and self.date_echeance < self.date_facture):
            raise ValidationError(
                "L'échéance doit être postérieure ou égale à la date de facture.")

    def recalculer_montants(self):
        """Recalcule montant_tva et montant_ttc depuis montant_ht + taux_tva."""
        ht = self.montant_ht or Decimal('0')
        taux = self.taux_tva or Decimal('0')
        tva = (ht * taux / Decimal('100')).quantize(Decimal('0.01'))
        self.montant_tva = tva
        self.montant_ttc = (ht + tva).quantize(Decimal('0.01'))
        self.save(update_fields=['montant_tva', 'montant_ttc'])


class LigneFactureFournisseur(models.Model):
    """Ligne de facture fournisseur : désignation, qté facturée, PU HT facturé."""
    company = models.ForeignKey(
        'authentication.Company',
        on_delete=models.CASCADE,
    ...
    # Lien optionnel vers la ligne du BC correspondante (facilite l'appariement
    # automatique). NULL si la ligne ne correspond à aucune commande.
    ligne_bc = models.ForeignKey(
        LigneBonCommandeFournisseur,
        on_delete=models.SET_NULL,
        null=True, blank=True,
    ...
    designation = models.CharField(max_length=255, verbose_name='Désignation')
    quantite_facturee = models.DecimalField(
        max_digits=12, decimal_places=4, default=Decimal('0'),
    ...
    def __str__(self):
        return f'{self.designation} facturé × {self.quantite_facturee}'

    def clean(self):
        super().clean()
        if self.quantite_facturee is not None and self.quantite_facturee < 0:
            raise ValidationError(
                "La quantité facturée ne peut pas être négative.")
        if self.prix_unitaire_ht is not None and self.prix_unitaire_ht < 0:
            raise ValidationError(
                "Le prix unitaire HT facturé doit être positif.")

    @property
    def montant_ligne_ht(self):
        qte = self.quantite_facturee or Decimal('0')
        pu = self.prix_unitaire_ht or Decimal('0')
        return (qte * pu).quantize(Decimal('0.01'))


class Rapprochement3Voies(models.Model):
    """Rapprochement 3 voies (BC ↔ réception ↔ facture fournisseur) — FG131.

    Compare les montants/quantités COMMANDÉS (BC), REÇUS (réception) et FACTURÉS
    (facture fournisseur AP) avant de VALIDER un paiement. Le statut passe
    ``approuve`` UNIQUEMENT si tous les écarts sont dans la tolérance et que le
    contrôleur a validé. Tant qu'il n'est pas ``approuve``, le paiement de la
    facture AP est BLOQUÉ.

    Les montants commandés/reçus/facturés sont figés sur le rapprochement au
    moment de la validation (``valider_rapprochement``) ; les écarts sont calculés
    côté serveur et figés eux aussi. Strictement additif ; ``company`` posée côté
    serveur.
    """
    class Statut(models.TextChoices):
        EN_COURS = 'en_cours', 'En cours de contrôle'
        APPROUVE = 'approuve', 'Approuvé — paiement autorisé'
        REJETE = 'rejete', 'Rejeté — écart non toléré'
    ...
    statut = models.CharField(
        max_length=12, choices=Statut.choices,
        default=Statut.EN_COURS, verbose_name='Statut')
    # Montants figés à la validation (résumé des trois pilliers).
    montant_commande_ht = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal('0'),
        verbose_name='Montant commandé HT (BC)')
    montant_recu_ht = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal('0'),
    ...
    # Écarts figés côté serveur (positif = fournisseur facture plus que commandé).
    ecart_commande_facture_ht = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal('0'),
        verbose_name='Écart facturé − commandé HT')
    ecart_recu_facture_ht = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal('0'),
        verbose_name='Écart facturé − reçu HT')
    # Tolérance configurée par le contrôleur (valeur absolue en MAD HT).
    tolerance_ht = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal('0'),
        verbose_name='Tolérance (MAD HT)')
    notes = models.TextField(blank=True, default='', verbose_name='Notes du contrôleur')
    ...
    def __str__(self):
        return (f'Raprochement 3V {self.id} — '
                f'BC {self.bon_commande_id} / '
                f'Fact. {self.facture.reference} '
                f'({self.get_statut_display()})')

    def clean(self):
        super().clean()
        if self.company_id:
            if (self.bon_commande_id
                    and self.bon_commande.company_id != self.company_id):
                raise ValidationError(
                    "Le bon de commande appartient à une autre société.")
            if (self.reception_id
                    and self.reception.company_id != self.company_id):
                raise ValidationError(
                    "La réception appartient à une autre société.")
            if (self.facture_id
                    and self.facture.company_id != self.company_id):
                raise ValidationError(
                    "La facture fournisseur appartient à une autre société.")
        if (self.reception_id and self.bon_commande_id
                and self.reception.bon_commande_id != self.bon_commande_id):
            raise ValidationError(
                "La réception ne correspond pas au bon de commande.")

    @property
    def est_approuve(self):
        return self.statut == self.Statut.APPROUVE

    @property
    def paiement_bloque(self):
        """Vrai tant que le rapprochement n'est pas approuvé."""
        return self.statut != self.Statut.APPROUVE

    @property
    def ecarts_dans_tolerance(self):
        """Vrai si les deux écarts HT sont dans la tolérance configurée."""
        tol = self.tolerance_ht or Decimal('0')
        return (abs(self.ecart_commande_facture_ht) <= tol
                and abs(self.ecart_recu_facture_ht) <= tol)
