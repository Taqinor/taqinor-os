...
from .models import (
    ...
    ClotureCaisse, CompteComptable, DotationAmortissement, EcritureComptable,
    Effet, ExerciceComptable, FactureFournisseur, Immobilisation, Journal,
    LigneBonCommandeFournisseur, LigneEcriture, LigneFactureFournisseur,
    ...
    PointageReleve, Rapprochement3Voies, RapprochementBancaire,
...
@transaction.atomic
def creer_bon_commande(company, date_commande, lignes, *,
                       reference='', fournisseur_nom='',
                       fournisseur_type='', fournisseur_id=None,
                       taux_tva=None, notes='', created_by=None):
    """Crée un ``BonCommandeFournisseur`` avec ses lignes et calcule le HT.

    ``lignes`` : liste de dicts ``{designation, quantite, prix_unitaire_ht,
    unite?}``.
    """
    bc = BonCommandeFournisseur(
        ...
        taux_tva=Decimal(str(taux_tva)) if taux_tva is not None else Decimal('20.00'),
        notes=notes,
        created_by=created_by,
    )
    bc.save()
    total_ht = Decimal('0')
    for lig in lignes:
        qte = Decimal(str(lig['quantite']))
        pu = Decimal(str(lig['prix_unitaire_ht']))
        ligne = LigneBonCommandeFournisseur(
            company=company,
            bon_commande=bc,
            designation=lig['designation'],
            quantite=qte,
            prix_unitaire_ht=pu,
            unite=lig.get('unite', '') or '',
            produit_id=lig.get('produit_id'),
        )
        ligne.clean()
        ligne.save()
        total_ht += (qte * pu).quantize(Decimal('0.01'))
    bc.montant_ht = total_ht
    bc.save(update_fields=['montant_ht'])
    return bc


@transaction.atomic
def creer_reception(company, bon_commande, date_reception, lignes, *,
                    reference='', statut=None, numero_bl_fournisseur='',
                    notes='', created_by=None):
    """Crée une ``ReceptionMarchandise`` pour un BC avec ses lignes de réception.

    ``lignes`` : liste de dicts ``{ligne_bc_id, quantite_recue, notes?}``.
    Valide que toutes les lignes du BC appartiennent bien à la même société.
    """
    if bon_commande.company_id != company.id:
        raise ValidationError(
    ...
    rec = ReceptionMarchandise(
        ...
        statut=statut or ReceptionMarchandise.Statut.COMPLETE,
    ...
    rec.save()
    for lig in lignes:
        ligne_bc = LigneBonCommandeFournisseur.objects.filter(
            id=lig['ligne_bc_id'], company=company,
            bon_commande=bon_commande).first()
        if ligne_bc is None:
            raise ValidationError(
                f"Ligne BC {lig['ligne_bc_id']} inconnue ou hors périmètre.")
        lr = LigneReceptionMarchandise(
            company=company,
            reception=rec,
            ligne_bc=ligne_bc,
            quantite_recue=Decimal(str(lig['quantite_recue'])),
            notes=lig.get('notes', '') or '',
        )
        lr.clean()
        lr.save()
    return rec
...
@transaction.atomic
def creer_facture_fournisseur(company, reference, date_facture, montant_ht, *,
                              taux_tva=None, date_echeance=None,
                              fournisseur_nom='', fournisseur_type='',
                              fournisseur_id=None, notes='',
                              lignes=None, created_by=None):
    """Crée une ``FactureFournisseur`` et calcule TVA + TTC.

    ``lignes`` : liste optionnelle de dicts ``{designation, quantite_facturee,
    prix_unitaire_ht, ligne_bc_id?, unite?}``. Montants saisie à la main.
    """
    taux = Decimal(str(taux_tva)) if taux_tva is not None else Decimal('20.00')
    ...
    facture.clean()
    facture.save()
    for lig in (lignes or []):
        lf = LigneFactureFournisseur(
            company=company,
            facture=facture,
            designation=lig['designation'],
            quantite_facturee=Decimal(str(lig['quantite_facturee'])),
            prix_unitaire_ht=Decimal(str(lig['prix_unitaire_ht'])),
            unite=lig.get('unite', '') or '',
            ligne_bc_id=lig.get('ligne_bc_id'),
        )
        lf.clean()
        lf.save()
    return facture
...
def _montant_recu_ht(reception):
    """Calcule le montant HT total reçu = Σ(qté reçue × PU HT du BC).

    Utilise le prix unitaire du BC (contrat négocié), pas celui de la facture.
    """
    total = Decimal('0')
    for lr in reception.lignes_reception.select_related('ligne_bc').all():
        pu = lr.ligne_bc.prix_unitaire_ht or Decimal('0')
        total += (lr.quantite_recue * pu).quantize(Decimal('0.01'))
    return total


@transaction.atomic
def creer_rapprochement_3voies(company, bon_commande, reception, facture, *,
                                libelle='', tolerance_ht=None,
                                notes='', created_by=None):
    """Crée un ``Rapprochement3Voies`` et calcule les montants/écarts.

    Valide la cohérence société + BC/réception correspondance avant création.
    Les montants commandé/reçu/facturé et les écarts sont figés côté serveur.
    Le rapprochement est créé en statut ``en_cours`` ; il faut ensuite appeler
    ``valider_rapprochement`` pour l'approuver ou le rejeter.
    """
    # Vérifications inter-société.
    if bon_commande.company_id != company.id:
        raise ValidationError(
            "Le bon de commande appartient à une autre société.")
    if reception.company_id != company.id:
        raise ValidationError(
    ...
    if reception.bon_commande_id != bon_commande.id:
        raise ValidationError(
            "La réception ne correspond pas au bon de commande sélectionné.")

    montant_cmd = bon_commande.montant_ht or Decimal('0')
    montant_recu = _montant_recu_ht(reception)
    montant_fact = facture.montant_ht or Decimal('0')
    ecart_cmd_fact = (montant_fact - montant_cmd).quantize(Decimal('0.01'))
    ecart_recu_fact = (montant_fact - montant_recu).quantize(Decimal('0.01'))

    rap = Rapprochement3Voies(
        company=company,
        bon_commande=bon_commande,
        reception=reception,
        facture=facture,
        libelle=libelle,
        montant_commande_ht=montant_cmd,
        montant_recu_ht=montant_recu,
        montant_facture_ht=montant_fact,
        ecart_commande_facture_ht=ecart_cmd_fact,
        ecart_recu_facture_ht=ecart_recu_fact,
        tolerance_ht=Decimal(str(tolerance_ht)) if tolerance_ht is not None
        else Decimal('0'),
        notes=notes,
        created_by=created_by,
    )
...
@transaction.atomic
def valider_rapprochement_3voies(rap, *, approuver, user=None, notes=''):
    """Approuve ou rejette un ``Rapprochement3Voies``.

    ``approuver=True`` : vérifie que les écarts sont dans la tolérance et
    passe le statut à ``approuve`` ; ``approuver=False`` le rejette (``rejete``).
    Dans les deux cas, ``valide_par`` et ``date_validation`` sont figés.

    Lève ``ValidationError`` si on tente d'approuver un rapprochement dont
    les écarts dépassent la tolérance.
    """
    if rap.statut != Rapprochement3Voies.Statut.EN_COURS:
        raise ValidationError(
            "Seul un rapprochement en cours peut être approuvé ou rejeté.")
    if approuver and not rap.ecarts_dans_tolerance:
        tol = rap.tolerance_ht
        raise ValidationError(
            f"Les écarts dépassent la tolérance de {tol} MAD HT : "
            f"écart commandé/facturé = {rap.ecart_commande_facture_ht}, "
            f"écart reçu/facturé = {rap.ecart_recu_facture_ht}. "
            "Rejetez le rapprochement ou ajustez la tolérance.")
    if notes:
        rap.notes = notes
    rap.valide_par = user
    rap.date_validation = timezone.now()
    rap.statut = (Rapprochement3Voies.Statut.APPROUVE if approuver
                  else Rapprochement3Voies.Statut.REJETE)
    rap.save(update_fields=[
        'statut', 'valide_par', 'date_validation', 'notes'])
    return rap
