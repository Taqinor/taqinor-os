...
Parcourt LA chaîne réelle, par les endpoints existants :
  ...
  4. Règlement (PaiementFournisseur) → facture PAYÉE.

SOLMVP12 (20/09/2026) — les assertions sur l'écriture comptable générée
(lecture du module compta, détaché de stock) ont été retirées : ce module ne
couvre plus que le côté STOCK de la chaîne (le bus d'événements YLEDG2 reste
inchangé, la comptabilité a ses propres tests).

Frontières : installations UNIQUEMENT via ``apps.installations.selectors``
— jamais un import des models d'une autre app.
...
class TestProcessP2P(TenantAPITestCase):
    ...
    def test_p2p_bc_reception_facture_reglement(self):
        ...
        # 4) Règlement intégral → facture payée.
