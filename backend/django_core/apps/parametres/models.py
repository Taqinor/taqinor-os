...
# ── Journal d'audit des Paramètres (N55) — ADDITIF ───────────────────────────
# Trace chaque modification de la configuration (qui a changé quoi, ancien→
# nouveau, quand). Réutilise le motif d'audit établi (stock.ProduitAuditLog +
# chatter CRM) : société scopée, utilisateur côté serveur, champ + ancienne/
# nouvelle valeur en texte. LECTURE SEULE côté API ; l'écriture se fait dans
# les chemins d'enregistrement des paramètres (CompanyProfile + messages).
class SettingsAuditLog(models.Model):
    """Une ligne par changement de paramètre (company-scopée).
    ...
    (str() de la valeur), comme ProduitAuditLog. Aucune donnée sensible de
    prix d'achat / marge n'est concernée par les paramètres."""
    ...
    @classmethod
    def log_change(cls, company, user, section, field, field_label,
                   old, new):
        """Écrit une ligne d'audit (ancien→nouveau) en texte.

        Suit le motif `ProduitAuditLog._log` : valeurs converties en str,
        utilisateur uniquement s'il est authentifié."""
