...
def build(ctx) -> str:
  ...
  <div class="a4-msg">
    <div class="a4-punch"><b>« Le butane est bon marché tant qu'il est subventionné.
      ...
      votre facture va plus que doubler. Le solaire supprime ce coût et ce risque.</div>
    {_env_block(show_env, co2_t, trees, fuel_qty, C, fmt, fmt_dec)}
  </div>
"""
    elif not has_water:
    ...
    steps_html = "".join(
        ...
        f'<span class="a4-stt">{t}</span><span class="a4-sts">{s}</span></div>'
        for n, t, s in steps)
...
.a4-step{{display:flex;align-items:baseline;gap:7px;padding:6px 0;border-bottom:1px dashed {line_soft};}}
...
.a4-sn{{width:16px;min-width:16px;height:16px;border-radius:50%;background:{navy};color:#fff;
  font-size:7.5pt;font-weight:700;text-align:center;line-height:16px;}}
.a4-stt{{font-size:8.2pt;color:{ink};font-weight:700;}}
.a4-sts{{font-size:7.4pt;color:{muted};margin-left:auto;}}
