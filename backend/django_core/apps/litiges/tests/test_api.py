"""Tests API des Réclamations & litiges.

Couvre : société posée côté serveur (jamais du corps), isolation entre sociétés
(A ne voit pas les réclamations de B), et accès réservé au palier
...
class LitigesApiTests(TestCase):
    BASE = '/api/django/litiges/reclamations/'

    def setUp(self):
        self.co_a = make_company('litiges-a', 'A')
        self.co_b = make_company('litiges-b', 'B')
        self.user_a = make_user(self.co_a, 'litiges-a')
        self.user_b = make_user(self.co_b, 'litiges-b')

    def _payload(self):
        return {'objet': 'Facture contestée'}
    ...
    def test_list_isolation(self):
        Reclamation.objects.create(company=self.co_a, **self._model_kwargs())
    ...
    def test_role_normal_refuse(self):
        normal = make_user(self.co_a, 'litiges-normal', role='normal')
