...
from apps.assurances.models import Assureur, PoliceActivity, PoliceAssurance
...
class PoliceChatterTests(TestCase):
    ...
    def test_creation_loggee(self):
        ...
        act = PoliceActivity.objects.filter(
            police_id=resp.data['id'], kind=PoliceActivity.Kind.CREATION)
    ...
    def test_renouvellement_change_date_echeance_loggue_le_chatter(self):
        ...
        entree = PoliceActivity.objects.get(
            police=self.police, champ='date_echeance')
        self.assertEqual(entree.ancienne_valeur, str(ancienne_echeance))
        self.assertEqual(entree.nouvelle_valeur, str(nouvelle_echeance))

        resp = api.get(
            f'/api/django/assurances/polices/{self.police.id}/historique/')
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(any(
            e['champ'] == 'date_echeance' for e in resp.data))
    ...
    def test_noter_ajoute_une_note_manuelle(self):
        ...
        self.assertEqual(resp.status_code, 201, resp.data)
        self.assertEqual(
            PoliceActivity.objects.filter(
                police=self.police, kind=PoliceActivity.Kind.NOTE).count(), 1)
