...
# ── VX61 — Web Vitals RÉELS captés terrain (INP/LCP/CLS/TTFB) ───────────────

class VitalMetric(models.Model):
    """VX61 — une mesure Web Vital envoyée par le beacon front (une ligne par
    métrique par navigation). ``company`` posée CÔTÉ SERVEUR depuis
    l'utilisateur authentifié (jamais lue du corps de requête) ; anonyme
    (utilisateur non authentifié) accepté avec ``company=None`` pour ne
    jamais perdre un signal terrain (ex. avant connexion).

    Croissance rapide attendue (une ligne par métrique par navigation) —
    ENREGISTRÉE au registre de rétention partagé YOPSB10 (voir
    ``apps.py ready()``), purge programmée."""

    class Metrique(models.TextChoices):
        INP = 'INP', 'INP'
        LCP = 'LCP', 'LCP'
        CLS = 'CLS', 'CLS'
        TTFB = 'TTFB', 'TTFB'
    ...
    company = models.ForeignKey(
        'authentication.Company', on_delete=models.CASCADE,
        null=True, blank=True, related_name='vital_metrics')
    name = models.CharField(max_length=10, choices=Metrique.choices)
    ...
    class Meta:
        ...
        ordering = ['-created_at', '-id']
        indexes = [
            models.Index(fields=['company', 'name', 'created_at']),
        ]
