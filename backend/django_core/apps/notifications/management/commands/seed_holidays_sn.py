...
Usage :
    python manage.py seed_holidays_sn
    python manage.py seed_holidays_sn --company-id 3   # une société précise
...
Fêtes MOBILES (islamiques : Korité/Aïd el-Fitr, Tabaski/Aïd el-Adha,
Maouloud, Tamkharit ; chrétiennes calées sur Pâques : Lundi de Pâques,
Ascension, Lundi de Pentecôte) : DÉLIBÉRÉMENT EXCLUES du seed automatique —
même politique que ``seed_ma_holidays`` — et doivent être saisies
manuellement via l'API ou l'admin.
"""
import datetime
import logging
...
# 6 jours fériés FIXES sénégalais (mois, jour, libellé).
SN_FIXED_HOLIDAYS = [
    (1, 1,  "Jour de l'An"),
...
class Command(BaseCommand):
    help = (
        "Seed les 6 jours fériés fixes sénégalais pour toutes les sociétés "
    ...
    def handle(self, *args, **options):
        ...
        for company in companies:
            for month, day, name in SN_FIXED_HOLIDAYS:
        ...
        self.stdout.write(self.style.SUCCESS(
            f"seed_holidays_sn : {created_total} jour(s) créé(s), "
