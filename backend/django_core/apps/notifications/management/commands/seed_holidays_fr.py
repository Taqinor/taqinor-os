...
Usage :
    python manage.py seed_holidays_fr
    python manage.py seed_holidays_fr --company-id 3   # une société précise
...
Fêtes chrétiennes MOBILES calées sur Pâques (Lundi de Pâques, Ascension,
Lundi de Pentecôte) : DÉLIBÉRÉMENT EXCLUES du seed automatique — même
politique que les fêtes islamiques du Maroc (``seed_ma_holidays``) — et
doivent être saisies manuellement via l'API ou l'admin.
"""
import datetime
import logging
...
# 8 jours fériés FIXES français (mois, jour, libellé) — Code du travail,
# art. L3133-1. Source : https://www.service-public.fr/particuliers/vosdroits/F2405
FR_FIXED_HOLIDAYS = [
    (1, 1,  "Jour de l'An"),
    (5, 1,  "Fête du Travail"),
    (5, 8,  "Victoire 1945"),
...
class Command(BaseCommand):
    help = (
        "Seed les 8 jours fériés fixes français pour toutes les sociétés "
    ...
    def handle(self, *args, **options):
        ...
        for company in companies:
            for month, day, name in FR_FIXED_HOLIDAYS:
        ...
        self.stdout.write(self.style.SUCCESS(
            f"seed_holidays_fr : {created_total} jour(s) créé(s), "
            f"{skipped_total} déjà présent(s). "
            f"Note : les fêtes chrétiennes mobiles (Pâques, Ascension, "
            f"Pentecôte) doivent être saisies manuellement."
        ))
