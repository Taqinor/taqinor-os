...
class WebPushTests(TestCase):
    ...
    def test_dispatch_webpush_actually_signs_with_autogenerated_key(self):
        """RÉGRESSION : un push réel doit pouvoir être SIGNÉ + chiffré + envoyé
        avec la clé VAPID auto-générée (stockée en PEM).

        Le bug de prod : on passait le TEXTE du PEM à
        ``pywebpush.webpush(vapid_private_key=...)`` qui ne sait pas le
        désérialiser (« Could not deserialize key data / ASN.1 invalid length »)
        → CHAQUE push échouait, sur TOUS les appareils, sans qu'aucun test ne le
        voie (les autres tests ne couvrent que le no-op). Ici on génère un VRAI
        abonnement (clés client valides), on mocke UNIQUEMENT le POST réseau, et
        on exige que l'envoi réussisse (``sent == 1``) : si la clé VAPID ne peut
        pas être chargée, ``webpush`` lève à la signature et ``sent`` reste 0."""
        import base64
        import os
        ...
        # Abonnement client réaliste : p256dh = point EC non compressé (base64url),
        # auth = 16 octets aléatoires — nécessaires au chiffrement du payload.
        ...
        PushSubscription.objects.create(
            company=self.company, user=self.user,
            endpoint='https://web.push.apple.com/regression-test',
            p256dh=p256dh, auth=auth)

        # On mocke SEULEMENT l'appel réseau : la signature VAPID + le chiffrement
        # (là où était le bug) s'exécutent réellement.
        resp = mock.MagicMock(status_code=201)
        with mock.patch('requests.post', return_value=resp) as mpost:
            sent = _dispatch_webpush(
                self.user, 'Nouveau lead', 'Bennani — Casablanca',
                link='/leads/1')

        self.assertEqual(
            sent, 1,
            "le push VAPID auto-généré doit pouvoir être signé et envoyé "
            "(régression « Could not deserialize key data »)")
