"""NTAPI30 — Export live vers Google Sheets `GET /api/public/v1/exports/<entite>.csv?token=<clé>`.
...
class Ntapi30CsvPullExportTests(TestCase):
    ...
    def test_pull_returns_csv_of_own_company(self):
        ...
        resp = APIClient().get(f'/api/public/v1/exports/leads.csv?token={self.raw}')
    ...
    def test_missing_token_is_401(self):
        resp = APIClient().get('/api/public/v1/exports/leads.csv')
        self.assertEqual(resp.status_code, 401)

    def test_invalid_token_is_401(self):
        resp = APIClient().get('/api/public/v1/exports/leads.csv?token=bogus')
        self.assertEqual(resp.status_code, 401)
    ...
    def test_wrong_scope_is_403(self):
        ...
        # read:stock requis).
        resp = APIClient().get(f'/api/public/v1/exports/produits.csv?token={self.raw}')
    ...
    def test_write_only_key_cannot_pull(self):
        ...
        resp = APIClient().get(f'/api/public/v1/exports/leads.csv?token={write_raw}')
        self.assertEqual(resp.status_code, 403)

    def test_unknown_entity_is_400(self):
        resp = APIClient().get(f'/api/public/v1/exports/inconnue.csv?token={self.raw}')
    ...
    def test_produits_key_can_pull_produits_never_purchase_price(self):
        ...
        resp = APIClient().get(f'/api/public/v1/exports/produits.csv?token={raw2}')
    ...
    def test_authorization_header_alone_is_not_accepted(self):
        ...
        resp = api.get('/api/public/v1/exports/leads.csv')
        self.assertEqual(resp.status_code, 401)
