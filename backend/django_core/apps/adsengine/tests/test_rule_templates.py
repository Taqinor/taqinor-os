...
class CatalogueShapeTests(SimpleTestCase):
    def test_exactly_eight_plan_templates(self):
        self.assertEqual(set(rt.RULE_TEMPLATES), EXPECTED_KEYS)
        self.assertEqual(len(rt.RULE_TEMPLATES), 8)
...
class SeedDefaultPoliciesTests(TestCase):
    ...
    def test_seed_creates_eight_safe_off_dry_run(self):
    ...
    def test_seed_is_idempotent(self):
        ...
        self.assertEqual(
            RulePolicy.objects.filter(company=self.company).count(), 8)
...
class CatalogueApiTests(TestCase):
    ...
    def test_catalogue_lists_eight_templates(self):
    ...
    def test_seed_endpoint_creates_and_is_idempotent(self):
        ...
        self.assertEqual(resp2.data['total'], 8)
