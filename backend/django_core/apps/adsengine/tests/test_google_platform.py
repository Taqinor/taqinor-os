"""ADSENG50 — Tests de contrat de l'adaptateur Google Ads (STUB, rien d'activé).

Prouve : le stub satisfait le contrat ``AdsPlatform`` ; il n'active RIEN (chaque
opération lève) ; l'invariant #3 survit (aucun ``status`` de création, aucune
méthode d'activation) ; la plateforme reste GATED (absente de la matrice ADSENG49).
"""
from django.test import SimpleTestCase

from apps.adsengine.platforms import capabilities as caps
from apps.adsengine.platforms.base import AdsPlatform
from apps.adsengine.platforms.google import (
    GoogleAdsPlatform, PlatformNotEnabledError,
)


class GoogleStubContractTests(SimpleTestCase):
    def setUp(self):
        self.platform = GoogleAdsPlatform()

    def test_is_an_adsplatform(self):
        self.assertIsInstance(self.platform, AdsPlatform)
        self.assertEqual(self.platform.name, 'google')

    def test_stays_gated_out_of_capability_matrix(self):
        # GATED : jamais dans la matrice construite (ADSENG49 reste Meta-only).
        self.assertNotIn('google', caps.known_platforms())
        entry = self.platform.capabilities()
        self.assertEqual(entry['platform'], 'google')
    ...
    def test_every_operation_is_gated_no_spend(self):
        for call in (
            ...
            lambda: self.platform.create_campaign(name='C', objective='SEARCH'),
    ...
    def test_no_create_path_accepts_status(self):
        with self.assertRaises(TypeError):
            self.platform.create_campaign(
                name='C', objective='SEARCH', status='ACTIVE')
        with self.assertRaises(TypeError):
